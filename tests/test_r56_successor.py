import unittest
from tools import r56_successor as r

class R56Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls): cls.manifest,cls.refs,cls.ids,cls.sources,cls.names=r.evidence()
    def test_scope(self):
        self.assertEqual((len(self.ids),sum(len(s['operations']) for s in self.sources)),(76,86))
        self.assertEqual({s['source'] for s in self.sources},{'stacks.tex'})
