import unittest
from tools import r52_successor as r

class R52Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls): cls.manifest,cls.refs,cls.ids,cls.sources,cls.names=r.evidence()
    def test_scope(self):
        self.assertEqual((len(self.ids),sum(len(s['operations']) for s in self.sources)),(83,107))
        self.assertEqual({s['source'] for s in self.sources},{'categories.tex'})
