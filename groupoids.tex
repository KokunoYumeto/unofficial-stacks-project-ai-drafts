...
$R \subset A \times A$. We usually write $a R b$ to indicate
...
we see that $j$ is unramified. Then
...
\item (inverse) for all $g \in G$ there exists an $i(g) \in G$
...
& \leftarrow &
\mathbf{Z}[x_{ij}, 1/d] \\
\sum_k x_{ik} \otimes x_{kj} & \leftarrow & x_{ij}
\end{eqnarray*}
...
\begin{lemma}
\label{lemma-connected-group-power-surjective}
\begin{reference}
\cite[Expos\'e 236, proof of Theorem 1.1(iv), p. 227]{FGA}.
\end{reference}
Let $k$ be a field and let $G$ be a connected commutative group scheme
locally of finite type over $k$. If $n \geq 1$ is invertible in $k$, then
$$
...
\begin{proof}
We may extend the ground field to an algebraic closure. The reduction
$G_{red}$ is a subgroup scheme by
Lemma \ref{lemma-reduced-subgroup-scheme-perfect}; it is connected and has
the same underlying topological space as $G$. It is smooth by
Lemma \ref{lemma-reduced-group-scheme-prefect-field-characteristic-p-smooth}
in positive characteristic and by
Lemma \ref{lemma-group-scheme-characteristic-zero-smooth} in characteristic
zero.

The differential of $[n]$ at the identity of $G_{red}$ is multiplication
by $n$, by repeated application of
Lemma \ref{lemma-group-scheme-addition-tangent-vectors}. It is therefore an
isomorphism. Translation shows that $[n]$ is \`etale everywhere on
$G_{red}$. Its image is consequently an open subgroup of $G_{red}$, and it
is closed by Lemma \ref{lemma-open-subgroup-closed-over-field}. Connectedness
gives $[n](G_{red})=G_{red}$. This proves surjectivity on underlying
topological spaces for $G$, which is the assertion.
\end{proof}

\begin{lemma}
\label{lemma-smooth-flat-locus-group-scheme}
Let $S$ be a scheme and let $f : G \to S$ be a group scheme locally of
finite presentation. Let $\mathcal{P}$ be either ``smooth'' or ``flat''.
Denote by $W \subset G$ the locus where $f$ has property $\mathcal{P}$,
and set
$$
U = e^{-1}(W) \subset S.
$$
Then $U$ is open in $S$, the open subscheme
$W_U = W \times_S U$ is an open subgroup scheme of $G_U$, and every
section of $G_U \to U$ factors through $W_U$.
\end{lemma}

\begin{proof}
The smooth locus is open by the definition of a smooth morphism. The
flat locus is open by More on Morphisms, Theorem
\ref{more-morphisms-theorem-openness-flatness}, applied to
$\mathcal{O}_G$. Thus $U$ is open. Formation of $W$ commutes with flat
base change. In the smooth case this follows from Morphisms, Lemma
\ref{morphisms-lemma-set-points-where-fibres-smooth}, and in the flat
case from More on Morphisms, Lemma
\ref{more-morphisms-lemma-flat-locus-base-change}.

We may replace $S$ by $U$ and assume that $e(S) \subset W$. In either
case $W \to S$ is flat. Consider the base change $G_W = W \times_S G$.
The property $\mathcal{P}$ locus in $G_W$ is $W \times_S W$. The
translation by the universal point
$$
G_W \longrightarrow G_W, \qquad (w, g) \longmapsto (w, m(w, g))
$$
is an isomorphism over $W$, and hence it preserves this locus. It
follows that multiplication on $G$ maps $W \times_S W$ into $W$.
The inverse morphism of $G$ preserves $W$ as well, since it is an
isomorphism over $S$. As $e(S) \subset W$, this proves that $W$ is an
open subgroup scheme of $G$.

Let $\sigma : S \to G$ be a section. Translation by $\sigma$ is an
isomorphism of $G$ over $S$ and therefore preserves $W$. Since it maps
$e(S)$ onto $\sigma(S)$, the section $\sigma$ factors through $W$.
\end{proof}

\begin{lemma}
\label{lemma-smooth-flat-along-identity-component}
\begin{reference}
\cite[Expos\'e 236, Corollary 1.12 and Proposition 2.10(i),
pp. 230 and 235--236]{FGA}.
\end{reference}
In the situation of Lemma \ref{lemma-smooth-flat-locus-group-scheme},
assume that $f$ has property $\mathcal{P}$ at every point of $e(S)$.
Then $f$ has property $\mathcal{P}$ at every point in the image of any
section of $f$ and at every point of
$$
G^0 = \bigcup_{s \in S} (G_s)^0,
$$
where $(G_s)^0$ is the connected component of the identity in $G_s$.
\end{lemma}

\begin{proof}
Lemma \ref{lemma-smooth-flat-locus-group-scheme} shows that the
$\mathcal{P}$ locus $W$ is an open subgroup scheme of $G$ containing
every section. For $s \in S$, the subgroup $W_s \subset G_s$ is both
open and closed. Indeed, if $x \in G_s \setminus W_s$ and
$K = \kappa(x)$, then translation by the corresponding $K$-point of
$G_K$ maps the open $W_K$ to an open whose image in $G_s$ is an open
neighbourhood of $x$ disjoint from $W_s$. Here the projection
$G_K \to G_s$ is open by Morphisms, Lemma
\ref{morphisms-lemma-scheme-over-field-universally-open}. Thus the
complement of $W_s$ is open. Since $W_s$ contains the identity, it
contains the connected component $(G_s)^0$.
\end{proof}
...
Let $(G, m)$ be a group scheme over a field $k$. Let $U \subset G$
...
$W \to G$ is quasi-compact by Schemes, Lemma
...
To show that $G^0$ is a closed subgroup scheme we will use the
...
\begin{lemma}
\label{lemma-reduced-identity-component-no-additive}
\begin{reference}
\cite[Expos\'e 236, Proposition 3.1, p. 236 and correction,
p. 303]{FGA}.
\end{reference}
Let $k$ be a field and let $G$ be a commutative group scheme of finite type
over $k$. Let $\overline{k}$ be an algebraic closure of $k$. Assume that
$G^0_{\overline{k}}$ contains no closed subgroup scheme isomorphic to
$\mathbf{G}_{a, \overline{k}}$. Then $(G^0)_{red}$ is a smooth connected
closed subgroup scheme of $G$. Moreover,
$$
((G^0)_{red})_{\overline{k}}=(G^0_{\overline{k}})_{red}
$$
as closed subschemes of $G_{\overline{k}}$.
\end{lemma}

\begin{proof}
Set $\overline{G}=G_{\overline{k}}$ and
$H=(\overline{G}^0)_{red}$. Since $\overline{k}$ is perfect, $H$ is a closed
subgroup scheme of $\overline{G}$ by
Lemma \ref{lemma-reduced-subgroup-scheme-perfect}, and it is smooth by
Lemma \ref{lemma-reduced-group-scheme-prefect-field-characteristic-p-smooth}
in positive characteristic and by
Lemma \ref{lemma-group-scheme-characteristic-zero-smooth} in characteristic
zero.

The standard structure theorem for connected commutative algebraic groups
says that the hypothesis on additive subgroups makes $H$ an extension of an
abelian variety by a torus. Consequently multiplication by every positive
integer on $H$ is surjective. This is immediate for a torus, follows from
Lemma \ref{lemma-degree-multiplication-by-d} for an abelian variety, and
passes to an extension in the fppf topology.

By the standard quotient theorem for algebraic groups over a field, the fppf
quotient $Q=\overline{G}/H$ is represented by a group scheme. It is finite:
its underlying space is the finite set of connected components of
$\overline{G}$ and its identity component is the finite infinitesimal quotient
of $\overline{G}^0$ by its reduction. Choose $m>0$ annihilating $Q$. Such an
$m$ exists because the finite \`etale quotient of $Q$ has finite exponent and,
in positive characteristic, the connected part of $Q$ is killed by a power of
the characteristic. It follows that
$$
[m] : \overline{G}\longrightarrow\overline{G}
$$
factors through $H$. Its restriction to $H$ is surjective, so the scheme
theoretic image of $[m]$ is exactly $H$; here we also use that $H$ is reduced
and Morphisms, Lemma
\ref{morphisms-lemma-scheme-theoretic-image-reduced}.

Let $Z\subset G$ be the scheme theoretic image of $[m]:G\to G$. The morphism
$[m]$ is quasi-compact. Hence formation of its scheme theoretic image
commutes with the flat base change $\Spec(\overline{k})\to\Spec(k)$ by
Morphisms, Lemma
\ref{morphisms-lemma-flat-base-change-scheme-theoretic-image}. Thus
$Z_{\overline{k}}=H$. In particular $Z$ is smooth and geometrically
connected over $k$. The identity, inverse, and multiplication maps make $Z$
a closed subgroup scheme because this can be checked after the faithfully
flat extension $\overline{k}/k$. Finally $Z$ has the identity component of
$G$ as its underlying space and is reduced. Therefore $Z=(G^0)_{red}$, and
the displayed base change identity follows.
\end{proof}
...
of $G$ and are the translates of $G^0$ by our $g_i$. We claim that
$$
...
$A \subset G_{\overline{k}}$ be the closed subgroup scheme representing
...
$k$\footnote{For equivalent definitions see Remark \ref{remark-16-equivalent}.}.
\end{definition}
...
reduced'', then we recover Definition \ref{definition-abelian-variety}
...
Lemmas \ref{lemma-connected-group-scheme-over-field-irreducible} and
...
\ref{proposition-review-abelian-varieties}), whence satisfies
...
a $G$-equivariant isomorphism $G \to X$ over $S$ where $G$ acts on
...
\end{enumerate}
In general the answers to these questions are no. To get a positive answer
...
\begin{lemma}
\label{lemma-smooth-torsor-etale}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 6, p. 326]{FGA}.
\end{reference}
Let $G \to S$ be a smooth group scheme. Every $G$-torsor over $S$
is a $G$-torsor in the \`etale topology.
\end{lemma}

\begin{proof}
Let $X$ be a $G$-torsor. There is an fpqc covering
$\{S_i \to S\}$ such that $X_{S_i} \cong G_{S_i}$. Hence $X \to S$
is smooth by Descent, Lemma
\ref{descent-lemma-descending-property-smooth}. It is surjective because
this is true after the covering $\{S_i \to S\}$. For every $s \in S$,
More on Morphisms, Lemma
\ref{more-morphisms-lemma-etale-nbhd-dominates-smooth}
gives an \`etale neighbourhood $S' \to S$ of $s$ and a section
$S' \to X$. These neighbourhoods form an \`etale covering of $S$.
\end{proof}

\begin{lemma}
\label{lemma-torsors-special-linear-symplectic}
\begin{reference}
\cite[Expos\'e 190, Proposition 6.1 and the discussion following it,
p. 326]{FGA}.
\end{reference}
Let $S$ be a scheme and let $n \geq 1$. Write
$$
\text{SL}_{n, S} = \Ker(\det :
\text{GL}_{n, S} \to \mathbf{G}_{m, S})
$$
and let $\text{Sp}_{2n, S} \subset \text{GL}_{2n, S}$ be the group
scheme preserving the standard perfect alternating form. Every torsor under
$\text{SL}_{n, S}$ or $\text{Sp}_{2n, S}$ is locally trivial. The same is
true for torsors under finite products of these groups and general linear
groups.
\end{lemma}

\begin{proof}
Let $P$ be an $\text{SL}_{n, S}$-torsor. Choose an fpqc covering
trivializing $P$. The transition matrices belong to $\text{SL}_n$.
By fpqc descent they define a finite locally free $\mathcal{O}_S$-module
$\mathcal{E}$ of rank $n$ together with an isomorphism
$$
\epsilon : \wedge^n\mathcal{E} \longrightarrow \mathcal{O}_S.
$$
Here we use Descent, Proposition
\ref{descent-proposition-fpqc-descent-quasi-coherent} and Lemma
\ref{descent-lemma-finite-locally-free-descends}. The same descent
identifies $P$ with the scheme of frames $(e_1, \ldots, e_n)$ of
$\mathcal{E}$ such that
$\epsilon(e_1 \wedge \ldots \wedge e_n) = 1$. Zariski locally
$\mathcal{E}$ is free. If the value of $\epsilon$ on a chosen frame is
$u \in \mathcal{O}_S^*$, then replacing $e_1$ by $u^{-1}e_1$ gives a
section of $P$.

The same argument for an $\text{Sp}_{2n, S}$-torsor gives a finite locally
free module $\mathcal{E}$ of rank $2n$ with a perfect alternating form
$b$, and identifies the torsor with the scheme of symplectic frames.
Such frames exist Zariski locally. Indeed, over a local ring choose a
basis vector $e$. Since $b$ is perfect, there is an $f$ such that
$b(e, f)$ is a unit; rescale $f$ so that $b(e, f) = 1$. The span of
$e, f$ is a nondegenerate direct summand, and its orthogonal complement
is again finite free with a perfect alternating form. Induction produces
a symplectic basis. The resulting basis over a local ring spreads out to
an open neighbourhood.

Finally, the projections of a trivializing cocycle for a torsor under a
finite product give torsors under the factors. Their fibre product has the
original cocycle. A common Zariski refinement which trivializes the factor
torsors therefore trivializes the original torsor.
\end{proof}

\begin{lemma}
\label{lemma-subgroup-torsor-local-triviality}
\begin{reference}
\cite[Expos\'e 190, discussion following Proposition 6.1, p. 326]{FGA}.
\end{reference}
Let $H \to G$ be a homomorphism of affine group schemes over $S$. Assume
the fppf quotient $H \backslash G$ is represented by a scheme $Q$ and
$G \to Q$ is an $H$-torsor. Assume also that every $G_T$-torsor is
Zariski locally trivial for every scheme $T$ over $S$.

Let $\mathcal{C}$ be either the class of finite faithfully flat morphisms
or the class of surjective \`etale morphisms. If $G \to Q$ is, Zariski
locally on $Q$, trivialized by a morphism in $\mathcal{C}$, then every
$H_T$-torsor is, Zariski locally on $T$, trivialized by a morphism in
$\mathcal{C}$.
\end{lemma}

\begin{proof}
Let $P$ be an $H_T$-torsor. Choose an fpqc covering trivializing $P$ and
view its $H$-valued transition functions as $G$-valued transition
functions. Affine fpqc descent, Descent, Lemma
\ref{descent-lemma-affine}, produces a $G_T$-torsor $E$ and an
$H$-equivariant morphism $P \to E$. Zariski locally on $T$ the torsor
$E$ is trivial, so we may assume that $E = G_T$.

The $H$-equivariant morphism $P \to G_T$ descends to a morphism
$q : T \to Q$. Moreover, the diagram
$$
\xymatrix{
P \ar[r] \ar[d] & G \ar[d] \\
T \ar[r]^q & Q
...
$$
is cartesian. This can be checked after the fpqc covering $P \to T$,
where it is the standard description of the quotient. Pulling back the
assumed local trivializations of $G \to Q$ along $q$ proves the result.
\end{proof}

\begin{remark}
\label{remark-linear-subgroup-torsors}
In Lemma \ref{lemma-subgroup-torsor-local-triviality}, one may take
$G = \text{GL}_{n, S}$ by Remark \ref{remark-fun-with-torsors}.
Inversion identifies $H \backslash G$ with $G/H$, exchanging left and
right actions. Thus the lemma applies in either quotient convention to a
flat closed subgroup $H \subset \text{GL}_{n, S}$ whenever the quotient
and the displayed universal $H$-torsor exist. It gives both the finite
faithfully flat and the \`etale conclusions discussed in the reference.
\end{remark}
...
we find that
$$
\sum (f_n)_m \otimes x^m \otimes x^n = \sum f_n \otimes x^n \otimes x^n
$$
...
s^*s_*t^*\mathcal{G}
$$
The middle equality holds because $t \circ c = t \circ \text{pr}_0$ as
morphisms $R \times_{s, U, t} R \to U$, and the first and last
equalities hold because we know that base change and pushforward commute in
...
such that $(\mathcal{F}, \alpha)$ is a quasi-coherent module on
$(U, R, s, t, c)$. By
...
$W_{ijk} = \Spec(B_{ijk})$.
Let $\kappa$ be any infinite cardinal $\geq$ the cardinality
...
$i \in I$, $k \in I_{ii}$, and $m \in M_i$. Fix such a collection $S(i, j, k, m)$
...
turns $R_p$ into a $G_{\kappa(p)}$-torsor over $\kappa(p)$.
\end{lemma}
...
We sometimes use the notation $R' = R|_{U'}$ in this case.
\end{definition}
...
Since $t$ is open the set $W' = t(s^{-1}(W))$ is an open subset of $U$.
...
Let $S$ be a scheme.
Let $(U, R, s, t, c)$ be a groupoid scheme over $S$.
Let $g : U' \to U$ be a morphism of schemes over $S$.
...
length elsewhere; here we just discuss some
...
\begin{reference}
\cite[Expos\'e 212, Th\'eor\`eme 5.1 (i), p. 110]{FGA}.
\end{reference}
Let $S$ be a scheme. Let $(U, R, s, t, c)$ be a groupoid scheme over $S$.
Assume $U = \Spec(A)$ and $R = \Spec(B)$ are affine and
$s, t : R \to U$ finite locally free.
...
are finite locally free of rank $r$. Pulling back by $U' = \Spec(A') \to U$
...
\label{lemma-points}
\begin{reference}
\cite[Expos\'e 212, Th\'eor\`eme 5.1 (ii) and Corollaire 5.2,
pp. 110--111]{FGA}.
\end{reference}
Let $S$ be a scheme. Let $(U, R, s, t, c)$ be a groupoid scheme over $S$.
Assume $U = \Spec(A)$ and $R = \Spec(B)$ are affine and
...
$$
(t, s) : R \longrightarrow U \times_M U
$$
is surjective.
\end{enumerate}
\end{lemma}
...
hence its norm is not a unit. We conclude that $C^1$ contains
...
\medskip\noindent
Finally, let $z \in U \times_M U$ be a point and let $k$ be an algebraic
closure of $\kappa(z)$. The corresponding geometric point of
$U \times_M U$ is a pair of points $u_0, u_1 \in U(k)$ with the same image
in $M(k)$. By (2), there is an $r \in R(k)$ mapping to $(u_0, u_1)$.
Consequently $z$ is in the image of $(t, s)$, which proves (3).
\end{proof}

\begin{lemma}
\label{lemma-etale}
Let $S$ be a scheme. Let $f : (U', R', s', t', c') \to (U, R, s, t, c)$ be a
...
Then $M_1 = \lim M_i$ and similarly for the other schemes.
...
$$
Moreover the two left squares are cocartesian in the category of rings, and
...
in particular that $s$ (and hence also the isomorphic arrow $t$)
is faithfully flat.
...
\begin{reference}
\cite[Expos\'e 212, Th\'eor\`eme 5.1 (iv), p. 111]{FGA}.
\end{reference}
Let $S$ be a scheme.
Let $(U, R, s, t, c)$ be a groupoid scheme over $S$.
Assume
...
\begin{reference}
\cite[Expos\'e 212, proof of Th\'eor\`eme 5.3, pp. 111--112]{FGA}.
\end{reference}
Let $S$ be a scheme.
Let $(U, R, s, t, c)$ be a groupoid scheme over $S$.
...
\begin{proof}
As in the proof of Lemma \ref{lemma-descend-along-finite-with-bound}, we
...
Lemma \ref{lemma-descend-along-finite-with-bound}, which completes the proof.
\end{proof}
...
\begin{lemma}
\label{lemma-projective-after-finite-locally-free-base-change}
Let $S' \to S$ be a surjective finite locally free morphism of
constant degree. Let $X \to S$ be a morphism. If
$X' = S' \times_S X \to S'$ is projective, then $X \to S$ is projective.
\end{lemma}

\begin{proof}
The morphism $X \to S$ is proper by
Descent, Lemma \ref{descent-lemma-descending-property-proper}.
Choose an $S'$-ample invertible sheaf $\mathcal{L}'$ on $X'$; this is
possible by Morphisms, Lemma
\ref{morphisms-lemma-projective-quasi-projective}. The projection
$\pi : X' \to X$ is surjective finite locally free. The finite locally
free norm and the norm of an invertible sheaf
(Divisors, Lemmas \ref{divisors-lemma-finite-locally-free-has-norm}
and \ref{divisors-lemma-norm-invertible}) give an invertible sheaf
$$
\mathcal{N} = \mathop{\rm Norm}\nolimits_\pi(\mathcal{L}')
$$
on $X$.

We claim that $\mathcal{N}$ is $S$-ample. This may be checked over affine
opens $U \subset S$. Then $S'_U$ is affine and
$\mathcal{L}'|_{X'_U}$ is ample. The norm construction commutes with
restriction to open subschemes. Hence
$\mathcal{N}|_{X_U}$ is ample by
Divisors, Lemma \ref{divisors-lemma-norm-ample}. Thus $X \to S$ is
quasi-projective. Since it is also proper, it is projective by
Morphisms, Lemma
\ref{morphisms-lemma-projective-is-quasi-projective-proper}.
\end{proof}

\begin{lemma}
\label{lemma-descend-along-fpqc-radicial}
\begin{reference}
\cite[Expos\'e 190, Th\'eor\`eme 3, p. 318]{FGA}
\end{reference}
Let $X \to Y$ be a surjective, quasi-compact, flat, and radicial morphism.
...
Let $\varphi$ be a descent datum as in
Descent, Definition \ref{descent-definition-descent-datum}.
Since $X \to Y$ is radicial, the diagonal
$X \to X \times_Y X$ is surjective by
Morphisms, Lemma \ref{morphisms-lemma-universally-injective}.

Let $v \in V$ and choose an affine open $A \subset X$ containing the
image of $v$. The inverse images of $A$ by the two projections
$X \times_Y X \to X$ agree: their inverse images by the surjective
diagonal agree, and open subsets are determined by their underlying sets.
By Descent, Lemma \ref{descent-lemma-open-fpqc-covering}, there is an open
$U \subset Y$ such that $A = X \times_Y U$. After replacing everything by
its restriction over $U$, we may assume that $X$ is affine.

Choose an affine open neighbourhood $W \subset V$ of $v$. The two opens
$$
W \times_Y X
\quad\text{and}\quad
\varphi^{-1}(X \times_Y W)
$$
of $V \times_Y X$ have the same inverse image by the map
$V \to V \times_Y X$ induced by the diagonal. This map is surjective on
underlying topological spaces, and the pullback of $\varphi$ by the diagonal
is the identity. Hence the two opens agree. Thus $W$ is
$\varphi$-invariant. The morphism $W \to X$ is affine, so the restricted
descent datum is effective by
Descent, Lemma \ref{descent-lemma-affine}. We have found such a neighbourhood
for every $v$, and the result follows from
Descent, Lemma \ref{descent-lemma-effective-for-fpqc-is-local-upstairs}.
\end{proof}
...
\begin{proof}
Apply Lemma \ref{lemma-descend-along-fpqc-radicial}.
\end{proof}
