...
Let $x, y \in \Ob(\mathcal{C})$.
A {\it coproduct}, or {\it sum} of $x$ and $y$ is
...
object of $\mathcal{C}/X$ on $W$ and $(\varphi_i)$ defines an
...
Let $\mathcal{I}$ and $\mathcal{J}$ be categories and denote
...
$q_{y_i} : W \to M(F(y_i))$ and $q_{z_i} : W \to M(F(z_i))$
as in
Definition \ref{definition-limit}
...
$$
we conclude that $q_{y_i} = q_{z_i}$ for all $i$. Set $q_i$ equal to this common
value. The compatibility of
$q_{s(j)} = q_{y_{s(j)}}$ and $q_{t(j)} = q_{z_{t(j)}}$ with the morphism
...
\noindent
It turns out we sometimes need a more fine-grained control over the
...
condition in Lemma \ref{lemma-directed-commutes}
...
$i = i_0, i_1, \ldots, i_{2n} = i'$ of objects of $\mathcal{I}$
and morphisms
$$
\xymatrix{
&
...
generated subcategory of $\mathcal{I} \times \omega$ which has $i_\infty$ as
...
Note that a finite directed set $(I, \leq)$ always has a greatest object
...
colimit of a diagram $M : \mathcal{I} \to \textit{Sets}$ is the image of the
...
$M = \bigoplus_{n \geq 0} \mathbf{Z}$ and let $S : M \to M$ be the
...
$$
commutes. After replacing $m(n)$ by $\max_{l \leq k \leq n}\{m(n, k), m(k, l)\}$
...
two pairs $(m_1, a_1)$ and $(m_2, a_2)$ as above then these define the same
...
\begin{definition}
\label{definition-strict-prorepresentable-minimal-pair}
Let $\mathcal{C}$ be a category.
\begin{enumerate}
\item A monomorphism $u : X' \to X$ is called a
{\it strict monomorphism} if the following holds: a morphism
$v : Z \to X$ factors through $u$ whenever
$$
a \circ v = b \circ v
$$
for every pair $a, b : X \to W$ such that
$a \circ u = b \circ u$.
\item A pro-object is called {\it strict} if it can be represented by a
cofiltered diagram all of whose transition morphisms are epimorphisms.
\item A functor $F : \mathcal{C} \to \textit{Sets}$ is called
{\it strictly prorepresentable} if there is a strict pro-object
$(X_i)$ and an isomorphism
$$
F(Y) = \colim_i \Mor_\mathcal{C}(X_i, Y)
$$
functorial in $Y$.
\item A {\it pointed object of $F$} is a pair $(X, \xi)$ with
$\xi \in F(X)$. A morphism $(X, \xi) \to (Y, \eta)$ is a morphism
$f : X \to Y$ such that $F(f)(\xi) = \eta$.
\item A pointed object $(X, \xi)$ is {\it minimal} if every morphism
$(X', \xi') \to (X, \xi)$ whose underlying morphism is a strict
monomorphism is an isomorphism. We say that $(X, \xi)$
{\it dominates} $(Y, \eta)$ if there is a morphism
$(X, \xi) \to (Y, \eta)$.
\end{enumerate}
\end{definition}

\begin{lemma}
\label{lemma-strict-monomorphism-properties}
Let $\mathcal{C}$ be a category.
\begin{enumerate}
\item Every equalizer is a strict monomorphism.
\item A strict monomorphism which is also an epimorphism is an isomorphism.
\end{enumerate}
\end{lemma}

\begin{proof}
Suppose that $u : E \to X$ is the equalizer of $a, b : X \to Y$.
If $v : Z \to X$ satisfies the condition in
Definition \ref{definition-strict-prorepresentable-minimal-pair}, then in
particular $a \circ v = b \circ v$. Thus $v$ factors through $E$.

Suppose that $u : X' \to X$ is both a strict monomorphism and an
epimorphism. Every pair $a, b : X \to Y$ satisfying
$a \circ u = b \circ u$ has $a = b$. Hence the identity of $X$ factors
through $u$. The resulting inverse on one side is an inverse on both sides
because $u$ is a monomorphism.
\end{proof}

\begin{theorem}
\label{theorem-strict-prorepresentable-minimal-pairs}
\begin{reference}
\cite[Expos\'e 195, n\textsuperscript{o} A.3, pp. 373--374]{FGA}.
\end{reference}
Let $\mathcal{C}$ be a small category with finite limits and let
$F : \mathcal{C} \to \textit{Sets}$ be a functor. The following are
equivalent.
\begin{enumerate}
\item The functor $F$ is strictly prorepresentable.
\item The functor $F$ is left exact and every pointed object of $F$ is
dominated by a minimal pointed object.
\end{enumerate}
\end{theorem}

\begin{proof}
Suppose that $F$ is represented by a cofiltered diagram $(X_i)$ whose
transition morphisms are epimorphisms. A filtered colimit of sets commutes
with finite limits. Since each functor
$\Mor_\mathcal{C}(X_i, -)$ commutes with limits, $F$ is left exact.

Let $\xi \in F(Y)$. It is represented by a morphism $g : X_i \to Y$.
Denote by $\xi_i \in F(X_i)$ the class of $\text{id}_{X_i}$. We claim that
$(X_i, \xi_i)$ is minimal. Suppose that $u : Y' \to X_i$ is a strict
monomorphism and that $\eta \in F(Y')$ maps to $\xi_i$. Represent $\eta$
by a morphism $X_j \to Y'$. The equality in the filtered colimit implies,
after passing to a common refinement $X_k$, that a transition morphism
$X_k \to X_i$ factors through $u$. This transition morphism is an
epimorphism, hence $u$ is an epimorphism. Lemma
\ref{lemma-strict-monomorphism-properties} shows that $u$ is an isomorphism.
Thus $(X_i, \xi_i)$ is minimal, and $g$ shows that it dominates
$(Y, \xi)$.

Conversely, suppose (2) holds. Let $\mathcal{I}$ be the category of minimal
pointed objects of $F$. It is small. We first show that between two objects
of $\mathcal{I}$ there is at most one morphism. Indeed, the equalizer of two
such morphisms carries a point lifting the point of their source because
$F$ is left exact. The equalizer is a strict monomorphism by Lemma
\ref{lemma-strict-monomorphism-properties}, and minimality of the source
shows that it is an isomorphism.

The category $\mathcal{I}$ is cofiltered. It is nonempty: apply the
domination hypothesis to the unique point of the final object. Given two
minimal pointed objects, form their product as pointed objects and dominate
it by a minimal pointed object. This gives a common source. There is no
condition left to check for parallel arrows by the preceding paragraph.

The forgetful functor $\mathcal{I} \to \mathcal{C}$ is a strict
pro-object. To see this, let
$$
f : (X, \xi) \longrightarrow (Y, \eta)
$$
be a morphism in $\mathcal{I}$. If $a, b : Y \to Z$ satisfy
$a \circ f = b \circ f$, then the equalizer $E \to Y$ carries a point
lifting $\eta$. Minimality of $(Y, \eta)$ shows that $E \to Y$ is an
isomorphism. Hence $a = b$, and $f$ is an epimorphism.

There is a natural map
$$
\colim_{(X, \xi) \in \mathcal{I}^{opp}}
\Mor_\mathcal{C}(X, Y) \longrightarrow F(Y),
\qquad
g \longmapsto F(g)(\xi).
$$
It is surjective by the domination hypothesis. Suppose that
$g : (X, \xi) \to (Y, \zeta)$ and
$h : (X', \xi') \to (Y, \zeta)$ give the same point of $F(Y)$.
The fibre product $X \times_Y X'$ carries the point $(\xi, \xi')$
because $F$ is left exact. Dominate this pointed object by a minimal one.
The resulting common refinement identifies the classes of $g$ and $h$.
Thus the displayed map is injective and $F$ is strictly prorepresentable.
\end{proof}

\begin{proposition}
\label{corollary-artinian-left-exact-strict-prorepresentable}
Let $\mathcal{C}$ be a small category with finite limits. Assume that every
nonempty collection of subobjects of any object of $\mathcal{C}$ has a
minimal member. Then a functor
$F : \mathcal{C} \to \textit{Sets}$ is strictly prorepresentable if and
only if it is left exact. In particular, every pro-object of $\mathcal{C}$
is isomorphic to a strict pro-object.
\end{proposition}

\begin{proof}
Only the converse requires proof. Given a pointed object $(X, \xi)$,
consider the nonempty collection of subobjects $X' \to X$ through which
$\xi$ lifts. Choose a minimal member and a lift $\xi' \in F(X')$.
Then $(X', \xi')$ is minimal, since any further strict subobject carrying
$\xi'$ would give a smaller subobject of $X$. Theorem
\ref{theorem-strict-prorepresentable-minimal-pairs} applies.

The functor associated with any pro-object is left exact. Hence it is
strictly prorepresentable by the first assertion. Remark
\ref{remark-pro-category-copresheaves} then shows that the original
pro-object is isomorphic to a strict one.
\end{proof}
...
\begin{proof}
This lemma is dual to Lemma \ref{lemma-characterize-essentially-constant-ind}.
\end{proof}
...
$$
is a bijection, where the first map is the natural one and
...
system of morphisms from the constituents of the colimit into the
...
morphism such that the induced map $\Mor(C, v_1D) \to \Mor(C, v_2D)$
...
and $X = \colim_{i \in I} X_i$ and $Y = \colim_{j \in J} Y_j$
are written as filtered colimits of objects in $\mathcal{C}'$.
...
is a filtered colimit in $\mathcal{C}$. Since $F$ is a functor
...
makes sense to define $q \circ p$ as the equivalence class of
...
Composition of transformations of functors such as
$$
\xymatrix{
\mathcal{A}
\rruppertwocell^{F''}{t'}
...
of transformations of functors give rise to a functor
$$
(\circ, \star) :
\text{Fun}(\mathcal{B}, \mathcal{C})
\times
...
and subcategories $\Mor_{\mathcal{C}'}(x, y)$ of the
categories $\Mor_\mathcal{C}(x, y)$ for all
$x, y \in \Ob(\mathcal{C}')$ such that these, together with
the operations $\circ$ (composition of $1$-morphisms), $\circ$ (vertical
composition of $2$-morphisms), and $\star$ (horizontal composition)
form a $2$-category.
\end{definition}
...
$\mathcal{C}$ is a proper class, but for all objects $x, y \in \Ob(\mathcal{C})$
...
``usual'' functor into the category formed out of the 2-category by forgetting
all the 2-morphisms.
...
\item for every $x\in \Ob(\mathcal{A})$ a $2$-morphism
...
(w', a', b', \phi')$ the composition of $1$-morphisms
...
a final object in the $2$-category of $2$-commutative diagrams
...
from $(\mathcal{W}, a, b, t)$ to
$(\mathcal{A} \times_\mathcal{C} \mathcal{B}, p, q, \psi)$.

\medskip\noindent
Let
$(k, \alpha', \beta') :
(\mathcal{W}, a, b, t) \to (\mathcal{A} \times_\mathcal{C} \mathcal{B}, p, q, \psi)$
...
Also, identify $M(Z)$ with $F(L(X))$ via $\beta_X$ and
identify $M(Z)$ with $G(K(Y))$ via $\alpha_Y$. Similarly for
$Z' = H(X')$ and $M(Z')$.
...
Then the full faithfulness of $M$ guarantees that we can
...
We claim these constructions give mutually quasi-inverse functors.
We omit describing how to deal with morphisms
and showing they are mutually quasi-inverse.
\end{proof}
...
that they hold in $\textit{Cat}$. To see this one can also use that
...
$x\in \Ob(\mathcal{S}_U)$. We will also sometimes say
...
Lemma \ref{lemma-2-product-categories-over-C} and of categories in
Lemma \ref{lemma-2-fibre-product-categories}
(as in Example \ref{example-2-fibre-product-categories})
...
$2$-fibre product $\mathcal{X} \times_\mathcal{S} \mathcal{X}$
as categories over $\mathcal{S}$ is the discrete category with one object.
The difference is that (in the notation of
Lemma \ref{lemma-2-product-categories-over-C}), we were allowed to choose
...
Indeed, the endo-functor of $\mathcal{I}_{\mathcal{S}/\mathcal{S}'}$ induced
...
defines a pseudo functor from $\mathcal{C}^{opp}$ into
...
A' \ar@{-->}[u]^{??} \ar[ru]_{h'} & & \ar@{}[u]^{\text{above}} &
...
We claim the $2$-fibre product of Lemma \ref{lemma-2-product-fibred-categories}
...
(see Lemma \ref{lemma-2-product-categories-over-C})
...
induces an injection (resp.\ bijection) from morphisms
...
\medskip\noindent
Finally suppose $G_U$ is an equivalence for all $U$, so it is
...
categories over $\mathcal{C}$. As a functor $\mathcal{X} \to \mathcal{X}'$ we take
...
in $\mathcal{S}'_V$ we set $g^\ast(\varphi, \text{id}_V) =
...
its set of objects. Thus we sometimes confuse discrete categories
with sets.
...
is the sub $2$-category of the $2$-category of categories fibred in groupoids
over $\mathcal{C}$ (see
...
set $F(U)$ and we have already seen in
Example \ref{example-functor-groupoids}
...
sets whose associated presheaf is representable. However, this would not
...
(see Lemma \ref{lemma-diagonal-1} and
Remark \ref{remark-2-product-fibred-categories-same})
as desired.
...
\mathcal{C}/V \times_{(\mathcal{S} \times \mathcal{S})} \mathcal{S}
\end{eqnarray*}
(for the last isomorphism see Lemma \ref{lemma-diagonal-2} and
Remark \ref{remark-2-product-fibred-categories-same})
...
$(\mathcal{C}, \otimes, \phi)$ as above is coherent. A reformulation
...
commute (the pentagon diagram above is an example). For $n = 5$
...
\begin{proof}
Given a unit $(\mathbf{1}, l, r)$ we get an isomorphism
...
\begin{proof}
Part (1) was shown in the proof of Lemma \ref{lemma-monoidal-unit}.
...
up to unique isomorphism (Lemma \ref{lemma-monoidal-unit-unique})
...
\begin{proof}
We have seen (1) in the proof of Lemma \ref{lemma-monoidal-unit}.
We have seen in the proof of Lemma \ref{lemma-monoidal-unit}
...
the functor $M \mapsto M \otimes_A B$ is a functor of monoidal
...
$$
functorial in $Z$ and the counit of the adjunction produces maps
$$
...
Lemma \ref{lemma-monoidal-unit-unique} implies that
...
and the results of Lemma \ref{lemma-monoidal-coherence}
...
$$
We do this by swapping the last two tensor factors using the
