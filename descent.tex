...
\section{Finite locally free descent and differential operators}
\label{section-finite-locally-free-operator-descent}

\noindent
For a finite locally free covering, module descent can be encoded by one
endomorphism algebra. In characteristic $p$ this gives a concrete form of
Cartier descent.

\begin{lemma}
\label{lemma-finite-locally-free-endomorphism-descent}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 3, pp. 319--322]{FGA}.
\end{reference}
Let $A \to B$ be a faithfully flat finite locally free ring map and set
$$
E = \operatorname{End}_A(B).
$$
View $B$ as a subalgebra of $E$ by left multiplication. Then
\begin{enumerate}
\item the functor
$$
\text{Mod}_A \longrightarrow \text{left }E\text{-modules},
\qquad N \longmapsto B \otimes_A N
$$
where $E$ acts on the first factor is an equivalence, and
\item for a $B$-module $M$, descent data on $M$ relative to $A \to B$
are naturally equivalent to left $E$-module structures on $M$ whose
restriction to $B$ is the given $B$-module structure.
\end{enumerate}
\end{lemma}

\begin{proof}
Write $B^\vee = \Hom_A(B, A)$ and make it a right $E$-module by
precomposition. An inverse in (1) is
$$
M \longmapsto B^\vee \otimes_E M.
$$
The maps which exhibit the two composites as the identity are induced by
$$
B^\vee \otimes_E B \longrightarrow A,
\quad \lambda \otimes b \longmapsto \lambda(b)
$$
and
$$
B \otimes_A B^\vee \otimes_E M \longrightarrow M,
\quad b \otimes \lambda \otimes m \longmapsto (b\lambda)m,
$$
where $b\lambda \in E$ is the endomorphism $c \mapsto b\lambda(c)$.
These maps are isomorphisms. Indeed, this can be checked after localization
at every prime of $A$, where $B$ is a nonzero finite free module and the
assertion is the usual calculation with matrix units. This proves (1).

By Proposition \ref{proposition-descent-module}, the functor from $A$-modules
to descent data relative to $A \to B$ is an equivalence. This functor and the
functor in (1) have the same underlying $B$-module $B \otimes_A N$.
Consequently they identify the extra structures on a fixed $B$-module,
which proves (2).
\end{proof}

\begin{lemma}
\label{lemma-height-one-differential-operators}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 3, p. 321]{FGA}.
\end{reference}
Let $A$ be an $\mathbf{F}_p$-algebra and let
$$
B = A[x_1, \ldots, x_n]/(x_1^p-a_1, \ldots, x_n^p-a_n)
$$
for some $a_i \in A$. Let $\partial_i : B \to B$ be the $A$-derivation
determined by $\partial_i(x_j) = \delta_{ij}$. The $A$-algebra
$E = \operatorname{End}_A(B)$ is generated by the subalgebra $B$ of left multiplication
operators and by $\partial_1, \ldots, \partial_n$, with defining relations
$$
\partial_i b - b\partial_i = \partial_i(b),
\qquad
\partial_i\partial_j = \partial_j\partial_i,
\qquad
\partial_i^p = 0.
$$
In particular, $E$ is free as a left $B$-module with basis
$$
\partial^F = \partial_1^{f_1} \cdots \partial_n^{f_n},
\qquad 0 \leq f_i < p.
$$
\end{lemma}

\begin{proof}
The displayed relations hold in $E$ and show that the proposed generators
and relations are spanned as a left $B$-module by the $\partial^F$.
The monomials
$$
x^E = x_1^{e_1} \cdots x_n^{e_n},
\qquad 0 \leq e_i < p,
$$
form an $A$-basis of $B$. If $F \leq E$ componentwise, then
$$
\partial^F(x^E) =
\prod_i \frac{e_i!}{(e_i-f_i)!} x^{E-F},
$$
and this is zero if $F \not\leq E$. In particular,
$\partial^E(x^E) = \prod_i e_i!$ is a unit of $A$.

Let $T \in \operatorname{End}_A(B)$. Recursively on $|E| = \sum e_i$,
choose $b_E \in B$
so that
$$
T(x^E) = \sum_{F \leq E} b_F\partial^F(x^E).
$$
At the step indexed by $E$, all terms except $b_E\partial^E(x^E)$
are already known and the coefficient of $b_E$ is a unit. Thus every
$A$-linear endomorphism is a unique sum $\sum b_F\partial^F$.
The uniqueness also shows that there can be no further defining relation.
\end{proof}

\begin{proposition}
\label{proposition-cartier-descent-modules}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 3, pp. 321--322]{FGA}.
\end{reference}
With $A$, $B$, and $\partial_i$ as in
Lemma \ref{lemma-height-one-differential-operators}, let $M$ be a $B$-module.
The following kinds of data are naturally equivalent:
\begin{enumerate}
\item a descent datum on $M$ relative to $A \to B$, and
\item additive operators $\nabla_i : M \to M$, $i = 1, \ldots, n$, such that
$$
\nabla_i(bm) = \partial_i(b)m + b\nabla_i(m),
\qquad
[\nabla_i, \nabla_j] = 0,
\qquad
\nabla_i^p = 0.
$$
\end{enumerate}
Every datum in (2) is effective. More precisely, if
$$
N = \bigcap_{i = 1}^n \Ker(\nabla_i),
$$
then the canonical map $B \otimes_A N \to M$ is an isomorphism.
\end{proposition}

\begin{proof}
The ring map $A \to B$ is finite free and faithfully flat. By
Lemma \ref{lemma-finite-locally-free-endomorphism-descent}, descent data on
$M$ are the same as $E = \operatorname{End}_A(B)$-module structures
extending its given
$B$-module structure. By
Lemma \ref{lemma-height-one-differential-operators}, such a structure is
exactly a collection of operators satisfying the conditions in (2).

Effectivity follows from Proposition \ref{proposition-descent-module}.
Under an isomorphism $M = B \otimes_A N_0$ supplied by that proposition,
the operators $\nabla_i$ become $\partial_i \otimes 1$. Expanding in the
$A$-basis $x^E$ shows that their common kernel is $1 \otimes N_0$.
Thus $N = N_0$ and the final assertion follows.
\end{proof}

\begin{lemma}
\label{lemma-cartier-descent-algebras}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 3, pp. 321--322]{FGA}.
\end{reference}
With $A$, $B$, and $\partial_i$ as in
Lemma \ref{lemma-height-one-differential-operators}, let $C$ be a $B$-algebra.
Descent data on $C$ as an algebra relative to $A \to B$ are naturally
equivalent to additive derivations $\nabla_i : C \to C$ which extend
$\partial_i : B \to B$ and satisfy
$$
[\nabla_i, \nabla_j] = 0
\quad\text{and}\quad
\nabla_i^p = 0.
$$
If $C_0 = \bigcap \Ker(\nabla_i)$, then $C_0$ is an $A$-algebra and
$B \otimes_A C_0 \to C$ is an isomorphism of $B$-algebras. Equivalently,
the corresponding descent datum on the affine scheme $\Spec(C)$ is effective
with descended scheme $\Spec(C_0)$.
\end{lemma}

\begin{proof}
Apply Proposition \ref{proposition-cartier-descent-modules} to the underlying
$B$-module of $C$. The multiplication and unit are morphisms of descent data
if and only if each $\nabla_i$ is a derivation extending $\partial_i$.
The common kernel is then an $A$-subalgebra, and the module isomorphism in the
proposition respects multiplication and the unit.
\end{proof}

\noindent
Here is the coordinate-free interpretation of the proposition. The
$B$-module $\mathfrak g = \operatorname{Der}_A(B, B)$ is free on the
$\partial_i$ and is a
restricted Lie algebra with $D^{[p]} = D^p$. The operators in (2) determine a
$B$-linear assignment $D \mapsto \nabla_D$ satisfying
$$
\nabla_D(bm) = D(b)m + b\nabla_D(m),
\qquad
[\nabla_D, \nabla_{D'}] = \nabla_{[D,D']},
...
\nabla_D^p = \nabla_{D^{[p]}}.
$$
Thus Proposition \ref{proposition-cartier-descent-modules} identifies descent
data with flat connections having zero $p$-curvature, and
Lemma \ref{lemma-cartier-descent-algebras} gives the algebra and
affine-scheme versions. The construction commutes with localization in $A$,
and hence gives the analogous statement for quasi-coherent modules on finite
locally free morphisms which locally have the displayed height-one
presentation.
...
\begin{lemma}
\label{lemma-rigidified-invertible-fpqc-descent}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 4, pp. 322--323]{FGA}.
\end{reference}
Let $f : X \to S$ be a morphism with a section $s : S \to X$. Assume that
for every $T \to S$ the canonical map
$$
\mathcal{O}_T \longrightarrow (f_T)_*\mathcal{O}_{X_T}
$$
is an isomorphism. Let $S' \to S$ be an fpqc covering and set
$X' = X \times_S S'$. An invertible sheaf $\mathcal{L}'$ on $X'$ together
with a trivialization of $s'^*\mathcal{L}'$ is the pullback of a unique such
rigidified invertible sheaf on $X$ if and only if its two pullbacks to
$X' \times_X X'$ are isomorphic as rigidified invertible sheaves.

Without choosing a rigidification, an invertible sheaf $\mathcal{L}'$ is
Zariski locally on $S'$ isomorphic to the pullback of an invertible sheaf on
$X$ if and only if its two pullbacks to $X' \times_X X'$ are Zariski locally
on $S' \times_S S'$ isomorphic.
\end{lemma}

\begin{proof}
For every $T \to S$ and every invertible sheaf $\mathcal{L}$ on $X_T$, we
have
$$
\operatorname{Aut}(\mathcal{L}) =
\Gamma(X_T, \mathcal{O}_{X_T}^*) =
\Gamma(T, \mathcal{O}_T^*).
$$
It follows that an automorphism preserving a rigidification along $s_T$ is
the identity. Thus an isomorphism between two rigidified invertible sheaves,
if it exists, is unique. On the triple fibre product the two composites of
the isomorphism in the statement are both rigidified isomorphisms, so they
agree. In other words, the cocycle condition is automatic. Effectivity now
follows from Proposition \ref{proposition-fpqc-descent-quasi-coherent}, and
the descended module is invertible by
Lemma \ref{lemma-finite-locally-free-descends}. The rigidification descends by
full faithfulness. This proves the first assertion.

For the second assertion, replace $\mathcal{L}'$ by
$$
\mathcal{L}'_0 =
\mathcal{L}' \otimes (f')^*(s'^*\mathcal{L}')^{-1},
$$
which has a canonical rigidification. Local isomorphisms between the two
pullbacks can be uniquely normalized to preserve the induced
rigidifications; uniqueness makes them glue. The first assertion descends
$\mathcal{L}'_0$. Since $s'^*\mathcal{L}'$ is Zariski locally trivial on
$S'$, this is exactly the claimed local pullback condition. The converse is
immediate.
\end{proof}

\begin{proposition}
\label{proposition-atiyah-cartier-invertible}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 4, p. 323]{FGA}.
\end{reference}
Let $A$, $B$, and $\partial_i$ be as in
Lemma \ref{lemma-height-one-differential-operators}. Let
$f : X \to \Spec(A)$ have a section $s$. For $r \geq 1$, set
$$
S_r = \Spec(B^{\otimes_A r}),
\qquad X_r = X \times_A S_r,
$$
write $f_r : X_r \to S_r$ for the projection, and assume that
$\mathcal{O}_{S_r} \to (f_r)_*\mathcal{O}_{X_r}$ is an
isomorphism for every $r$. Set $X_B = X_1$, let $q : X_B \to X$ be the
projection, and let $\mathcal{L}$ be an invertible sheaf on $X_B$.
Normalize it along the induced section by setting
$$
\mathcal{L}_0 =
\mathcal{L} \otimes f_B^*(s_B^*\mathcal{L})^{-1}.
$$
Define the {it Atiyah--Cartier class} $a(\mathcal{L})$ to be the extension
class of the sequence of principal parts
$$
0 \longrightarrow
\Omega^1_{X_B/X} \otimes \mathcal{L}_0 \longrightarrow
\mathcal{P}^1_{X_B/X}(\mathcal{L}_0) \longrightarrow
\mathcal{L}_0 \longrightarrow 0.
$$
Thus
$$
a(\mathcal{L}) \in
\Ext^1_{\mathcal{O}_{X_B}}
(\mathcal{L}_0, \Omega^1_{X_B/X} \otimes \mathcal{L}_0)
= H^1(X_B, \Omega^1_{X_B/X}).
$$
This is the ordinary first Atiyah class for $q$.
Then the following are equivalent:
\begin{enumerate}
\item $a(\mathcal{L}) = 0$, and
\item there is an invertible sheaf $\mathcal{M}$ on $X$ such that
$\mathcal{L}$ is Zariski locally on $\Spec(B)$ isomorphic to $q^*\mathcal{M}$.
\end{enumerate}
\end{proposition}

\begin{proof}
The sequence in the statement is the sequence of
Modules, Lemma \ref{modules-lemma-sequence-of-principal-parts}. Its splittings
are the same as connections on $\mathcal{L}_0$ relative to $X_B/X$. Since
$$
\Omega^1_{X_B/X} =
\mathcal{O}_{X_B} \otimes_B \Omega^1_{B/A}
$$
and $\Omega^1_{B/A}$ is free on $\mathrm{d}x_1, \ldots, \mathrm{d}x_n$,
such a connection is given by additive operators
$\nabla_i : \mathcal{L}_0 \to \mathcal{L}_0$ satisfying
$$
\nabla_i(gm) = \partial_i(g)m + g\nabla_i(m).
$$

Assume (1). Choose a connection. Its restriction along $s_B$ is a connection
on $s_B^*\mathcal{L}_0 = \mathcal{O}_{\Spec(B)}$. Subtracting multiplication
by suitable elements of $B$ from the $\nabla_i$, we may and do arrange that
this restricted connection is the canonical one given by the $\partial_i$.
The resulting normalized connection is unique, because the difference of
two normalized connections is given by global functions on $X_B$ and these
are exactly the elements of $B$.

The commutators $[\nabla_i, \nabla_j]$ and the operators $\nabla_i^p$ are
$\mathcal{O}_{X_B}$-linear. Hence they are multiplication by global functions
on $X_B$. Their restrictions along $s_B$ vanish because the $\partial_i$
commute and satisfy $\partial_i^p = 0$. The hypothesis on global functions
therefore gives
$$
[\nabla_i, \nabla_j] = 0
\quad\text{and}\quad
\nabla_i^p = 0.
$$
On every affine open of $X$, Proposition
\ref{proposition-cartier-descent-modules} identifies the horizontal sections
with a module whose pullback is $\mathcal{L}_0$. These modules commute with
localization and hence form the quasi-coherent sheaf
$$
\mathcal{M} = \bigcap_i \Ker(q_*\nabla_i)
$$
on $X$. The canonical map $q^*\mathcal{M} \to \mathcal{L}_0$ is an
isomorphism, and $\mathcal{M}$ is invertible by
Lemma \ref{lemma-finite-locally-free-descends}.
The original $\mathcal{L}$ differs from $q^*\mathcal{M}$ by the pullback of
the invertible sheaf $s_B^*\mathcal{L}$ on $\Spec(B)$, which is Zariski
locally trivial. Thus (2) holds.

Conversely, assume (2). After normalization, the local pullback
descriptions give connections which restrict to the canonical connection
along $s_B$. They agree on overlaps by the uniqueness just proved and hence
give a global splitting of the sequence of principal parts. Thus
$a(\mathcal{L}) = 0$.
\end{proof}
...
\begin{lemma}
\label{lemma-equalizer-flat-module-descent}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 2(c), pp. 306--307]{FGA}.
\end{reference}
Assume that
$$
R \longrightarrow S
...
S \otimes_R S
$$
is an equalizer diagram, where the two arrows are
$s \mapsto s \otimes 1$ and $s \mapsto 1 \otimes s$. If $M$ and $N$ are
$R$-modules and $N$ is flat over $R$, then the canonical map
$$
\Hom_R(M, N) \longrightarrow \Hom_{DD_{S/R}}(f^*M, f^*N)
$$
is bijective. In particular, base extension is fully faithful on the full
subcategory of flat $R$-modules.
\end{lemma}

\begin{proof}
Tensoring the equalizer diagram in the statement with the flat $R$-module
$N$ gives an equalizer diagram
$$
N \longrightarrow N \otimes_R S
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
N \otimes_R S \otimes_R S.
$$
This immediately gives injectivity of the map in the statement. Let
$$
u : M \otimes_R S \longrightarrow N \otimes_R S
$$
be a morphism between the canonical descent data. For $m \in M$, compatibility
with the descent data says that $u(m \otimes 1)$ has the same image under the
two arrows in the displayed equalizer. Thus there is a unique $v(m) \in N$
whose image in $N \otimes_R S$ is $u(m \otimes 1)$. Uniqueness shows that
$m \mapsto v(m)$ is $R$-linear. Since $u$ is $S$-linear, we have
$u = v \otimes_R S$, proving surjectivity.
\end{proof}

\begin{lemma}
\label{lemma-artinian-small-effective-descent-flat-modules}
\begin{reference}
\cite[Expos\'e 195, B, Th\'eor\`eme 2, p. 378]{FGA}.
\end{reference}
Let $(R, \mathfrak m)$ be an Artinian local ring and let
$R \to S$ be an injective ring map. Assume that
\begin{enumerate}
\item $\mathfrak mS \subset R$, and
\item the diagram
$$
R \longrightarrow S
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
S \otimes_R S
$$
is an equalizer, where the two arrows are the canonical ones.
\end{enumerate}
Then base extension induces an equivalence between
\begin{enumerate}
\item flat $R$-modules, and
\item descent data relative to $R \to S$ whose underlying $S$-module is flat.
\end{enumerate}
\end{lemma}

\begin{proof}
We first record the shape of the ring map. Since $\mathfrak m$ is nilpotent,
$\mathfrak mS$ is a nilpotent ideal of $S$. By (1) it is contained in $R$,
and hence in the nilradical $\mathfrak m$ of $R$. The reverse inclusion is
automatic, so
$$
\mathfrak mS = \mathfrak m.
$$
Set $k = R/\mathfrak m$ and $C = S/\mathfrak m$. The map $k \to C$ is
injective and hence faithfully flat. Moreover, the diagram
$$
\xymatrix{
C & S \ar[l] \\
k \ar[u] & R \ar[l] \ar[u]
...
$$
is cartesian. Indeed, if $s \in S$ and $a \in R$ have the same image in
$C$, then $s - a \in \mathfrak mS = \mathfrak m$, and hence $s \in R$.

Let $(P, \theta)$ be a descent datum relative to $R \to S$ with $P$ flat
over $S$. Reduction modulo $\mathfrak m$ gives a descent datum on
$P/\mathfrak mP$ relative to the faithfully flat map $k \to C$. By
Proposition \ref{proposition-descent-module}, there are a $k$-vector space
$V$ and an isomorphism
$$
\beta : V \otimes_k C \longrightarrow P/\mathfrak mP
$$
compatible with these descent data. Apply More on Algebra, Lemma
\ref{more-algebra-lemma-flat-module-over-fibre-product} to the cartesian
diagram above and the triple $(V, P, \beta)$. We obtain a flat $R$-module
$M$ and an isomorphism
$$
\alpha : M \otimes_R S \longrightarrow P
$$
whose reduction modulo $\mathfrak m$ is $\beta$.

It remains to check that $\alpha$ respects the descent data. Transport the
canonical descent datum on $M \otimes_R S$ across $\alpha$ and denote it by
$\theta_0$. The reductions of $\theta$ and $\theta_0$ modulo $\mathfrak m$
agree by the choice of $\beta$. The module $M$ is free by Algebra, Lemma
\ref{algebra-lemma-local-artinian-characterize-flat}; fix a basis. Put
$D = S \otimes_R S$. For $a \in \mathfrak m$ and $s,t \in S$, we have
$$
a(s \otimes t) = as \otimes t = 1 \otimes ast \in \mathfrak m \subset D.
$$
The multiplication map $D \to S$ shows that the resulting copy of
$\mathfrak m$ in $D$ is embedded. Thus $\mathfrak mD = \mathfrak m$.
The same argument works in every higher tensor power of $S$ over $R$.

After identifying both pullbacks using the chosen basis, let $g$ be the
automorphism obtained by composing $\theta$ with $\theta_0^{-1}$. It is the
identity modulo $\mathfrak mD$. Hence all three pullbacks of $g$ to
$S \otimes_R S \otimes_R S$ are the same automorphism: after subtracting the
identity, its coefficients belong to the copy of $\mathfrak m$ coming from
$R$. The cocycle condition therefore reads $g = g^2$. Since $g$ is
invertible, $g = 1$. Thus $\alpha$ is compatible with the descent data, proving
essential surjectivity. Full faithfulness follows from
Lemma \ref{lemma-equalizer-flat-module-descent} and assumption (2).
\end{proof}

\begin{remark}
\label{remark-finite-artinian-flat-descent-counterexample}
The condition $\mathfrak mS \subset R$ in
Lemma \ref{lemma-artinian-small-effective-descent-flat-modules} cannot simply
be discarded. Venken constructed a finite injective map of local Artinian
rings which is a descent morphism for flat modules but which admits a
non-effective descent datum on a free module of rank one; see \cite{Venken}.
Thus an equalizer statement gives full faithfulness, but not effectivity by
itself.
\end{remark}
...
\begin{lemma}
\label{lemma-effective-descent-flat-modules}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 2(c), p. 307]{FGA},
\cite{mesablishvili2}.
\end{reference}
If $f : R \to S$ is universally injective, then base extension induces an
equivalence between
\begin{enumerate}
\item flat $R$-modules, and
\item descent data relative to $f$ whose underlying $S$-module is flat.
\end{enumerate}
\end{lemma}

\begin{proof}
Theorem \ref{theorem-descent} says that base extension is an equivalence on
all modules. It carries flat modules to flat modules by Algebra, Lemma
\ref{algebra-lemma-flat-base-change}. Conversely, if $(P, \theta)$ is a
descent datum with $P$ flat over $S$, write
$(P, \theta) \cong f^*M$ using Theorem \ref{theorem-descent}. Then
$M \otimes_R S$ is flat over $S$, and Theorem
\ref{theorem-descend-module-properties} shows that $M$ is flat over $R$.
\end{proof}
...
\section{Finite epimorphisms}
\label{section-finite-epimorphisms}

\noindent
In this section an effective epimorphism is meant in the category of schemes,
in the sense of Sites, Definition
\ref{sites-definition-universal-effective-epimorphisms}, for a family
consisting of one morphism. We first translate this categorical condition
into a condition on functions for finite morphisms.

\begin{lemma}
\label{lemma-finite-epimorphism-injective-functions}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 2(b), p. 306]{FGA}.
\end{reference}
Let $f : X \to S$ be a finite morphism of schemes and set
$\mathcal{A} = f_*\mathcal{O}_X$. Then $f$ is an epimorphism in the category
of schemes if and only if
$$
\mathcal{O}_S \longrightarrow \mathcal{A}
$$
is injective.
\end{lemma}

\begin{proof}
Assume that $f$ is an epimorphism. If
$\mathcal{I} = \Ker(\mathcal{O}_S \to \mathcal{A})$ is nonzero, then $f$
factors through the proper closed subscheme $Z \subset S$ cut out by
$\mathcal{I}$. The pushout $S \amalg_Z S$ exists by More on Morphisms,
Lemma \ref{more-morphisms-lemma-pushout-along-closed-immersions}. The two
maps $S \to S \amalg_Z S$ are distinct and become equal after composition
with $f$, a contradiction. To see directly that the two maps are distinct,
work on an affine open on which $\mathcal{I}$ is nonzero and use the two
projections
$$
A \times_{A/I} A \longrightarrow A.
$$

\medskip\noindent
Conversely, assume that $\mathcal{O}_S \to \mathcal{A}$ is injective. The
morphism $f$ is surjective: this follows affine locally from lying over for
an injective integral ring map, see Algebra, Lemma
\ref{algebra-lemma-integral-overring-surjective}. Let $g, h : S \to Y$ be
morphisms such that $g \circ f = h \circ f$. The inverse image $Z \to S$ of
the diagonal of $Y$ under $(g, h)$ is an immersion by Schemes, Lemma
\ref{schemes-lemma-diagonal-immersion}, and $f$ factors through $Z$. Since
$f$ is surjective, the immersion $Z \to S$ is surjective and hence is a
closed immersion by Schemes, Lemma
\ref{schemes-lemma-immersion-when-closed}. If $\mathcal{J}$ is its ideal
sheaf, then $\mathcal{J}$ maps to zero in $\mathcal{A}$. Injectivity gives
$\mathcal{J} = 0$, and hence $Z = S$ and $g = h$.
\end{proof}

\begin{lemma}
\label{lemma-finite-coequalizer-flat-base-change}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 2(c), pp. 306--307]{FGA}.
\end{reference}
Let
$$
\xymatrix{
S'' \ar@<0.5ex>[r]^a \ar@<-0.5ex>[r]_b &
S' \ar[r]^f & S
}
$$
be a coequalizer diagram in the category of schemes. Assume that $S'$ and
$S''$ are finite over $S$. Then the following statements hold.
\begin{enumerate}
\item The diagram of quasi-coherent $\mathcal{O}_S$-algebras
$$
\mathcal{O}_S \longrightarrow f_*\mathcal{O}_{S'}
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
(f \circ a)_*\mathcal{O}_{S''}
$$
is an equalizer diagram.
\item For every flat morphism $T \to S$, the base change
$$
\xymatrix{
S''_T \ar@<0.5ex>[r] \ar@<-0.5ex>[r] & S'_T \ar[r] & T
}
$$
is a coequalizer diagram in the category of schemes.
\item If $T$ and $Y$ are $S$-schemes and $T$ is flat over $S$, then
$$
\Mor_S(T, Y) \longrightarrow \Mor_{S'}(T_{S'}, Y_{S'})
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
\Mor_{S''}(T_{S''}, Y_{S''})
$$
is an equalizer diagram.
\item If $\mathcal{F}$ and $\mathcal{G}$ are quasi-coherent
$\mathcal{O}_S$-modules and $\mathcal{G}$ is flat, then the analogous
diagram of sets of module maps after pullback to $S'$ and $S''$ is an
equalizer diagram.
\end{enumerate}
\end{lemma}

\begin{proof}
For a diagram of schemes finite over $S$, the coequalizer condition is
equivalent to the equalizer condition on the corresponding finite
quasi-coherent algebras. Here is a reminder of the argument. The assertion
is local on $S$, where all three schemes are affine. For affine targets it
is the anti-equivalence between rings and affine schemes. For an arbitrary
target, finiteness shows that the structure map $S' \to S$ is closed. The
inverse image of an affine open under a map out of $S'$ which equalizes
$a$ and $b$ is saturated and hence descends to an open of $S$. The affine
case constructs the descended map on these opens, and uniqueness glues
them. This is the same finite topological argument used in the proof of
Lemma \ref{lemma-finite-effective-epimorphism-functions}. This proves (1)
and the stated equivalence.

Formation of the finite pushforward algebras commutes with base change.
Tensoring the equalizer in (1) with the flat $\mathcal{O}_S$-algebra
$\mathcal{O}_T$ preserves it. The equivalence just recalled gives (2),
and the universal property of the coequalizer gives (3).

We prove (4) affine locally. Write the equalizer in (1) as
$$
A \longrightarrow B
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}} C.
$$
Let $M$ and $N$ correspond to $\mathcal{F}$ and $\mathcal{G}$. Since $N$
is flat, tensoring gives an equalizer
$$
N \longrightarrow N \otimes_A B
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
N \otimes_A C.
$$
A compatible $B$-linear map $M \otimes_A B \to N \otimes_A B$ sends
$m \otimes 1$ into this equalizer for every $m \in M$. It therefore comes
from a unique $A$-linear map $M \to N$. These maps glue by uniqueness.
\end{proof}

\begin{proposition}
\label{proposition-finite-schemes-geometric-fibre-colimits}
\begin{reference}
\cite[Expos\'e 212, Proposition 2.1, pp. 103--104]{FGA}.
\end{reference}
Let $S$ be a locally Noetherian scheme, let $s \in S$, and let $\Omega$
be an algebraically closed extension of $\kappa(s)$. The category of schemes
finite over $S$ has finite colimits, and the functor
$$
X \longmapsto X_s(\Omega)
$$
from this category to the category of sets commutes with finite colimits.
\end{proposition}

\begin{proof}
Finite coproducts of finite $S$-schemes exist and are finite over $S$.
Consider two morphisms
$X_1 \mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}} X_0$
of finite $S$-schemes.
Write
$$
\mathcal{A}_i = (X_i \to S)_*\mathcal{O}_{X_i}
$$
and let $\mathcal{A}$ be the equalizer of
$\mathcal{A}_0
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
\mathcal{A}_1$. This is a finite
quasi-coherent $\mathcal{O}_S$-algebra: the assertion is affine local, where
it follows because a submodule of a finite module over a Noetherian ring is
finite. The finite $S$-scheme
$$
X = \underline{\Spec}_S(\mathcal{A})
$$
is the coequalizer of
$X_1 \mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}} X_0$
in the category of finite $S$-schemes. Thus finite colimits exist by
Categories, Lemma
\ref{categories-lemma-colimits-coproducts-coequalizers}.

\medskip\noindent
Choose a separable closure $\kappa(s)^{sep} \subset \Omega$ and let
$R^{sh}$ be the corresponding strict henselization of
$R = \mathcal{O}_{S, s}$. The morphism
$$
T = \Spec(R^{sh}) \longrightarrow S
$$
is flat by More on Algebra, Lemma
\ref{more-algebra-lemma-dumb-properties-henselization}. Consequently, the
construction by equalizers above shows that base change from $S$ to $T$
preserves coequalizers of finite schemes.

\medskip\noindent
We claim that the functor
$$
G : \{\text{schemes finite over }T\} \longrightarrow
\{\text{finite sets}\}, \qquad Y \longmapsto Y(\Omega)
$$
is left adjoint to the functor
$$
E \longmapsto E_T = \coprod_{e \in E} T.
$$
To see this, write $Y = \Spec(B)$. Since $R^{sh}$ is henselian, Algebra,
Lemma \ref{algebra-lemma-characterize-henselian} gives a product
decomposition
$$
B = B_1 \times \ldots \times B_n
$$
with each $B_i$ local. The residue field of $B_i$ is a finite purely
inseparable extension of the separably algebraically closed residue field
$\kappa(s)^{sep}$. It therefore has a unique
$\kappa(s)^{sep}$-embedding into $\Omega$. It follows that $Y(\Omega)$ has
exactly one element for each factor $B_i$. On the other hand, a morphism
$Y \to E_T$ is exactly a choice of an element of $E$ for each of these
factors. We have therefore obtained, naturally in $Y$ and $E$, a bijection
$$
\Mor_T(Y, E_T) = \operatorname{Map}(Y(\Omega), E),
$$
which proves the claim.

\medskip\noindent
As a left adjoint, $G$ commutes with finite colimits. Finally, for every
finite $S$-scheme $Y$ we have
$$
Y_s(\Omega) = (Y \times_S T)(\Omega).
$$
Flat base change to $T$ preserves the coequalizers constructed above, and
$G$ preserves them by the claim. Finite coproducts are preserved as well,
which proves the proposition.
\end{proof}

\begin{lemma}
\label{lemma-finite-equivalence-relation-kernel-pair}
\begin{reference}
\cite[Expos\'e 212, p. 104]{FGA}.
\end{reference}
Let $S$ be a locally Noetherian scheme. Let
$$
j : R \longrightarrow X \times_S X
$$
be an equivalence relation with $R$ and $X$ finite over $S$. If $Y = X/R$
is the coequalizer in the category of finite $S$-schemes, then the canonical
morphism
$$
R \longrightarrow X \times_Y X
$$
is a surjective closed immersion. In particular, it is surjective and
radicial.
\end{lemma}

\begin{proof}
The canonical morphism is a monomorphism because $j$ is a monomorphism. It is
finite, as every morphism between schemes finite over $S$ is finite, and hence
it is a closed immersion by Morphisms, Lemma
\ref{morphisms-lemma-finite-monomorphism-closed}.

\medskip\noindent
Let $z \in X \times_Y X$, let $s \in S$ be its image, and choose an
algebraically closed extension $\Omega$ of $\kappa(z)$. The point $z$ gives
two elements of $X_s(\Omega)$ with the same image in $Y_s(\Omega)$. By
Proposition \ref{proposition-finite-schemes-geometric-fibre-colimits}, the
latter set is the quotient of $X_s(\Omega)$ by $R_s(\Omega)$. Since
$R_s(\Omega)$ is already an equivalence relation, there is an element of
$R_s(\Omega)$ mapping to the given pair. Thus $z$ is in the image of the
canonical morphism, which proves surjectivity. A monomorphism is universally
injective, so the final assertion follows as well.
\end{proof}

\begin{lemma}
\label{lemma-finite-effective-epimorphism-functions}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 2(b), pp. 305--306]{FGA}.
\end{reference}
Let $f : X \to S$ be a finite morphism of schemes. Let
$f_2 : X \times_S X \to S$ be the structure morphism and set
$$
\mathcal{A} = f_*\mathcal{O}_X, \qquad
\mathcal{A}_2 = (f_2)_*\mathcal{O}_{X \times_S X}.
$$
The following are equivalent:
\begin{enumerate}
\item $f$ is an effective epimorphism in the category of schemes, and
\item the diagram of quasi-coherent $\mathcal{O}_S$-algebras
$$
\xymatrix{
\mathcal{O}_S \ar[r] &
\mathcal{A} \ar@<0.5ex>[r] \ar@<-0.5ex>[r] &
\mathcal{A}_2
}
$$
is an equalizer diagram.
\end{enumerate}
These conditions are preserved by flat base change on $S$.
\end{lemma}

\begin{proof}
Assume (1) and let $\mathcal{B}$ be the equalizer of the two maps from
$\mathcal{A}$ to $\mathcal{A}_2$. By
Lemma \ref{lemma-finite-epimorphism-injective-functions}, the map
$\mathcal{O}_S \to \mathcal{A}$ is injective, and hence we have inclusions
$$
\mathcal{O}_S \longrightarrow \mathcal{B} \longrightarrow \mathcal{A}.
$$
They correspond to a factorization
$$
X \xrightarrow{g} T = \underline{\Spec}_S(\mathcal{B})
\xrightarrow{h} S
$$
of $f$. By construction $g \circ \text{pr}_1 = g \circ \text{pr}_2$ on
$X \times_S X$. Effectivity gives a morphism $e : S \to T$ such that
$g = e \circ f$. Thus
$$
g = e \circ h \circ g, \qquad f = h \circ e \circ f.
$$
The maps $\mathcal{B} \to \mathcal{A}$ and
$\mathcal{O}_S \to \mathcal{A}$ are injective. On applying the functor on
functions to the displayed identities, we find that $h^\sharp$ and
$e^\sharp$ are inverse. Hence $T \to S$ is an isomorphism and (2) holds.

\medskip\noindent
Assume (2). By Lemma
\ref{lemma-finite-epimorphism-injective-functions}, $f$ is an epimorphism.
It remains to prove existence in the coequalizer property. Suppose that
$a : X \to Y$ satisfies
$$
a \circ \text{pr}_1 = a \circ \text{pr}_2
$$
on $X \times_S X$. The finite surjective morphism $f$ is closed, and hence
is a quotient map on underlying topological spaces. If $V \subset Y$ is
affine open, then $a^{-1}(V)$ is saturated for $f$. Indeed, two points of
$X$ in the same fibre lift to a point of $X \times_S X$ after a residue
field extension; see Schemes, Lemma
\ref{schemes-lemma-points-fibre-product}. Consequently, there is an open
$U \subset S$ such that
$$
f^{-1}(U) = a^{-1}(V).
$$
Explicitly, one can take
$U = S \setminus f(X \setminus a^{-1}(V))$.

The ring map defining $a|_{f^{-1}(U)} : f^{-1}(U) \to V$ has image in the
equalizer of the two maps from
$\Gamma(f^{-1}(U), \mathcal{O}_X)$ to
$\Gamma(f^{-1}(U) \times_U f^{-1}(U), \mathcal{O})$.
By (2), this equalizer is $\Gamma(U, \mathcal{O}_U)$. Schemes, Lemma
\ref{schemes-lemma-morphism-into-affine} therefore gives a morphism
$U \to V$ whose pullback to $f^{-1}(U)$ is $a$. As $V$ runs through an
affine open covering of $Y$, the corresponding opens $U$ cover $S$. The
resulting morphisms agree on overlaps because $f$ is an epimorphism, and
they glue to the desired morphism $S \to Y$.

Finally, formation of $\mathcal{A}$ and $\mathcal{A}_2$ commutes with base
change because the morphisms involved are affine. Kernels of maps of
quasi-coherent modules commute with flat base change. Thus (2), and hence
(1), is preserved by flat base change.
\end{proof}

\begin{lemma}
\label{lemma-finite-effective-epimorphism-flat-schemes}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 2(c), pp. 306--307]{FGA}.
\end{reference}
Let $f : X \to S$ be a finite effective epimorphism. Let $T$ and $Y$ be
$S$-schemes and assume that $T$ is flat over $S$. Set
$X_2 = X \times_S X$. Then
$$
\Mor_S(T, Y) \longrightarrow
\Mor_X(T \times_S X, Y \times_S X)
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
\Mor_{X_2}(T \times_S X_2, Y \times_S X_2)
$$
is an equalizer diagram, where the two arrows are pullback by the projections
$X_2 \to X$.
\end{lemma}
...
The base change
$$
T \times_S X \longrightarrow T
$$
is an effective epimorphism by Lemma
\ref{lemma-finite-effective-epimorphism-functions}, because $T \to S$ is
flat. The equalizer condition on a morphism
$T \times_S X \to Y \times_S X$ over $X$ says that its underlying
$S$-morphism to $Y$ agrees after the two pullbacks to
$$
(T \times_S X) \times_T (T \times_S X) = T \times_S X_2.
$$
The effective epimorphism therefore gives a unique morphism $T \to Y$.
Its pullback to $X$ is the original morphism, which proves the assertion.
\end{proof}

\begin{lemma}
\label{lemma-finite-effective-epimorphism-flat-quasi-coherent}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 2(c), pp. 306--307]{FGA}.
\end{reference}
Let $f : X \to S$ be a finite effective epimorphism. Pullback from $S$ to
$X$ is fully faithful from the category of quasi-coherent
$\mathcal{O}_S$-modules which are flat over $S$ to the category of descent
data relative to $f$.
\end{lemma}

\begin{proof}
The assertion is local on $S$. If $S = \Spec(R)$, then
$X = \Spec(A)$ because $f$ is finite. Lemma
\ref{lemma-finite-effective-epimorphism-functions} says that
$$
R \longrightarrow A
\mathrel{\substack{\longrightarrow \\[-0.6ex] \longrightarrow}}
A \otimes_R A
$$
is an equalizer. Full faithfulness now follows from Lemma
\ref{lemma-equalizer-flat-module-descent}. The descended morphisms on affine
opens agree on overlaps by uniqueness and hence glue.
\end{proof}

\begin{example}
\label{example-finite-effective-epimorphism-not-pure}
The equalizer condition for a finite effective epimorphism does not imply
universal injectivity. Let $k$ be a field and set
$$
A = k[x, y]/(xy), \qquad B = k[x] \times k[y].
$$
Map $A$ to $B$ by restriction to the two coordinate axes. We have
$$
B \otimes_A B \cong k[x] \times k \times k \times k[y].
$$
The equalizer of the two maps $B \to B \otimes_A B$ consists of the pairs
$(p(x), q(y))$ such that $p(0) = q(0)$, and hence is $A$. Thus
$\Spec(B) \to \Spec(A)$ is a finite effective epimorphism by
Lemma \ref{lemma-finite-effective-epimorphism-functions}.

On the other hand, it is not universally injective. For the ideal
$I = (x + y) \subset A$, we have
$$
x \in IB \cap A \quad\text{but}\quad x \notin I.
$$
The last assertion can be checked after identifying
$A/I = k[x]/(x^2)$. Thus tensoring $A \to B$ with $A/I$ kills the
nonzero class of $x$. By Algebra, Definition
\ref{algebra-definition-universally-injective}, the map $A \to B$ is not
universally injective. By Theorem
\ref{theorem-descent}, it is not an effective descent morphism for all
modules.

This does not by itself answer the more restricted historical question for
descent data whose underlying module is flat. Under the additional
hypothesis of universal injectivity, Lemma
\ref{lemma-effective-descent-flat-modules} gives an affirmative answer.
\end{example}

\begin{theorem}
\label{theorem-finite-epimorphism-canonical-factorization}
\begin{reference}
\cite[Expos\'e 190, n\textsuperscript{o} 2(b), p. 306]{FGA},
\cite[Proposition 2.3.2]{Romagny-Rydh-Zalamansky}.
\end{reference}
Let $S$ be a Noetherian scheme and let $f : X \to S$ be a finite
epimorphism. Put
$$
\mathcal{A} = \mathcal{O}_S, \qquad
\mathcal{A}_0 = f_*\mathcal{O}_X,
$$
and define recursively
$$
\mathcal{A}_{i + 1} =
\Ker\left(
\mathcal{A}_i \longrightarrow
\mathcal{A}_i \otimes_{\mathcal{A}} \mathcal{A}_i,
\quad b \longmapsto b \otimes 1 - 1 \otimes b
\right),
\qquad
...
\begin{enumerate}
\item the inclusions
$\mathcal{A} \subset \mathcal{A}_{i + 1} \subset \mathcal{A}_i$
define finite effective epimorphisms $X_i \to X_{i + 1}$;
\item there is an $n \geq 0$ such that $\mathcal{A}_n = \mathcal{A}$, and
hence
$$
X = X_0 \longrightarrow X_1 \longrightarrow \ldots
\longrightarrow X_n = S
$$
is a factorization of $f$ into finite effective epimorphisms;
\item the construction commutes with flat base change on $S$; and
\item this factorization has minimal length among factorizations of $f$
into finite effective epimorphisms.
\end{enumerate}
We call it the canonical factorization of $f$.
\end{theorem}

\begin{proof}
Lemma \ref{lemma-finite-epimorphism-injective-functions} gives
$\mathcal{A} \subset \mathcal{A}_0$, and it is clear inductively that
$\mathcal{A} \subset \mathcal{A}_{i + 1} \subset \mathcal{A}_i$.
All these algebras are finite as $\mathcal{A}$-modules: a submodule of a
finite module on a Noetherian scheme is finite. The canonical surjection
$$
\mathcal{A}_i \otimes_{\mathcal{A}} \mathcal{A}_i
\longrightarrow
\mathcal{A}_i \otimes_{\mathcal{A}_{i + 1}} \mathcal{A}_i
$$
has kernel generated locally by
$b \otimes 1 - 1 \otimes b$ for $b \in \mathcal{A}_{i + 1}$. These
elements are already zero in the source by the definition of
$\mathcal{A}_{i + 1}$. Thus the displayed map is an isomorphism, and
$$
\xymatrix{
\mathcal{A}_{i + 1} \ar[r] &
\mathcal{A}_i \ar@<0.5ex>[r] \ar@<-0.5ex>[r] &
\mathcal{A}_i \otimes_{\mathcal{A}_{i + 1}} \mathcal{A}_i
}
$$
is an equalizer diagram. Lemma
\ref{lemma-finite-effective-epimorphism-functions} proves (1).

The same description shows (3): tensor products commute with arbitrary base
change and kernels commute with flat base change.

\medskip\noindent
We prove (2). First observe that if
$\mathcal{A}_{i + 1} = \mathcal{A}_i$, then the isomorphism above says that
the multiplication map
$$
\mathcal{A}_i \otimes_{\mathcal{A}} \mathcal{A}_i
\longrightarrow \mathcal{A}_i
$$
is an isomorphism. Affine locally, $A \to A_i$ is therefore a finite
epimorphism of rings. It is surjective by Algebra, Lemma
\ref{algebra-lemma-finite-epimorphism-surjective}, and it is injective by
construction. Hence $\mathcal{A}_i = \mathcal{A}$.

It remains to show that the descending sequence $\mathcal{A}_i$ is locally
stationary. By localization and quasi-compactness, we may work over the
spectrum of a Noetherian local ring $A$ and write $A_i$ for the corresponding
finite $A$-algebras. We argue by induction on $d = \dim(A)$. If $d = 0$,
then $A_0/A$ has finite length, so its descending sequence of submodules
$A_i/A$ is stationary.

Suppose $d > 0$ and let $\mathfrak m$ be the maximal ideal. For every
$\mathfrak p \neq \mathfrak m$, the localized sequence is stationary by
induction because $\dim(A_{\mathfrak p}) < d$. The punctured spectrum is
quasi-compact, and every $A_i/A$ is finite. For each such $\mathfrak p$,
choose an index where the localized sequence is stationary; its stationary
value is $A_{\mathfrak p}$ by the first paragraph. This equality holds on a
neighbourhood of $\mathfrak p$. A finite subcover and the largest of the
corresponding indices therefore give an $N$ such that
$(A_N/A)_{\mathfrak p} = 0$ for all
$\mathfrak p \neq \mathfrak m$. Thus $A_i/A$ has finite length for all
$i \geq N$. The descending sequence is stationary, and the observation of
the preceding paragraph shows that its stationary value is $A$. Applying
this at every point of $S$ and using that $S$ is quasi-compact gives one
integer $n$ with $\mathcal{A}_n = \mathcal{A}$.

Finally, consider any factorization
$$
X = X'_0 \longrightarrow X'_1 \longrightarrow \ldots
\longrightarrow X'_m = S
$$
into finite effective epimorphisms, and write
$\mathcal{B}_i$ for the finite $\mathcal{A}$-algebra corresponding to
$X'_i$. We show by induction that $\mathcal{A}_i \subset \mathcal{B}_i$.
This is equality for $i = 0$. If it holds for $i$, then the equalizer
criterion of Lemma \ref{lemma-finite-effective-epimorphism-functions}
shows that every section of $\mathcal{A}_{i + 1}$ maps into
$\mathcal{B}_{i + 1}$. Hence the inclusion holds for $i + 1$. Since
$\mathcal{B}_m = \mathcal{A}$, we obtain
$\mathcal{A} \subset \mathcal{A}_m \subset \mathcal{B}_m = \mathcal{A}$.
Thus the canonical sequence terminates in at most $m$ steps, proving (4).
\end{proof}
...
Let $\mathcal{P}$ be a property of morphisms of schemes which is
\'etale local on source-and-target. Let $\mathcal{Q}$ be the associated property
...
The first morphism has a retraction, hence induces an equivalence of
...
via the morphism $g_i$. We claim that $\{g : X \to Y\}$
...
datum as in Schemes, Section \ref{schemes-section-glueing-schemes}
...
descent data for schemes relative to an fpqc-covering
...
descend are particularly simple, then it is sometimes the
...
morphism of families $\mathcal{V} \to \mathcal{U}$, if we pull back
...
restricting to smaller affine opens in $S$. This implies that
...
For each $i$ denote by $F_i$ the sheaf $h_{X_i}$.
