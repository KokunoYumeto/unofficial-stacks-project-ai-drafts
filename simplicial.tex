...
which are morphisms $U_2 \to U_1$. And so on.
...
which are morphisms $U_1 \to U_2$. And so on.
...
Suppose that $X\to Y$ is a morphism of $\mathcal{C}$ such that all
...
For every $n \geq 0$ we denote by $C[n]$ the cosimplicial set
$$
...
\section{Nerves of categories}
\label{section-nerves-categories}

\begin{definition}
\label{definition-nerve-category}
Let $\mathcal{C}$ be a category. View $[n]$ as the category whose objects are
$0, \ldots, n$ with a unique morphism $i \to j$ if $i \leq j$. The
{\it nerve of $\mathcal{C}$} is the simplicial set $N(\mathcal{C})$ given by
$$
N(\mathcal{C})_n = \{\text{functors }[n] \longrightarrow \mathcal{C}\}.
$$
The simplicial operations are given by precomposition. For a simplicial set
$U$ and $n \geq 2$, set
$$
P_n(U) = \{(a_1, \ldots, a_n) \in U_1^n \mid
d_0(a_i) = d_1(a_{i + 1})\text{ for }1 \leq i < n\}.
$$
The {\it $n$th Segal map} is the map
$$
U_n \longrightarrow P_n(U)
$$
which sends a simplex to its consecutive edges.
\end{definition}

\begin{lemma}
\label{lemma-characterize-nerves-categories}
The functor
$$
N : \textit{Cat} \longrightarrow \textit{Simp}(\textit{Sets})
$$
is fully faithful. A simplicial set $U$ is isomorphic to the nerve of a
category if and only if all its Segal maps are bijective.
\end{lemma}

\begin{proof}
An $n$-simplex of $N(\mathcal{C})$ is the same thing as a string
$$
x_0 \xrightarrow{a_1} x_1 \xrightarrow{a_2} \ldots
\xrightarrow{a_n} x_n
$$
of composable arrows of $\mathcal{C}$. Thus all the Segal maps of
$N(\mathcal{C})$ are bijective.

Conversely, suppose the Segal maps of $U$ are bijective. Define a category
$\mathcal{C}_U$ as follows. Its objects are the elements of $U_0$, its arrows
are the elements of $U_1$, and the source and target of an arrow $a$ are
$d_1(a)$ and $d_0(a)$. The identity of $x \in U_0$ is $s_0(x)$. If $a$ and
$b$ are composable, let $z \in U_2$ be the unique simplex whose consecutive
edges are $a$ and $b$, and set
$$
b \circ a = d_1(z).
$$
The simplicial identities in degrees at most $2$ give the identity axioms.
The bijectivity of the Segal map in degree $3$, together with the simplicial
identities, gives associativity.

The Segal maps give an isomorphism $U \to N(\mathcal{C}_U)$. Finally, a map
between nerves is determined in degrees $0$ and $1$. Compatibility with the
face and degeneracy maps says that it preserves sources, targets, and
identities, while compatibility in degree $2$ says that it preserves
composition. Hence it comes from a unique functor. This proves full
faithfulness and the description of the essential image.
\end{proof}

\begin{remark}
\label{remark-internal-category-nerve}
Let $\mathcal{A}$ be a category with finite limits. The same construction
works internally. Namely, a simplicial object $U$ of $\mathcal{A}$ is the
nerve of a category object if and only if the Segal morphisms
$$
U_n \longrightarrow
U_1 \times_{U_0} \ldots \times_{U_0} U_1
$$
are isomorphisms for all $n \geq 2$. The source, target, and identity are
$d_1$, $d_0$, and $s_0$. Composition is obtained from $d_1 : U_2 \to U_1$
after inverting the Segal morphism in degree $2$. The proof of the preceding
lemma uses only finite limits and commutative diagrams, so it applies without
change.
\end{remark}
...
products. Let $U$, $V$ be simplicial objects of $\mathcal{C}$.
...
has a final object in this case, namely the identity
map $[n] \to [n]$.
\end{proof}
...
\begin{enumerate}
\item We have seen in Example \ref{example-cosk0} that if $\mathcal{C}$
...
$i = 1$ and $u \in U_1$. In other words, we have
$$
\Mor(U, \Delta[n])
=
...
\begin{reference}
\cite[Chapter III, \S 1, no. 53, Lemma 4, p. 246]{FAC}
\end{reference}
\begin{history}
The cited proof identifies the positive-degree alternating Cech complex of a
special finite covering of a classical irreducible curve with the cochain
complex of the simplex on its finite vertex set, and uses the latter's
trivial higher cohomology. The example below gives the underlying simplicial
homotopy contraction for every nonempty standard simplex.
\end{history}
...
\begin{lemma}
\label{lemma-nerve-groupoid-kan}
Let $\mathcal{C}$ be a category. The following are equivalent.
\begin{enumerate}
\item The category $\mathcal{C}$ is a groupoid.
\item The nerve $N(\mathcal{C})$ is a Kan complex.
\item Every map
$\Lambda_k[n] \to N(\mathcal{C})$ with $n \geq 2$ has a unique extension
to $\Delta[n]$.
\end{enumerate}
\end{lemma}

\begin{proof}
Suppose $\mathcal{C}$ is a groupoid. An inner horn in its nerve has a unique
filler because composition in $\mathcal{C}$ is defined and associative. An
outer horn has a unique filler because every arrow is invertible. This proves
(3). Horns in degree $1$ can be filled using identity arrows, so (3) implies
(2).

Assume (2), and let $a : x \to y$ be an arrow of $\mathcal{C}$. Filling the
$2$-horn whose two given edges are $a : x \to y$ and
$\text{id}_y : y \to y$ produces an arrow $b : y \to x$ such that
$a \circ b = \text{id}_y$. Filling the other outer $2$-horn produces an arrow
$c : y \to x$ such that $c \circ a = \text{id}_x$. Then
$$
b = \text{id}_x \circ b = (c \circ a) \circ b
= c \circ (a \circ b) = c,
$$
and $a$ is invertible. Thus $\mathcal{C}$ is a groupoid.
\end{proof}

\begin{remark}
\label{remark-internal-groupoid-nerve}
The argument is internal. In the situation of
Remark \ref{remark-internal-category-nerve}, the category object is a
groupoid object if and only if the two outer horn morphisms in degree $2$ are
isomorphisms. The inverse morphism is recovered by filling either outer
$2$-horn.
\end{remark}
...
satisfying the equations of Lemma \ref{lemma-godement}.
...
In Situation \ref{situation-adjoint-functors} the maps
$$
...
\begin{proof}
As in the proof of Lemma \ref{lemma-standard-simplicial}
we set $Y = U \circ V$ so that we are
in the situation of Example \ref{example-godement}.

\medskip\noindent
Proof of the first homotopy equivalence. By Lemma \ref{lemma-godement-section}
to construct a map
...
Lemma \ref{lemma-godement-two-maps} (with
$G = G' = V$ and $F = F' = \text{id}_\mathcal{S}$).

\medskip\noindent
The proof of the second homotopy equivalence. By
Lemma \ref{lemma-godement-section} to construct a map
...
\ref{lemma-trivial-kan-homotopy}, \ref{lemma-homotopy-equivalence}, and
...
Example \ref{example-module}.) For an $A$-algebra $B$
...
In Example \ref{example-module} we have $X_n(M) = R[R[\ldots [M]\ldots]]$
...
\begin{example}
\label{example-relative-bar-resolution}
\begin{reference}
\cite[II, Section 11.4.4, pp. 172--173]{cotangent}
\end{reference}
\begin{history}
The cited source calls this the bar resolution relative to $K \to R$.
Here we record its ordinary commutative-ring specialization. It is the
extension--restriction instance of the standard resolution above.
\end{history}
Let $K \to R$ be a ring map. In
Situation \ref{situation-adjoint-functors} take
$$
\mathcal{A} = \text{Mod}_R,\qquad \mathcal{S} = \text{Mod}_K,
$$
let $V : \mathcal{A} \to \mathcal{S}$ be restriction of scalars, and let
$U : \mathcal{S} \to \mathcal{A}$ be given by
$U(N) = R \otimes_K N$. For an $R$-module $M$ the resulting augmented
simplicial $R$-module, denoted $\operatorname{Bar}(R/K, M)$, has terms
$$
\operatorname{Bar}_n(R/K, M) =
R \otimes_K \ldots \otimes_K R \otimes_K M
$$
with $n + 1$ factors $R$. On a pure tensor
$r_0 \otimes \ldots \otimes r_n \otimes m$, the map $d_i$ multiplies
$r_i$ and $r_{i + 1}$ if $0 \leq i < n$, the map $d_n$ applies the action
of $r_n$ to $m$, and the map $s_i$ inserts $1$ immediately after $r_i$.
The augmentation in degree zero is
$$
R \otimes_K M \longrightarrow M,\qquad r \otimes m \longmapsto rm.
$$
By Lemma \ref{lemma-standard-simplicial-homotopy} the augmentation is a
homotopy equivalence after restriction to simplicial $K$-modules. Hence
Lemma \ref{lemma-homotopy-equivalence} shows that its associated augmented
chain complex is acyclic. The same holds for its normalized augmented chain
complex by Lemma \ref{lemma-quasi-isomorphism}. Thus both are resolutions of
$M$ by $R$-modules.
\end{example}
