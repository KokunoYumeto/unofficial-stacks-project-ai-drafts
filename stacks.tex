...
be families of morphisms of $\mathcal{C}$ with fixed target.
...
The second condition guarantees that the $\mathit{Mor}$-presheaves
of $\mathcal{S}'$ are sheaves (as they are identical to their counterparts
...
in $\mathcal{S}_2$ relative to the covering $\{U_i \to U\}$ of the site
$\mathcal{C}$. Then $(G(X_i), G(\varphi_{ii'}))$ is a descent datum
in $\mathcal{S}_1$ relative to the covering $\{U_i \to U\}$.
...
$U$ of $\mathcal{C}$. Next, let $(U, x', y', \phi')$ be a second
object lying over $U$.
...
$z' = f(x') = g(y')$. With these identifications
it is clear that
$$
...
is a functor from a ``big'' category to $\mathcal{C}$, i.e., suppose
...
by an equivalent category such that we obtain a stack. The reason is that
it can happen that a fibre category $\mathcal{S}_U$ may have a proper
...
over $U$ whose image under $G'$ in $\mathcal{S}_2$ is $\text{id}_x$.
...
is trivial. To do this we may replace $U$ by the members of
...
(h : V \to U)
\longmapsto
\{(x, f) \mid x \in \Ob(\mathcal{S}_V), f : F(x) \to h^*y\text{ over }V\}/\cong
$$
...
that $\varphi_{ij} = (\alpha_j|_{U_i \times_U U_j})^{-1} \circ
\alpha_i|_{U_i \times_U U_j}$. This finishes the proof.
\end{proof}
...
$x$ lying over $U$ and any object $y$ lying over $V$
...
that this functor preserves strongly cartesian morphisms,
...
$\xi = (X_i, \varphi_{ii'})$ is a descent datum relative to $\mathcal{U}$.
...
be the stack and $1$-morphism constructed in Lemma \ref{lemma-stackify}.
...
an object $y$ of $\mathcal{X}_U$ agreeing with $F(x_i)$ over $U_i$.
...
is the inertia of the stackification of $\mathcal{X}$.
\end{lemma}
...
be the stack in groupoids and $1$-morphism constructed in
...
fibred category. We say that $(\mathcal{S}, \text{Cov}(\mathcal{S}))$ as in
Lemma \ref{lemma-topology-inherited}
...
Then $F(x_i) \to F(x)$ and $y_i \to F(x)$ are both strongly cartesian
...
on $\mathcal{C}/p(x)$. Note that $s_i = F(\psi_i) \in \mathit{I}(q(y_i))$
...
\begin{proof}
By the uniqueness property of a $2$-fibre product we may assume that
...
Write $x'_i = (U, y'_i, x_i, \alpha_i)$. Note that $F'(x'_i) = y'_i$
and $G'(x'_i) = x_i$. By assumption there exists
a covering $\{U_i \to U\}$ in $\mathcal{C}$ and morphisms
...
Then both $(a, \beta) : (U, y) \to (U', y')$ and
...
only if $\beta$ is strongly cartesian in $\mathcal{S}$. Hence,
given a morphism $a : U \to U'$ of $\mathcal{C}$, not only
do we have the equalities
$(u^p\mathcal{S})_U = \mathcal{S}_{u(U)}$ and
$(u^p\mathcal{S})_{U'} = \mathcal{S}_{u(U')}$, but via these
...
we see that descent data relative to $\mathcal{U}$ are identical
to descent data relative to $\mathcal{V}$. Since by assumption we have
...
we see that $\phi'_1 = (\phi_1, \phi' \circ b) : V_1 \to u(U'_1)$ is
a morphism in $\mathcal{D}$. Let $\gamma_1 : x_1' \to x_1$
be a strongly cartesian morphism of $\mathcal{S}$ with
$p(\gamma_1) = c'$ (which exists because $\mathcal{S}$ is a
...
$\phi_1 = u(d) \circ \phi_2$. Let $\delta : x_2 \to x_1$ be a strongly
cartesian morphism of $\mathcal{S}$ with $p(\delta) = d$.
...
such that $p_p(f_2) = b \circ b_1$ for some $b_1 : V \to V_1$.
...
in $u_p\mathcal{S}$ with $p_p(f'_1) = b_1$ and $f'_2 = f \circ f'_1$.
...
$\gamma' : x'' \to x'$ is a strongly cartesian morphism lifting $c'$.
...
categories over $\mathcal{D}$. Denote by $G' : u_{pp}\mathcal{S} \to \mathcal{T}$
...
a pair $(U, y)$ with $y \in \Ob(\mathcal{T}_{u(U)})$. We denote by
...
(\phi')^*\text{pr}(H(x')) \ar[d] \\
\text{pr}(H(x)) \ar[r]^{\text{pr}(H(\alpha/a))} & \text{pr}(H(x'))
}
$$
...
Suppose that $f : X \to Y$ is strongly cartesian. By the characterization
of strongly cartesian morphisms in $u_p\mathcal{S}$ we can write $f$ as
$((a, b, \alpha) : X' \to Y, r : X' \to X)$ where $r \in R$ and
...
$\text{pr}(H(\alpha/a)) : \text{pr}(H(x)) \to \text{pr}(H(x'))$
is strongly cartesian in $\mathcal{T}$. Since in the commutative
square above now all arrows except possibly $\beta$ are strongly cartesian
...
\medskip\noindent
Full faithfulness. To prove this we are going to use
...
$$
directly from the definition of $f_*$. The same holds after
...
$$
Hence to finish the proof of full faithfulness it suffices
to show that for any $U$ and $x, y$ the map
$$
\Mor_{\mathcal{S}_U}(x, y)
\longrightarrow
\Mor_{(u_p\mathcal{S})_{u(U)}}(c'(x), c'(y))
$$
...
$c \circ c' = \text{id}_U$. Because $\gamma$ is strongly cartesian
...
presheaf is a sheaf. We denote by $j : \mathcal{C}/U \to \mathcal{C}$
