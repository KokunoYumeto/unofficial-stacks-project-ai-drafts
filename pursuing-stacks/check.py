...
def main():
    ...
    record = json.loads((ROOT / "integration.json").read_text(encoding="utf-8"))
    source = identity(ROOT / record["source"]["path"], record["source"]).decode("utf-8")
    mapping_raw = (ROOT / "source-map.json").read_bytes()
    ...
    assert statements == source.count(r"\begin{proof}") == record["proof_bearing_statements"] == 7
    ...
    checks = ["source identity", "reference closure", "seven statements and proofs", "source map", "standalone source"]
    if args.authority:
        ...
        checks.append("eleven exact historical source spans and license identity")
    if args.received:
    ...
    if args.build:
        ...
        identity(args.build / "a" / "02-category-models.pdf", receipt["pdf"])
        identity(args.build / "b" / "02-category-models.pdf", receipt["pdf"])
