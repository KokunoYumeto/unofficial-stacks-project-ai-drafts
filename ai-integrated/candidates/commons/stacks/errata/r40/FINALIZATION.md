# R40 additive finalization order
...
3. Map all eleven operations to the sealed candidate PDF, render every page,
   inspect every rendered page at the stated resolution, and preserve explicit
   visual limitations and any adverse evidence.
4. `python finalize_r40.py mechanical` rehashes source, build, PDF, link,
   source-page-map, mutex, and render evidence without asserting visual review.
5. `python finalize_r40.py prepare` freezes every completed artifact except the
...
7. `python finalize_r40.py seal` writes the top manifest last; then
