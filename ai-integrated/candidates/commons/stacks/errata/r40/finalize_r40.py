"""Append-only R40 finalization; every stage fails closed."""
...
def mechanical() -> dict:
    ...
    assert len(mapping["operations"]) == 11
    ...
    report = {
        "schema": "stacks-r40-final-mechanical-validation-v1",
        "passed": True,
        ...
        "adverse_evidence": [
            "Authority and candidate each have twenty overfull hboxes and one underfull vbox; visual disposition is bound separately.",
...
def visual_receipts() -> list[dict]:
    ...
    assert len(raw["viewed_contact_artifacts"]) == 6
    assert len(raw["viewed_high_resolution_artifacts"]) == 20
...
def prepare() -> dict:
    ...
    report = {
        "schema": "stacks-r40-final-stage-snapshot-v1",
...
def seal() -> dict:
    ...
    manifest = {
        ...
        "source_closure": {"enumerated": True, "expected_units": 11, "manifested_units": 11, "complete": True},
