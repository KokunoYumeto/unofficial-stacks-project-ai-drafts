"""Exact proposal replay for Sheaves on Spaces; no source/registry writes."""
import argparse
...
def verify_one(name, count, existing_count, model):
    ...
    retained_units = json.loads((HERE / 'SHEAVES_PRIOR_ADMITTED_UNITS_20260923.json').read_bytes())
    ...
    require([g['id'] for g in review['groups']] ==
            [f'SHEAVES-RECON-{n:03d}' for n in range(1,len(review['groups'])+1)], 'Group IDs differ')
...
def main():
    ...
    if a.candidate_head:
        ...
        for path in git('diff-tree','-r','--no-commit-id','--name-only',CURRENT,head).decode().splitlines():
            require(path=='ai-integrated/registry/leases.json' or path.startswith('ai-integrated/candidates/commons/stacks/errata/r54/'),'Not a candidate-only descendant: '+path)
    else:
        require(head==CURRENT,'HEAD advanced; rebind explicitly')
    model=intake.import_model()
    summaries=[]
    for name,count,existing in [('sheaves.tex',146,71)]:
