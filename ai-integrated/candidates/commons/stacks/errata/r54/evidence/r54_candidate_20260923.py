"""R54 registrar reservation, then candidate-only single-source materialization/replay."""
import argparse
...
BATCH_HASH = '78253EA6EA9C639EA9EFFF1CFA0EDED76505D8A0B37625F8C6CDBCFBD88C04BF'
...
def main():
    ...
    raw = (CONTROL / 'SHEAVES_CORRECTION_BATCH_INPUT_20260923.json').read_bytes()
    ...
    require(first == 1742 and len(batch['units']) == 12 and batch['operation_count'] == 16, 'Unexpected scope')
    sources = {s['source']: s for s in batch['sources']}
    require(set(sources) == {'sheaves.tex'}, 'Unexpected sources')
    if a.action == 'lease':
        ...
        namespace = 'commons/stacks/errata/r54'
        ...
        event = {'event_id': f"lease-event-{len(leases['events'])+1:06d}", 'event': 'issued',
            'lease_id': f'stacks-lease-{number:06d}-errata-r54', 'namespace': namespace,
        ...
        require(number == 60 and event['event_id'] == 'lease-event-000116', 'Lease state advanced')
    ...
    if a.action in ('materialize', 'revise'):
        ...
        for name in ('r54_candidate_20260923.py', 'r51_candidate_20260922.py'):
            write(candidate/'evidence'/name, (CONTROL/name).read_bytes())
        if a.action == 'revise':
            write(candidate/'evidence/initial-build-regression.json', (CONTROL/'R54_INITIAL_BUILD_REGRESSION_20260922.json').read_bytes())
        ...
        for i, u in enumerate(batch['units']):
            ...
            formula.append({'unit_id': sid, 'class': kind, 'operation_ids': operation_ids,
                'formula_policy': 'Only listed tokens change; label, reference, citation-key and environment inventories are invariant.'})
        require(len(units) == 12 and len(operations) == 16, 'Materialized count drift')
        ...
        require(len(rejected) == 20, 'Rejected/optional coverage changed')
        ...
        write(candidate/'materialization.json', {'schema': 'stacks-r54-materialization/v1', 'candidate_id': 'stacks-errata-a04446e-r54',
        ...
        text = ['# Sheaves on Spaces: proposed corrections and clarification\n',
    ...
    for name, source in sources.items():
        ...
        checks.append({'source': name, 'passed': True, 'isolated': ident(isolated), 'cumulative': ident(cumulative), 'labels_refs_citation_keys_environments_unchanged': True})
    write(candidate/'replay/source-replay.json', {'schema': 'stacks-r54-independent-source-replay/v1', 'candidate_id': 'stacks-errata-a04446e-r54',
