...
is an abelian group, then the zero element and the negation map are
...
\begin{reference}
\cite[Chapter I, \S 1, nos. 1--3, pp. 199--201]{FAC}
\end{reference}
Let $X$ be a topological space.
\begin{enumerate}
...
of sections $s_i$'' from the definition above actually forms
an element of the empty product which is the final object
of the category in which the sheaf takes values. In other words,
...
\begin{definition}
\label{definition-locally-constant-sheaf}
Let $X$ be a topological space. A sheaf of sets $\mathcal{F}$ on $X$
is {\it locally constant} if every point of $X$ has an open neighbourhood
$U$ such that $\mathcal{F}|_U$ is isomorphic to a constant sheaf on $U$.
\end{definition}

\begin{lemma}
\label{lemma-locally-constant-irreducible}
\begin{reference}
\cite[Chapter II, \S 1, no. 36, Lemmas 1 and 2, p. 229]{FAC}
\end{reference}
Let $X$ be a topological space and let $A$ be a set.
\begin{enumerate}
\item If $X$ is connected, then the canonical map
$A \to \Gamma(X, \underline{A})$ is bijective.
\item If $X$ is irreducible, then every locally constant sheaf of sets
on $X$ is constant.
\end{enumerate}
\end{lemma}

\begin{proof}
The first assertion follows from Definition \ref{definition-constant-sheaf}:
a locally constant map from a connected space to a discrete space is constant.

Let $X$ be irreducible and let $\mathcal{F}$ be locally constant. Put
$B = \mathcal{F}(X)$. Restriction of global sections gives a map from the
constant presheaf with value $B$ to $\mathcal{F}$ and hence a map
$$
\varphi : \underline{B} \longrightarrow \mathcal{F}.
$$
We show that $\varphi$ is an isomorphism on stalks.

Suppose first that $s, t \in B$ have the same germ at some $x \in X$.
The locus where their germs agree is open. On an open over which
$\mathcal{F}$ is constant, this locus and its complement are open, since
the two sections become locally constant maps to a set. Thus the locus is
also closed in $X$. It is nonempty, and an irreducible space is connected,
so it is all of $X$. This proves that $B \to \mathcal{F}_x$ is injective.

Conversely, let $m \in \mathcal{F}_x$. Represent $m$ by a section $s$
over a nonempty open $U$ containing $x$. Choose a covering of $X$ by
nonempty opens $U_i$ such that $\mathcal{F}|_{U_i}$ is constant. The opens
$U \cap U_i$ and $U_i$ are irreducible and hence connected. Choose
$x_i \in U \cap U_i$. By the first assertion there is a section $s_i$
over $U_i$ whose germ at $x_i$ is the germ of $s$. The equality-locus
argument above shows that $s_i$ and $s$ agree over $U \cap U_i$.
For every $i,j$, the open $U \cap U_i \cap U_j$ is nonempty, so $s_i$
and $s_j$ agree there and hence on the connected open $U_i \cap U_j$.
The sections $s_i$ therefore glue to a section of $\mathcal{F}$ over $X$
whose germ at $x$ is $m$. Thus $B \to \mathcal{F}_x$ is surjective.

We conclude that $\varphi$ is an isomorphism by
Lemma \ref{lemma-points-exactness}.
\end{proof}
...
\begin{reference}
\cite[Chapter I, \S 1, no. 6, pp. 202--203, and Chapter II,
\S 2, no. 38, p. 231]{FAC}
\end{reference}
Let $X$ be a topological space.
...
$s|_{U''} = s'|_{U''}$. Given a pair $(U, s)$ we sometimes denote by
...
if $V \subset U \subset X$ are open, the projection map
$$
\prod\nolimits_{u \in U} \mathcal{F}_u
\longrightarrow
...
\begin{reference}
\cite[Chapter I, \S 1, no. 3, pp. 200--201]{FAC}
\end{reference}
...
for the same reason that the presheaf of continuous
functions is a sheaf, see Example \ref{example-basic-continuous-maps}
...
\begin{reference}
\cite[Chapter I, \S 1, no. 3, Proposition 1, p. 200]{FAC}
\end{reference}
Let $X$ be a topological space.
...
if $\psi : \mathcal{F} \to i(\mathcal{G})$ is a morphism of presheaves
...
$a$ is a map of sheaves if and only if the corresponding family of maps $a_V$
satisfies the condition in Definition \ref{definition-f-map}.
...
$\xi_V : \mathcal{G}(V) \to \mathcal{F}(f^{-1}(V))$
by the restriction map $\mathcal{F}(f^{-1}(V)) \to \mathcal{F}(U)$
...
Let $\psi : \mathcal{H} \to \mathcal{G}$ be a $g$-map.
...
$\mathcal{G} \mapsto f_p\mathcal{G}$ are functors on
...
$\psi \mapsto f_*\psi \circ i_\mathcal{G}$ and $\varphi \mapsto c_\mathcal{F} \circ f_p\varphi$
...
is an abelian sheaf on $Y$. This is true because of the definition
...
\begin{proof}
Let $V \subset Y$ be open. We define the map of the lemma
to be the map
$$
...
\begin{proof}
Let $U \subset X$ be open. We define the map of the lemma
to be the map
\begin{eqnarray*}
...
of abelian presheaves which is $f_*\mathcal{O}$-linear. Namely,
...
Let $X$ be a topological space.
\begin{enumerate}
\item Let $x \in X$ be a point. Let $i_x : \{x\} \to X$ be the inclusion map.
...
Let $X$ be a topological space, and let $x \in X$ be a point.
...
abelian groups and algebraic structures. In the case of
...
be the closed subset of points $x$ such that the images of
...
we see that the images of $s$ and $s'$ in the stalk of
$p_i^{-1}\mathcal{G}$ at $x$ are different too since
$(p_i^{-1}\mathcal{G})_x = (f_{a \circ b}^{-1}\mathcal{G})_{p_k(x)}$
...
Write $s_{l, j}$ for the restriction of the pullback of $s_{l, i}$
...
$y \in V_{ijy} \subset U_i \cap U_j$ such that $\sigma_i|_{V_{ijy}} = \sigma_j|_{V_{ijy}}$.
...
Let $X$ be a topological space.
Let $\mathcal{B}$ be a basis for the topology on $X$.
Denote by $\Sh(\mathcal{B})$ the category of
...
\begin{proof}
The inverse functor is given in Lemma \ref{lemma-extend-off-basis} above.
...
Lemma \ref{lemma-extend-off-basis} needs some care. Here it is.
...
Let $\mathcal{B}$ be a basis for the topology on $X$.
Let $(\mathcal{C}, F)$ be a type of algebraic structure.
Denote by $\Sh(\mathcal{B}, \mathcal{C})$ the category of
...
\begin{proof}
The inverse functor is given in
...
Let $\mathcal{B}$ be a basis for the topology on $X$.
Let $\mathcal{O}$ be a sheaf of rings on $X$.
Denote by $\textit{Mod}(\mathcal{O}|_\mathcal{B})$ the category of
...
\begin{proof}
Let us first prove this for sheaves of sets.
Fix $V \in \mathcal{B}_Y$. Pick $s \in \mathcal{G}(V)$.
...
\medskip\noindent
Finally, we note that the $f$-map of sheaves of sets so constructed
...
then we see that the maps on stalks are morphisms of algebraic
...
$$
\item On the category of presheaves on $U$ we have $j_pj_* = \text{id}$.
\item On the category of sheaves on $U$ we have $j^{-1}j_* = \text{id}$.
\end{enumerate}
...
is over the collection of all $W \subset X$ open such that $V \subset W$
...
\item Let $\mathcal{G}$ be a sheaf of sets, abelian groups, or
...
\right.
$$
\item On the category of presheaves on $U$ we have $j_pj_{p!} = \text{id}$.
\item On the category of sheaves on $U$ we have $j^{-1}j_! = \text{id}$.
\end{enumerate}
\end{lemma}
...
(of the given type). We define the
...
on $U$, then $j_{!, \mathrm{rings}}\mathcal{O} \not = j_{!, \mathrm{abelian}}\mathcal{O}$
...
\right.
$$
\item On the category of abelian presheaves on $U$
we have $j_pj_{p!} = \text{id}$.
\item On the category of abelian sheaves on $U$
...
such that $\mathcal{C}$ has an initial object $e$.
Consider the functors of restriction and extension
by $e$ for (pre)sheaves of algebraic structures defined above.
\begin{enumerate}
\item The functor $j_{p!}$ is a left adjoint to the
restriction functor $j_p$ (see Lemma \ref{lemma-j-pullback}).
\item The functor $j_!$ is a left adjoint to restriction,
...
$$
bifunctorially in $\mathcal{F}$ and $\mathcal{G}$.
\item Let $\mathcal{F}$ be a sheaf of $\mathcal{O}|_U$-modules on $U$.
The stalks of the sheaf $j_!\mathcal{F}$ are described
as follows
$$
(j_{!}\mathcal{F})_x =
...
\begin{proof}
Full faithfulness follows formally from $j^{-1} j_! = \text{id}$.
...
\begin{proof}
Full faithfulness follows formally from $i^{-1} i_* = \text{id}$.
We have seen that any sheaf in the image of the functor has
the property on the stalks mentioned in the lemma. Conversely, suppose
that $\mathcal{G}$ has the indicated property.
Then it is easy to check that
...
\begin{reference}
\cite[Chapter I, \S 1, no. 5, Propositions 5 and 6, p. 202]{FAC}
\end{reference}
Let $X$ be a topological space.
Let $i : Z \to X$ be the inclusion of a closed subset.
The functor
...
\}.
$$
Restriction mappings for $W' \subset W$ are defined by restricting
...
of a glueing datum assures that we may start with {\it any}
...
\begin{reference}
\cite[Chapter I, \S 1, no. 4, Proposition 4, pp. 201--202]{FAC}
\end{reference}
Let $X$ be a topological space.
Let $X = \bigcup U_i$ be an open covering.
Let $(\mathcal{F}_i, \varphi_{ij})$ be a glueing datum
