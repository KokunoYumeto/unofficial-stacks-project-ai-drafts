"""Fail-closed mechanical closure, snapshot, and manifest sealing for R47."""
...
PRIVATE_BASE = (
    ...
    / "03_projects/language_management/cjk/03_working_translations/"
      "stacks_cjk_20260821/canon/private_evidence/errata-r47-20260905"
)
PRIVATE_RENDER = PRIVATE_BASE / "render"
PRIVATE_BUILD_ROOTS = [PRIVATE_BASE / "build-a", PRIVATE_BASE / "build-b"]
CANDIDATE_ID = "stacks-errata-a04446e-r47"
STEM = "more-groupoids"
PAGE_COUNT = 36
UNIT_COUNT = 22
PRODUCER_ROW_COUNT = 23
OPERATION_COUNT = 27
REJECTION_COUNT = 0
FIRST_ID = 1523
LAST_ID = 1544
...
def replay_source() -> dict:
    authority_path = ROOT / f"authority/source/{STEM}.tex"
    payload_path = ROOT / f"payload/{STEM}.tex"
...
def validate_source_stage() -> dict:
    ...
    assert review["replay"]["producer_rows"] == PRODUCER_ROW_COUNT
    ...
    expected_ids = [f"MC-STK-ERR-{number}" for number in range(FIRST_ID, LAST_ID + 1)]
    assert units_doc["unit_count"] == len(units) == UNIT_COUNT
    assert stable_ids == expected_ids
    producer_ids = [pid for unit in units for pid in unit["producer_ids"]]
    assert len(producer_ids) == len(set(producer_ids)) == PRODUCER_ROW_COUNT
    ...
    assert len(decisions) == UNIT_COUNT
    assert all(row["stable_id"] is not None for row in decisions)
    assert {row["stable_id"] for row in decisions} == set(stable_ids)
    ...
    assert sorted(pid for row in source_map for pid in row["producer_ids"]) == sorted(producer_ids)

    config = load(ROOT / "candidate.config.json")
    assert config["candidate_id"] == CANDIDATE_ID
    assert config["accepted"] == UNIT_COUNT
    assert config["accepted_producer_rows"] == PRODUCER_ROW_COUNT
    ...
    assert regeneration["schema"] == "stacks-r47-source-regeneration/v1"
...
def validate_build() -> dict:
    ...
    assert attempt["guarded_commands"][0]["exit_code"] == 1
    for row in (
    ...
    for phase_name in ("candidate", "authority"):
        ...
        private_name = f"{STEM}.{phase_name}"
        for build_index, private_root in enumerate(PRIVATE_BUILD_ROOTS):
            private_fls = private_root / f"{private_name}.fls"
            private_inventory = private_root / f"{private_name}.fls-dependencies.json"
...
def validate_render() -> dict:
    ...
    assert visual["scope"]["high_resolution_pages"] == SENSITIVE_PAGES
    ...
    for row in stage.values():
        if isinstance(row, dict) and {"path", "sha256"}.issubset(row):
            bound(row)
    assert public["pdf"]["sha256"] == sha(ROOT / f"builds/{STEM}.pdf")
    assert [row["page"] for row in public["pages"]] == list(range(1, PAGE_COUNT + 1))
    assert [row["page"] for row in public["high_resolution"]["renders"]] == SENSITIVE_PAGES
...
def mechanical() -> dict:
    ...
    report = {
        "schema": "stacks-r47-final-mechanical-validation/v1",
        ...
        "producer_ids": source["producer_ids"],
        "adverse_evidence": [
            "Attempt 001 failed closed at the fresh-build boundary and is retained; the full corrected workflow used fresh temporary work roots and passed.",
            "The candidate and authority each retain one inherited overfull hbox and one inherited underfull vbox; page-complete visual review found no blocking defect.",
...
def prepare() -> dict:
    ...
    report = {
        "schema": "stacks-r47-final-stage-snapshot/v1",
        "candidate_id": CANDIDATE_ID,
    ...
    manifest = {
        ...
        **{key: ev(ROOT / relative) for key, relative in primary.items()},
        ...
        "unresolved_defects": [
            ...
            "One inherited overfull hbox and one inherited underfull vbox remain in both candidate and authority, with no blocking visual defect.",
