"""Independent exact-byte review of the already materialized R43 source stage."""
...
EXPECTED_IDS = [f"MC-STK-ERR-{number}" for number in range(1430, 1437)]
EXPECTED_PRODUCER_IDS = [f"DESCENT-{number:03d}" for number in range(41, 48)]
...
def main() -> int:
    ...
    assert len(payload) == 353754 and sha_bytes(payload) == PAYLOAD_SHA
    ...
    adjudication = load(ROOT / "authority/registrar/DESCENT_R43_INDEPENDENT_ADJUDICATION_20260905.json")
    assert spec["authority_sha256"] == AUTH_SHA and spec["operation_count"] == 7
    assert stable["unit_count"] == 7
    ...
    assert len({row["operation_id"] for row in operations}) == 7
    ...
    assert len(units_by_id) == len(maps_by_id) == 7
    ...
    observed = [
        ...
        for relative in (
            ...
            "authority/registrar/DESCENT_R43_INDEPENDENT_ADJUDICATION_20260905.json",
    ...
    report = {
        "schema": "stacks-r43-independent-source-validation/v1",
        ...
        "checks": {
            ...
            "operations": 7,
            "unique_operation_ids": 7,
            "exact_byte_preimages_lengths_hashes_and_line_numbers": 7,
            "exact_replacement_lengths_hashes": 7,
            "nonoverlapping": True,
            "descending_replay_equals_payload_byte_for_byte": True,
            "stable_units": 7,
            "stable_ids": "MC-STK-ERR-1430..MC-STK-ERR-1436",
        ...
        "producer_scope": {
            "r43_rows": "DESCENT-041..047",
            "preserved_predecessor_rows": "DESCENT-001..040 remain bound to earlier append-only rounds and are not resent",
        },
    ...
    print(json.dumps({"passed": True, "units": 7, "operations": 7, "payload_sha256": PAYLOAD_SHA, "receipt_sha256": sha_path(output)}))
    return 0
