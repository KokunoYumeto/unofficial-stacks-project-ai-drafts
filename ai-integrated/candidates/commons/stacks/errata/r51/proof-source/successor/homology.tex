...
where $1 < i < n$ if
$\Im(x_{i - 1} \to x_i) = \Ker(x_i \to x_{i + 1})$. We say a
...
So, $j$ is an epimorphism and thus an isomorphism. This implies (1).
\end{proof}
...
$x \xrightarrow{\alpha} u \xrightarrow{\pi''} \Coker(\alpha)$ and
...
The same holds if (3) holds. It follows that (2) and (3)
...
to see that it suffices to check after composition with
...
$F$ sends products to products, hence $F$ preserves direct sums,
...
\medskip\noindent
Let us show the construction above is well defined.
...
$a \circ r = s_1 \circ f$ and $b \circ r = s_2 \circ g$.
...
\cite[Chapter III, \S 2, no. 56, p. 249]{FAC}
\end{reference}
\begin{history}
The cited FAC passage applies Serre's original notion of a class to the
graded modules whose components vanish in all sufficiently large degrees.
Its closure statement for an exact sequence $A \to B \to C$ is exactly the
condition in (1). The cutoff is uniform for the module; it should not be
confused, for an infinitely generated module, with the weaker assertion
that every element is killed by some power of an irrelevant ideal.
\end{history}
Let $\mathcal{A}$ be an abelian category.
\begin{enumerate}
...
\end{reference}
\begin{history}
The cited source calls a morphism $\mathcal{C}$-injective or
$\mathcal{C}$-surjective when its kernel or cokernel, respectively, lies in
$\mathcal{C}$, and $\mathcal{C}$-bijective when both conditions hold. These
are exactly the arrows in the multiplicative system $S$ used below; they
become isomorphisms in the Serre quotient, but need not be isomorphisms in
...
The proofs of Lemmas \ref{lemma-serre-subcategory-is-kernel} and
\ref{lemma-localization-abelian} show that we can choose for $i \in I$
...
\begin{lemma}
\label{lemma-dualize-delta-functor}
\begin{reference}
\cite[Chapter III, \S 4, no. 70, exact sequences for the functors
$B^q$ and $T^q$, p. 264]{FAC}
\end{reference}
\begin{history}
In the cited source, $F^q=B^q$ is the cohomological delta functor attached
to the positive graded Koszul--{\v C}ech complexes and $D$ is the exact
degreewise $K$-dual of Algebra, Definition
\ref{algebra-definition-graded-K-dual}. Thus $T^q=D\circ B^q$ is
contravariant and its displayed exact sequence is the reverse of the
$B^q$ sequence. The statement below isolates that categorical operation.
\end{history}
Let $\mathcal A$, $\mathcal B$, and $\mathcal C$ be abelian categories.
Let $(F^n,\delta)$ be a cohomological delta functor from $\mathcal A$ to
$\mathcal B$, and let
$D:\mathcal B^{opp}\to\mathcal C$ be an additive exact functor. Set
$T^n=D\circ F^n$. Then the $T^n$ are contravariant functors and every short
exact sequence $0\to A\to B\to C\to0$ in $\mathcal A$ gives an exact
sequence
$$
\ldots\longrightarrow T^{n+1}(A)\longrightarrow T^n(C)
\longrightarrow T^n(B)\longrightarrow T^n(A)\longrightarrow\ldots.
$$
\end{lemma}

\begin{proof}
The delta-functor axioms give an exact sequence
$$
\ldots\longrightarrow F^n(A)\longrightarrow F^n(B)
\longrightarrow F^n(C)\longrightarrow F^{n+1}(A)\longrightarrow\ldots.
$$
Applying the exact contravariant functor $D$ reverses all arrows and
preserves exactness, which gives the displayed sequence. Functoriality is
the composite of the covariant functoriality of $F^n$ and the
contravariant functoriality of $D$.
\end{proof}
...
transformations of functors $t^n : F^n \to G^n$, $n \geq 0$ such
...
\begin{reference}
\cite[Chapter III, \S 4, no. 72, properties (ii)--(iv) and the
derived-functor argument, pp. 265--267]{FAC}
\end{reference}
\begin{history}
The cited source applies this criterion to contravariant functors of graded
modules, equivalently to covariant functors on the opposite category. A
surjection from a graded free module is an embedding in that opposite
category, and the positive functors vanish on the free module. More on
Algebra, Lemma
\ref{more-algebra-lemma-graded-local-duality-polynomial-ring} carries out
this application with the exceptional kernel--cokernel splice included.
\end{history}
Let $\mathcal{A}, \mathcal{B}$ be abelian categories.
Let $F = (F^n, \delta_F)$ be a $\delta$-functor
from $\mathcal{A}$ to $\mathcal{B}$.
...
transformations of functors $t^i$ for $i \leq n$ compatible with
...
\begin{reference}
\cite[Chapter III, \S 4, no. 72, axiomatic identification of the
functors $E^q$ with graded Ext, p. 267]{FAC}
\end{reference}
\begin{history}
The cited application first identifies $E^0$ with internal graded Hom and
then invokes the derived-functor characterization to identify all higher
$E^q$. In the modern formulation, both delta functors are effaceable on the
opposite category of graded modules, so the comparison is the unique
isomorphism extending the degree-zero pairing.
\end{history}
Let $\mathcal{A}, \mathcal{B}$ be abelian categories.
Let $F : \mathcal{A} \to \mathcal{B}$ be a functor.
...
the exactness in part (2) of Lemma \ref{lemma-snake}.
...
$$
which occur in the long exact cohomology sequence associated
...
$$
of vector spaces. As unit we use the graded $F$-vector space $\mathbf{1}$
...
\begin{reference}
\cite[Chapter I, \S 4, no. 27, pp. 219--220]{FAC}
\end{reference}
Let $\mathcal{A}$ be an additive category.
...
and then combine the sum of those with the last, or we can first combine the
...
be a homotopy between $f$ and $g$. If we denote
$h^{p, q} : A^{p, q} \to B^{p - 1, q}$ the component in degree $q$ of $h^p$,
then this means that
$$
...
Remark \ref{remark-shift-double-complex}. To see this denote
$s^{p, q} : C^{p, q} \to B^{p, q}$ and
$\pi^{p, q} : B^{p, q} \to A^{p, q}$ the components of $s^p$ and $\pi^p$.
As splittings
$(s')^n : \text{Tot}^n(C^{\bullet, \bullet}) \to
\text{Tot}^n(B^{\bullet, \bullet})$
and
...
\begin{proof}
We have $F^{p + 1}X = X \cap F^{p + 1}A$, hence the map
...
The two short exact sequences of (2) are special cases of the
...
map of finite filtered objects which induces an isomorphism
...
$A/F^p(A) \to B/F^p(B) \to C/F^p(C)$ is exact for $p \leq n$.
...
By Section \ref{section-exact-couple} we get an associated spectral sequence.
...
Lemma \ref{lemma-spectral-sequence-associated-exact-couple}
...
\item $d_r : E_r \to E_r$ is given as follows: given an element $\overline{z} \in Z_r$, choose a representative $z \in d^{-1}(\alpha^r A)$
...
\alpha : A \to A[-1]
$$
which is given in degree $p$ by the inclusions $F^{p + 1}K \to F^pK$.
This is clearly an injective morphism of differential objects
...
First of all we have that $E_r = (E_r^p)$ is an object of $\text{Gr}(\mathcal{A})$.
Then, since $T^rS = [r]$ we have $d_r : E_r \to E_r[r]$ which means that
$d_r^p : E_r^p \to E_r^{p + r}$.

\medskip\noindent
To see that the description of the graded pieces holds, we argue
...
translates in a straightforward manner into the formulae
...
$F^{p + r}K^{n + 1} = 0$ for $r \gg 0$. Thus (\ref{equation-on-top-bigraded})
...
$\mathcal{C}$ is a weak Serre subcategory of $\mathcal{A}$. Then $E_{r + 1}^{p, q}$ is
...
\end{align*}
Note that all the intervening objects are in the smallest weak Serre
...
\begin{enumerate}
\item for every $n$ there exists $p_0(n)$ such that
$H^n(F^pK^\bullet) = 0$ for $p \geq p_0(n)$,
\item for every $n$ there exists $p_1(n)$ such that
...
(\ref{equation-at-bottom-bigraded}) we use that for $p - r + 1 < p_1(n)$
we have
$$
d(F^{p - r + 1}K^{n - 1}) = \Im(d) \cap F^{p - r + 1}K^n
$$
because the map $F^{p - r + 1}K^\bullet \to K^\bullet$ induces an
isomorphism on cohomology in degree $n$. This shows that
we have
$$
F^pK^{n} \cap d(F^{p - r + 1}K^{n - 1}) + F^{p + 1}K^{n} =
\Im(d) \cap F^pK^{n} + F^{p + 1}K^{n}
...
\begin{reference}
\cite[Chapter I, \S 4, no. 27, Proposition 1, p. 220]{FAC}
\end{reference}
Let $\mathcal{A}$ be an abelian category.
...
and only if their images in $H^0(M^\bullet)$ are the same.
\end{proof}
...
by an element in $\Ker(d^{-n}_{A_{n - 1}^\bullet})$ we may assume
...
the following trick. Set $B_{n + 1}^\bullet = A_n^\bullet$ for $n \geq 0$
...
Then $x_1 + d^0_{A_1^\bullet}(y_0) = f_2^1(y_1)$ for some
...
if every object $A$ has a surjective morphism
$P \to A$ from a projective object $P$ onto it.
\end{definition}
...
\item for any object $A \in \Ob(\mathcal{A})$
the object $s(P(A))$ is a projective object of $\mathcal{A}$.
\end{enumerate}
We will denote such a functor by
$A \mapsto (P(A) \to A)$.
\end{definition}
...
Note that the projection onto $F$ defines a natural transformation
...
is exact if and only if each $K_i \to L_i \to M_i$ is exact.
\end{enumerate}
\end{lemma}
...
that the associated sequence of limits is exact. In order to discuss
...
$\lim_{\gamma < \beta} K^\bullet_\gamma$ are acyclic.
