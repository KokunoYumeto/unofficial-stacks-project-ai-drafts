"""Independent exact-byte review of the already materialized R41 source stage."""
...
EXPECTED_IDS = [f"MC-STK-ERR-{number}" for number in range(1413, 1419)]
EXPECTED_PRODUCER_IDS = [f"DESCENT-{number:03d}" for number in range(24, 30)]
...
def main() -> int:
    ...
    assert len(payload) == 353786 and sha_bytes(payload) == PAYLOAD_SHA
    ...
    adjudication = load(ROOT / "authority/registrar/DESCENT_R41_INDEPENDENT_ADJUDICATION_20260905.json")
    assert spec["authority_sha256"] == AUTH_SHA and spec["operation_count"] == 6
    assert stable["unit_count"] == 6
    ...
    assert len({row["operation_id"] for row in operations}) == 6
    ...
    observed = [
        ...
        for relative in (
            ...
            "authority/registrar/DESCENT_R41_INDEPENDENT_ADJUDICATION_20260905.json",
    ...
    report = {
        "schema": "stacks-r41-independent-source-validation/v1",
        ...
        "checks": {
            ...
            "operations": 6,
            "unique_operation_ids": 6,
            "exact_byte_preimages_lengths_hashes_and_line_numbers": 6,
            "exact_replacement_lengths_hashes": 6,
            "nonoverlapping": True,
            "descending_replay_equals_payload_byte_for_byte": True,
            "stable_units": 6,
            "stable_ids": "MC-STK-ERR-1413..MC-STK-ERR-1418",
        ...
        "producer_scope": {
            "r41_rows": "DESCENT-024..029",
            "preserved_out_of_scope_rows": "DESCENT-030..040 reserved for a later append-only round",
        },
    ...
    print(json.dumps({"passed": True, "units": 6, "operations": 6, "payload_sha256": PAYLOAD_SHA, "receipt_sha256": sha_path(output)}))
    return 0
