...
ROOT = helpers.PROD
BASE = helpers.BASE
CURRENT = '4cc7ccca622d29ae9893496e71ed4a162d285c03'
HASHES = {'fields.tex': '87E07F0373DC60CFC284E2F19078BC9BC7AB0B89C40EEF83941F6C38C765C549'}
...
def main():
    ...
    p.add_argument('--source', choices=HASHES, default='fields.tex')
    ...
    p.add_argument('--reports-only', action='store_true', help='Print reports without expanding long source-span envelopes')
    a = p.parse_args()
    if a.source_only and a.reports_only:
        p.error('Choose source-only or reports-only, not both')
    ...
    if a.reports_only:
        print('REPORTS',len(selected),'TOTAL',len(reports),'PRIMARY_SOURCE_DISPLAYED',False)
        return
