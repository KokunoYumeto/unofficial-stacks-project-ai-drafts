...
def main():
    ...
    for name,expected in [('fields.tex',75)]:
        stem=name.removesuffix('.tex').upper()
        review_path=source.HERE/f'{stem}_INTAKE_REVIEW_20260924.json'
        replay_path=source.HERE/f'{stem}_INTAKE_REVIEW_REPLAY_20260924.json'
        ...
        for f in [review_path.name,replay_path.name,f'{stem}_PENDING_SOURCE_REPAIRS_20260924.patch',
                  f'{stem}_PENDING_CUMULATIVE_REPAIRS_20260924.patch']:
    ...
    for f in ['FIELDS_PRIOR_ADMITTED_UNITS_20260924.json','fields_chapter_intake_20260924.py','verify_fields_intake_review_20260925.py',
              'prepare_fields_batch_20260925.py', 'test_fields_intake_replay_20260925.py',
              'test_sites_intake_replay_20260923.py', 'finalize_fields_proof_review_20260925.py']:
        evidence.append({'path':f,**identity((source.HERE/f).read_bytes())})
    result={'schema':'stacks-next-correction-batch-input/v1','prepared_on':'2026-09-25',
        ...
        'scope':'All 75 received Fields reports adjudicated exactly once. Proposed repairs retain all eight earlier Fields units and all original mathematical objects. They include a complete normal-decomposition proof and promote the transcendence-basis extension property already proved in the source. Further consequences are proved separately in the retained evidence. Source remains unchanged until separate admission and composition.',
        'registry_admission':False,'canonical_ids_allocated':False,'source_mutations':0,'build_claimed':False,
        'reviewer':'OpenAI Codex — GPT-6 Astra, Ultra effort','workers_used':False, 'worker_scope':'Primary session performed the complete Fields review and derivation',
    ...
    require(result['unit_count']==47 and result['operation_count']==56,'Completed review scope changed')
    ...
    target=source.HERE/'FIELDS_CORRECTION_BATCH_INPUT_20260924.json'
