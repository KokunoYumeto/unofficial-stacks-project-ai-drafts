...
from verify_fields_intake_review_20260925 import HERE, verify_proof_evidence, verify_structure


class ProofRetentionTests(unittest.TestCase):
    def setUp(self):
        self.review = json.loads((HERE / 'FIELDS_INTAKE_REVIEW_20260924.json').read_bytes())
        self.temp = tempfile.TemporaryDirectory(prefix='fields-proof-retention-')
    ...
    def test_changed_derivation_rejected(self):
        target = self.directory / 'FIELDS_NORMAL_DECOMPOSITION_PROOF_20260925.tex'
        target.write_bytes(target.read_bytes() + b'changed')
        with self.assertRaisesRegex(ValueError, 'evidence changed'):
            self.check()

    def test_omitted_source_proof_rejected_even_with_intact_receipts(self):
        next(g for g in self.review['groups'] if g['id'] == 'FIELDS-RECON-059')['operations'][-1]['replacement_text'] = 'Proof omitted.'
        with self.assertRaisesRegex(ValueError, 'checked complete proof'):
            self.check()

    def test_removed_required_proof_link_rejected(self):
        next(g for g in self.review['groups'] if g['id'] == 'FIELDS-RECON-059')['proof_evidence']['required_in_candidate'] = False
        with self.assertRaisesRegex(ValueError, 'complete proof link'):
            self.check()

    def test_dependency_cannot_drop_separability_transport_repair(self):
        self.review['dependency_groups'][0]['review_ids'].remove('FIELDS-RECON-034')
...
class ReferencePreservationTests(unittest.TestCase):
    def test_new_reference_change_rejected(self):
        ...
        op = {'review_id': 'FIELDS-RECON-059', 'line': 3679,
