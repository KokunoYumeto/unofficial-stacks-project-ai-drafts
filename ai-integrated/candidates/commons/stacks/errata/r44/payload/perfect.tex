...
every $\mathcal{O}_X$-module of finite presentation (automatically
...
\mathcal{L}^{\otimes n_{j, i}} \longrightarrow \mathcal{F}
$$
...
which contains the section $s_{j, i}$ over $V_j$.
Thus the resolution property of $Y$ produces surjections
$\mathcal{E}_{i, j} \to \mathcal{H}'_{i, j}$ and we conclude.
\end{proof}
...
$\mathcal{I}_{j'}|_{U_j}$ on the affine scheme $U_j$, see
...
$s'_{jk} : \mathcal{I}_j^{n_{jk}} \to \mathcal{F}$ for some $n_{jk} \geq 1$.
...
$$
is an exact functor of triangulated categories which factors through an
...
zeroth $K$-group of the abelian category
...
$$
c(\mathcal{E}^\bullet) = \sum (-1)^n[\mathcal{E}^n] =
...
Hence
$$
0 = c(C(a)^\bullet) = c(\mathcal{E}^\bullet) + c(\mathcal{G}^\bullet[1]) =
...
Then the existence of the distinguished triangles
...
exact sequence of $R^if_*$ on $Y$. If $Y$ is quasi-compact (the
...
$$
\Hom_{D(\mathcal{O}_X)}(K[-n], Rj_*(E|_U)) =
\Hom_{D(\mathcal{O}_U)}(K|_U[-n], E|_U) = 0
$$
...
Cohomology, Section \ref{cohomology-section-global-RHom}.
Parts (5) and (6) are equivalent as $H^i(X, -) = H^i(R\Gamma(X, -))$
...
The same argument works for all other properties, except that for properties
...
\item $D_\QCoh(X_{affine}, \mathcal{O})$, and
...
on $X$. Thus the result follows from the discussion in
Schemes, Section \ref{schemes-section-quasi-coherent}.
