"""Prepare the hash-bound two-source candidate input; no canonical IDs or edits."""
import argparse
...
def main():
    ...
    for name,expected in [('introduction.tex',1),('topology.tex',151)]:
        stem=name.removesuffix('.tex').upper()
        review_path=source.HERE/f'{stem}_INTAKE_REVIEW_20260922.json'
        replay_path=source.HERE/f'{stem}_INTAKE_REVIEW_REPLAY_20260922.json'
        ...
        for f in [review_path.name,replay_path.name,f'{stem}_PENDING_SOURCE_REPAIRS_20260922.patch',
                  f'{stem}_PENDING_CUMULATIVE_REPAIRS_20260922.patch']:
    ...
    for f in ['topology_intake_20260922.py','verify_topology_intake_review_20260922.py',
              'test_topology_intake_replay_20260922.py','prepare_topology_batch_20260922.py']:
        evidence.append({'path':f,**identity((source.HERE/f).read_bytes())})
    result={'schema':'stacks-next-correction-batch-input/v1','prepared_on':'2026-09-22',
        ...
        'scope':'All 151 received Topology reports and the one Introduction report adjudicated; 54 new proposed units / 64 operations, including one independently noticed Topology delimiter typo. Optional/rejected and already-composed reports remain in bound review evidence. No new theorem additions.',
        ...
        'next_action':'Allocate the next available two-source errata candidate after live registry/ref checks; complete source closure, bounded comparative deterministic builds under the TeX mutex, visual QA, separate executable replay/admission/composition, fixes-only regeneration and public readback. Do not rerun finished source adjudication.'}
    require(result['unit_count']==54 and result['operation_count']==64,'Unexpected batch scope')
    raw=(json.dumps(result,ensure_ascii=False,indent=2)+'\n').encode()
    target=source.HERE/'TOPOLOGY_INTRODUCTION_CORRECTION_BATCH_INPUT_20260922.json'
    if a.check:require(target.read_bytes()==raw,'Non-deterministic batch')
    else:target.write_bytes(raw)
    print(json.dumps({'path':str(target),**identity(raw),'units':54,'operations':64,
