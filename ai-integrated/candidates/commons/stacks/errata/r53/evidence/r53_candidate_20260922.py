"""R53 registrar reservation, then candidate-only two-source materialization/replay."""
import argparse
...
def main():
    ...
    raw = (CONTROL / 'TOPOLOGY_INTRODUCTION_CORRECTION_BATCH_INPUT_20260922.json').read_bytes()
    ...
    require(first == 1688 and len(batch['units']) == 54 and batch['operation_count'] == 64, 'Unexpected scope')
    sources = {s['source']: s for s in batch['sources']}
    require(set(sources) == {'introduction.tex', 'topology.tex'}, 'Unexpected sources')
    if a.action == 'lease':
        ...
        namespace = 'commons/stacks/errata/r53'
        ...
        event = {'event_id': f"lease-event-{len(leases['events'])+1:06d}", 'event': 'issued',
            'lease_id': f'stacks-lease-{number:06d}-errata-r53', 'namespace': namespace,
        ...
        require(number == 58 and event['event_id'] == 'lease-event-000112', 'Lease state advanced')
    ...
    if a.action == 'materialize':
        ...
        require(len(units) == 54 and len(operations) == 64, 'Materialized count drift')
        ...
        require(len(rejected) == 18, 'Rejected/optional coverage changed')
        ...
        write(candidate/'materialization.json', {'schema': 'stacks-r53-materialization/v1', 'candidate_id': 'stacks-errata-a04446e-r53',
    ...
    write(candidate/'replay/source-replay.json', {'schema': 'stacks-r53-independent-source-replay/v1', 'candidate_id': 'stacks-errata-a04446e-r53',
