...
U \times V$ and $(U \times V)\cap \Delta(X) = \emptyset$. In other words, we have
...
Let $f : X \to Y$ be a surjective, open, continuous map of topological spaces.
...
Let $f : X \to Y$ be a surjective, closed, continuous map of topological spaces.
Let $T \subset Y$ be a subset. Then
\begin{enumerate}
...
Let $A \subset f(E)$ be an open and closed subset of $f(E)$. Because $f$ is
...
By (1) we see $f^{-1}(\{t\})$ is connected, and we conclude that
...
each of its points, in other words $\mathcal{C}$ is open and (2) is proven.
\end{proof}
...
\begin{reference}
\cite[Chapter II, \S 1, no. 30, p. 223]{FAC}
\end{reference}
Let $X$ be a topological space.
\begin{enumerate}
...
\begin{lemma}
\label{lemma-irreducible-open-cover}
\begin{reference}
\cite[Chapter II, \S 1, no. 30, Proposition 3,
pp. 223--224]{FAC}
\end{reference}
Let $X$ be a topological space.
\begin{enumerate}
\item The space $X$ is irreducible if and only if $X$ is nonempty and
any two nonempty open subsets of $X$ have nonempty intersection.
\item Every nonempty open subset of an irreducible space is irreducible
and dense.
\item Suppose that
$$
X = U_1 \cup \ldots \cup U_n, \qquad n \geq 1,
$$
where the $U_i$ are nonempty open subsets. Then $X$ is irreducible if
and only if every $U_i$ is irreducible and
$U_i \cap U_j \not = \emptyset$ for all $i, j$.
\end{enumerate}
\end{lemma}

\begin{proof}
The first assertion follows by taking complements. If $X$ is irreducible
and $U \subset X$ is nonempty open, then any two nonempty open subsets of
$U$ are nonempty open subsets of $X$, hence meet. Thus $U$ is irreducible
by (1). Moreover, $U$ meets every nonempty open subset of $X$, so it is
dense. This proves (2).

If $X$ is irreducible, then (2) shows that every $U_i$ is irreducible,
and (1) shows that the $U_i$ pairwise intersect. Conversely, assume these
conditions. Let $V, W \subset X$ be nonempty open subsets. Choose $i, j$
such that $V \cap U_i$ and $W \cap U_j$ are nonempty. Since
$U_i \cap U_j$ is nonempty, irreducibility of $U_i$ shows that
$$
V \cap U_i \cap U_j \not = \emptyset.
$$
This is a nonempty open subset of $U_j$, and hence its intersection with
$W \cap U_j$ is nonempty by irreducibility of $U_j$. Thus
$V \cap W \not = \emptyset$, and $X$ is irreducible by (1).
\end{proof}
...
\begin{reference}
\cite[Chapter II, \S 1, no. 30, Proposition 2, p. 223]{FAC}
\end{reference}
...
\medskip\noindent
Conversely, take one of the $X_i$. By Lemma \ref{lemma-irreducible} it is
...
$f(U_1) \cap f(U_2) \not = \emptyset$. By (c) there is a point $y$ in this intersection
such that the fibre
...
$U'$ forms a topology on $X'$ and that we have a bijection as
stated in the lemma.
...
irreducible components $X_1, \ldots, X_n$. If $n > 1$ there is an index $j$ with
...
\cite[Chapter II, \S 1, no. 30, p. 223]{FAC}
\end{reference}
A topological space $X$ is called {\it Noetherian}
...
\begin{reference}
\cite[Chapter II, \S 1, no. 30, Propositions 1(b) and 2,
p. 223]{FAC}
\end{reference}
...
Because $X = Z \cup Z_1 \cup \ldots \cup Z_n$ (see Lemma \ref{lemma-irreducible}),
...
\begin{reference}
\cite[Chapter II, \S 1, no. 30, Proposition 1(c), p. 223]{FAC}
\end{reference}
Let $X$ be a topological space.
...
\begin{proof}
Let $\{F_m\}_{m \in \mathbf{N}}$ be a decreasing sequence of closed subsets
...
$\{G_m \cap X_i\}_{m \in \mathbf{N}}$ is a decreasing sequence
...
\begin{proof}
Suppose that $X$ is catenary. According to
...
$$
is maximal too (between $Y$ and $Y''$) and its length equals $e_1 + e_2$.
As $X$ is catenary, each maximal chain has the same length, equal to the
codimension. Thus the point (2) that
$\text{codim}(Y,Y'') = e_1 + e_2 = \text{codim}(Y,Y') + \text{codim}(Y',Y'')$
is verified.

\medskip\noindent
For the converse, we show by induction that if $Y = Y_1 \subset ... \subset
...
then there exist open subsets $U_i$ with $E_i \subset U_i$ and $U_1 \cap U_2 = \emptyset$.
\end{enumerate}
\end{lemma}
...
The complement is then empty, which means that
...
\begin{reference}
\cite[Chapter II, \S 1, no. 30, Proposition 1(a), p. 223]{FAC}
\end{reference}
Let $X$ be a Noetherian topological space.
\begin{enumerate}
...
A topological space $X$ is called {\it paracompact} if it is Hausdorff
and every open covering of $X$ has a locally finite open refinement.
Here locally finite has the meaning of
Definition \ref{definition-locally-finite}.
\end{definition}

\begin{lemma}
\label{lemma-paracompact-shrinking}
Let $X$ be a paracompact topological space and let
$X = \bigcup_{i \in I} U_i$ be an open covering. There exist
\begin{enumerate}
\item a locally finite open covering $X = \bigcup_{j \in J} V_j$,
\item a map $\alpha : J \to I$, and
\item an open covering $X = \bigcup_{j \in J} W_j$
\end{enumerate}
such that
$$
\overline{W_j} \subset V_j \subset U_{\alpha(j)}
$$
for every $j \in J$.
\end{lemma}

\begin{proof}
We first make two elementary observations about locally finite families.
If $\{E_j\}$ is locally finite, then $\{\overline{E_j}\}$ is locally
finite: an open set meeting $\overline{E_j}$ also meets $E_j$. Moreover,
the union of any subfamily of the closed sets $\overline{E_j}$ is closed.
Indeed, near a point only finitely many of them have to be avoided.

We next show that if $x \in U \subset X$ with $U$ open, then there is an
open $V$ such that
$$
x \in V \subset \overline{V} \subset U.
$$
For every $y \in X \setminus U$, choose disjoint open neighbourhoods
$A_y$ of $x$ and $B_y$ of $y$. Choose a locally finite refinement
$X = \bigcup_{j \in J} E_j$ of the open covering consisting of $U$ and
the opens $B_y$. Let $J' \subset J$ be the set of $j$ such that
$E_j \cap (X \setminus U)$ is nonempty. Such an $E_j$ is contained in
some $B_y$, and hence $x \notin \overline{E_j}$. Thus
$$
Z = \bigcup_{j \in J'} \overline{E_j}
$$
is closed and does not contain $x$. Set $V = X \setminus Z$. Every point
of $X \setminus U$ has an open neighbourhood $E_j$ contained in $Z$, so
no such point belongs to $\overline{V}$. This proves the claim.

Choose a locally finite refinement $X = \bigcup_{j \in J} V_j$ of the
given covering and a map $\alpha : J \to I$ such that
$V_j \subset U_{\alpha(j)}$. For every $x \in X$, choose $j(x) \in J$
with $x \in V_{j(x)}$. By the preceding claim there is an open $A_x$
such that
$$
x \in A_x \subset \overline{A_x} \subset V_{j(x)}.
$$
Choose a locally finite open refinement $X = \bigcup_{k \in K} B_k$
of the covering $\{A_x\}_{x \in X}$ and choose $x(k)$ such that
$B_k \subset A_{x(k)}$. For $j \in J$, set
$$
W_j = \bigcup_{j(x(k)) = j} B_k.
$$
The family $\{W_j\}$ is a locally finite open covering. By the elementary
observations at the start of the proof,
$$
\overline{W_j} \subset
\bigcup_{j(x(k)) = j} \overline{B_k} \subset V_j.
$$
This finishes the proof.
\end{proof}

\begin{lemma}
\label{lemma-paracompact-refine-covering}
Let $X$ be a paracompact topological space. Let
$X = \bigcup_{i \in I} U_i$ be an open covering. Suppose given an integer
$p \geq 0$ and, for every $(p + 1)$-tuple $i_0, \ldots, i_p$ of elements
of $I$, an open covering
$$
U_{i_0} \cap \ldots \cap U_{i_p}
= \bigcup_k T_{i_0 \ldots i_p, k}.
$$
Then there exist an open covering $X = \bigcup_{x \in X} N_x$ and a map
$\gamma : X \to I$ such that $N_x \subset U_{\gamma(x)}$ and every
nonempty intersection
$$
N_{x_0} \cap \ldots \cap N_{x_p}
$$
is contained in some
$T_{\gamma(x_0) \ldots \gamma(x_p), k}$.
\end{lemma}

\begin{proof}
By Lemma \ref{lemma-paracompact-shrinking}, there exist a locally finite
open covering $X = \bigcup_{j \in J} V_j$, a map $\alpha : J \to I$,
and an open covering $X = \bigcup_{j \in J} W_j$ such that
$$
\overline{W_j} \subset V_j \subset U_{\alpha(j)}.
$$
For every $x \in X$, choose an open neighbourhood $N_x$ of $x$ with the
following properties:
\begin{enumerate}
\item if $x \in V_j$, then $N_x \subset V_j$, and if $x \in W_j$, then
$N_x \subset W_j$;
\item if $N_x \cap W_j$ is nonempty, then $N_x \subset V_j$;
\item if $x \in V_{j_0} \cap \ldots \cap V_{j_p}$, then
$N_x \subset T_{\alpha(j_0) \ldots \alpha(j_p), k}$ for some $k$.
\end{enumerate}
Such a choice is possible because the families $\{V_j\}$ and $\{W_j\}$
are locally finite. For (2), if $x \in \overline{W_j}$, then
$x \in V_j$ and (1) applies; otherwise we may shrink $N_x$ to avoid
$W_j$. For (3), only finitely many $(p + 1)$-tuples occur near $x$.

Choose $\tau(x) \in J$ such that $x \in W_{\tau(x)}$ and set
$\gamma(x) = \alpha(\tau(x))$. By (1),
$$
N_x \subset W_{\tau(x)} \subset V_{\tau(x)}
\subset U_{\gamma(x)}.
$$
Suppose $N_{x_0} \cap \ldots \cap N_{x_p}$ is nonempty. For every
$a = 0, \ldots, p$, the set $N_{x_0}$ meets $W_{\tau(x_a)}$ because
$N_{x_a} \subset W_{\tau(x_a)}$. By (2),
$N_{x_0} \subset V_{\tau(x_a)}$. Hence (3), applied at $x_0$, shows that
$N_{x_0}$, and therefore the displayed intersection, is contained in
some $T_{\gamma(x_0) \ldots \gamma(x_p), k}$.
\end{proof}
...
$V_i = \bigcup_{j : i(x_j) = i} U_{x_j}$ the proof is finished.
\end{proof}
...
satisfy $V_{j_0, k} \cap V_{j_1} \cap \ldots \cap V_{j_p} \subset
...
Since $z$ and $i$ were arbitrary and since $Z$ is quasi-compact
...
with the induced topology. A basis for the topology of $\prod X_i$ is given by
...
we can pick an $i \in \Ob(\mathcal{I})$ and morphisms
...
we can choose a point $x_i \in X_i$ which is not in
...
of the corresponding morphism $X_j \to X_k$. Hence by
Lemma \ref{lemma-intersection-closed-in-quasi-compact}
...
set and the terminology ``constructible'' was used for what we call a locally
constructible set.}
...
(a) contains a nonempty open $V$ of $Y$ or (b) is not dense in $Y$.
...
open in $X \times Y$. Then there exist opens $A \subset U \subset X$
...
$$
Thus for every $b \in B$ there exist opens $U_b \subset X$ and
...
Assume $X_0$ is finite and $X$ is Jacobson.
...
$\overline{\{y\}} \subset \overline{F} = F$ as $F$ is closed. Thus for every
...
\begin{proof}
Let $z'\leadsto z$ be a specialization in $Z$ and let $x' \in X$ such that
...
other words, specializations lift along $g\circ f$.
\end{proof}
...
$x'\in T$ such that $f(x') = y'$. Because specializations lift along $f$, there
...
Let $Z_0 \subset Z_1 \subset \ldots \subset Z_e \subset Y$ be a chain
...
of $T$ is nowhere dense in $X$ by Lemma \ref{lemma-nowhere-dense}.
...
a point specializing to both $x$ and $y$, or there exist disjoint
...
\medskip\noindent
A basis of the topology of $X \times Y$ is given by the opens of the form
...
the intersection of two quasi-compact opens of $X'$ is quasi-compact.
...
cofiltered we may choose an object $k$ of $\mathcal{I}$ and morphisms
...
the topology of $X_i$ is given by the quasi-compact opens, we conclude
...
$$
and denote by $\beta(X)$ the closure of the image. Since each $Y_i$ is
...
Write $x = f(y)$ (possible as $f$ is surjective). If $y \in U \cap f^{-1}(V)$
...
Thus $\text{id}_E$ factors through the extremally disconnected
...
cardinal greater than or equal to the cardinality of $X$. Then the cardinality
...
(because products commute with products in any category), we
...
of $e$ such that $f(gw) = t$ for all $g \in U_t$ and $w \in W$.
