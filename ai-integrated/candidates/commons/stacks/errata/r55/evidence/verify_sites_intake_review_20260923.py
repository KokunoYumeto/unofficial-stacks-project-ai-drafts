"""Exact proposal replay for Sites and Sheaves; no source/registry writes."""
import argparse
...
import sites_intake_20260923 as source
...
EXPECTED_REFERENCE_CHANGES = [
    ('SITES-RECON-148', 11484, r'\ref{lemma-sieves-set}', r'\ref{lemma-topology-basic}'),
    ('SITES-RECON-148', 11574, r'\ref{lemma-sieves-set}', r'\ref{lemma-topology-basic}'),
]
...
def verify_one(name, count, existing_count, model):
    ...
    retained_units = json.loads((HERE / 'SITES_PRIOR_ADMITTED_UNITS_20260923.json').read_bytes())
    ...
    require([g['id'] for g in review['groups']] ==
            [f'SITES-RECON-{n:03d}' for n in range(1,len(review['groups'])+1)], 'Group IDs differ')
    ...
    receipt={'schema':'stacks-source-intake-review-replay/v1','source':name,'review':review_id,
        ...
        'reference_key_change_policy':'Exactly two source-bound covering-sieve corrections; all other references retained',
...
def main():
    ...
    if a.candidate_head:
        ...
        for path in git('diff-tree','-r','--no-commit-id','--name-only',CURRENT,head).decode().splitlines():
            require(path=='ai-integrated/registry/leases.json' or path.startswith('ai-integrated/candidates/commons/stacks/errata/r55/'),'Not a candidate-only descendant: '+path)
    else:
        require(head==CURRENT,'HEAD advanced; rebind explicitly')
    model=intake.import_model()
    summaries=[]
    for name,count,existing in [('sites.tex',246,107)]:
