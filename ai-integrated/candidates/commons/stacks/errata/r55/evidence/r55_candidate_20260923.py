"""R55 registrar reservation, then candidate-only single-source materialization/replay."""
import argparse
...
PRIOR = '35a01332d8714a2012c004bb161cf38f047c0119'
WRITER = '01a0cc46-4ba8-77b1-9c55-33449e55fea7'
PREFIX = 'ai-integrated/candidates/commons/stacks/errata/r55'
BATCH_HASH = 'C52EC10CD249E27205046ABA3DF3E4D00199CA997FF48C6E3787492B15B793F8'
...
def main():
    ...
    raw = (CONTROL / 'SITES_CORRECTION_BATCH_INPUT_20260923.json').read_bytes()
    ...
    require(first == 1754 and len(batch['units']) == 29 and batch['operation_count'] == 35, 'Unexpected scope')
    sources = {s['source']: s for s in batch['sources']}
    require(set(sources) == {'sites.tex'}, 'Unexpected sources')
    if a.action == 'lease':
        ...
        namespace = 'commons/stacks/errata/r55'
        ...
        event = {'event_id': f"lease-event-{len(leases['events'])+1:06d}", 'event': 'issued',
            'lease_id': f'stacks-lease-{number:06d}-errata-r55', 'namespace': namespace,
        ...
        require(number == 61 and event['event_id'] == 'lease-event-000118', 'Lease state advanced')
    ...
    if a.action in ('materialize', 'revise'):
        ...
        for name in ('r55_candidate_20260923.py', 'r51_candidate_20260922.py'):
            write(candidate/'evidence'/name, (CONTROL/name).read_bytes())
        if a.action == 'revise':
            for failure in sorted(CONTROL.glob('R55_*BUILD_REGRESSION_20260923.json')):
        ...
        for i, u in enumerate(batch['units']):
            ...
            provenance = {'discovery': u.get('discovery'), 'related_existing_canonical_ids': u.get('related_existing_canonical_ids', [])}
            ...
            formula.append({'unit_id': sid, 'class': kind, 'operation_ids': operation_ids,
                'formula_policy': 'Only listed tokens change. Labels, citation keys and environments remain invariant; SITES-RECON-148 changes exactly two covering-sieve references at authority lines 11484 and 11574, while every other reference is retained.'})
        require(len(units) == 29 and len(operations) == 35, 'Materialized count drift')
        ...
        require(len(rejected) == 22, 'Rejected/optional coverage changed')
        ...
        write(candidate/'materialization.json', {'schema': 'stacks-r55-materialization/v1', 'candidate_id': 'stacks-errata-a04446e-r55',
        ...
        text = ['# Sites and Sheaves: proposed corrections and clarifications\n',
    ...
    for name, source in sources.items():
        ...
        from verify_sites_intake_review_20260923 import verify_structure
    ...
    write(candidate/'replay/source-replay.json', {'schema': 'stacks-r55-independent-source-replay/v1', 'candidate_id': 'stacks-errata-a04446e-r55',
