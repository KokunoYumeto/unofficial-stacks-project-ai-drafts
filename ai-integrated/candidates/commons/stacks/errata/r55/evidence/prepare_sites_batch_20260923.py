...
def main():
    ...
    for name,expected in [('sites.tex',246)]:
    ...
    for f in ['SITES_PRIOR_ADMITTED_UNITS_20260923.json','sites_intake_20260923.py','verify_sites_intake_review_20260923.py',
              'test_sites_intake_replay_20260923.py','prepare_sites_batch_20260923.py']:
        evidence.append({'path':f,**identity((source.HERE/f).read_bytes())})
    result={'schema':'stacks-next-correction-batch-input/v1','prepared_on':'2026-09-23',
        ...
        'scope':'All 246 received Sites and Sheaves reports adjudicated; 29 new proposed units / 35 operations: 22 copyedits, five source corrections and two clarifications. All 107 prior units are retained, including the complete cumulative text; exactly two covering-sieve reference targets are corrected. Optional/rejected and already-composed reports remain in bound review evidence. No new theorem additions.',
    ...
    require(result['unit_count']==29 and result['operation_count']==35,'Unexpected batch scope')
    raw=(json.dumps(result,ensure_ascii=False,indent=2)+'\n').encode()
    target=source.HERE/'SITES_CORRECTION_BATCH_INPUT_20260923.json'
    if a.check:require(target.read_bytes()==raw,'Non-deterministic batch')
    else:target.write_bytes(raw)
    print(json.dumps({'path':str(target),**identity(raw),'units':29,'operations':35,
