"""Collect completed R55 proof evidence and seal the candidate; no registry admission."""
import argparse
...
BUILD_DIRS={'sites':'r55-build-sites-20260923'}
...
def main():
    ...
    reproduction_script=(root/'ai-integrated/candidates/commons/stacks/errata/r51/proof-source/reproduce.py').read_text(encoding='utf-8').replace('R51','R55').replace("choices=['sets','schemes','homology','algebra']", "choices=['sites']")
    ...
    write(source/'README.md','# Exact R55 chapter-proof sources\n\nThese are isolated cumulative-draft chapter proofs before and after 35 edits representing 29 proposed changes. External chapter auxiliaries are intentionally absent in both versions; unchanged external references appear as ?? in the proofs. This is not a replacement for the complete Stacks reader.\n\nUse Python with the bundled helper scripts and a TeX installation with the Stacks packages (including Latin Modern and XY-pic). Run `python reproduce.py --role successor --chapter sites --output /path/to/new-build`; replace the role or chapter as desired. On Windows the script holds Global\\InterlanguageTeXSlotV1 for the full captured build. It fails closed if the slot is unavailable. The exact PDFs were obtained with MiKTeX/pdfTeX; software/font differences may prevent byte-identical reproduction elsewhere. inputs.json records source and target hashes.\n\nModified Stacks source retains GNU FDL 1.2; COPYING is included. AI corrections, packaging and checks: OpenAI Codex — GPT-6 Astra, Ultra effort. No human review or official endorsement is claimed.\n')
    ...
    with zipfile.ZipFile(archive,'w',compression=zipfile.ZIP_STORED) as z:
        for item in [*files,{'path':'inputs.json'}]:
            info=zipfile.ZipInfo(item['path'],(2026,9,23,0,0,0));info.create_system=3;info.external_attr=0o100644<<16
    ...
    inspected=json.loads((CONTROL/'R55_ACTUAL_VISUAL_REVIEW_20260923.json').read_bytes())
    for chapter in ('sites',):
    ...
    for chapter in ('sites',):
        reproduction=(CONTROL/f'r55-proof-reproduction-{chapter}-20260923/REPRODUCTION.json').read_bytes()
        assert json.loads(reproduction)['passed']
        write(c/f'replay/package-reproduction-{chapter}.json',reproduction)
    review={'candidate_id':'stacks-errata-a04446e-r55','passed':True,'review_state':'performed','independent_replay':'passed',
        'unresolved_defects':[],'checks':[
            {'name':'Frozen-official and cumulative preimage replay, unchanged labels/citations/environments, two exact reference corrections and all other references retained','passed':True,'receipt':'replay/source-replay.json'},
            {'name':'Prior/successor Sites builds; two fresh successor identities; no new diagnostics','passed':True},
    ...
    text=['# Sites and Sheaves: corrections and clarification\n',
        'Twenty-nine proposed changes, consisting of 35 exact edits: 22 copyedits, five source corrections and two clarifications. The source corrections repair cohomology-set terminology, a missing sheafification sign, a presheaf target, a transition-map index and two covering-sieve references. The clarifications specify composition grouping and an element-to-stalk assignment. All 107 earlier corrections remain intact. No new theorems are included. [Read each correction and its reason](REVIEW.md).\n',
    ...
    text.append('\nUse the corrected PDF and its direct LaTeX together with the ZIP dependencies. The archive contains both versions of the Sites and Sheaves chapter, project preamble/class/chapter-list/bibliography, licence and guarded reproduction instructions. [Source instructions](proof-source/README.md).\n')
    write(c/'README.md','\n'.join(text))
    ...
    authorities=['authority/'+x+'.tex' for x in ('sites',)]
    ...
    manifest={'schema':'mathematics-commons-stacks-candidate-manifest/v1','candidate_id':'stacks-errata-a04446e-r55',
        ...
        'source_closure':{'enumerated':True,'expected_units':29,'manifested_units':29,'complete':True},
