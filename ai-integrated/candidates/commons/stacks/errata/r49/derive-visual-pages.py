...
def main() -> int:
    global STEM, SEALED_PDF, OUTPUT
    parser = argparse.ArgumentParser()
    parser.add_argument("--stem", required=True, choices=tuple(CONFIG["stems"]))
    parser.add_argument("--synctex-work-root", type=Path, required=True)
    args = parser.parse_args()
    STEM = args.stem
    SEALED_PDF = ROOT / "builds" / f"{STEM}.pdf"
    OUTPUT = ROOT / "builds" / f"source-page-map-{STEM}.json"
    ...
    rows = []
    selected_operations = [op for op in spec["operations"] if op["source"] == f"{STEM}.tex"]
    for operation in selected_operations:
        pages: set[int] = set()
        queried_lines = sorted({int(operation["payload_start_line"]), int(operation["payload_end_line"])})
    ...
    assert len(rows) == len(selected_operations) and unique_pages
    document = {
