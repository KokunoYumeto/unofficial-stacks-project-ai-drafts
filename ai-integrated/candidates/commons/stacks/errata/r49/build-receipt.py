...
def validate_mutex() -> dict:
    ...
    guard = receipt["captured_tree_guard"]
    assert guard["path"] == "CapturedR49Tree20260913.cs"
    assert guard["sha256"] == sha256(ROOT / guard["path"])
    assert guard["breakaway_allowed"] is False
    ...
    assert all(
        row["assigned_before_resume"] is True
        and row["cleanup_completed"] is True
        and row["captured_total_processes"] >= 1
        and row["termination_requested"] is False
        and row["failure"] is None
        for row in receipt["guarded_commands"]
    )
...
def validate() -> dict:
    ...
    receipt = {
        ...
        "expected_limitation": "Standalone chapter builds may retain unresolved cross-chapter references because the cumulative AUX set is absent. Recorder inventories cover each phase's final TeX pass in both executions; they do not prove all earlier reads or a complete BibTeX dependency closure.",
        "passed": True,
        "chapters": [],
    }
    for stem, expected in CONFIG["stems"].items():
        ...
        reference_deltas = build_exceptions.get("candidate_undefined_reference_target_deltas", {})
        for target, delta in reference_deltas.items():
            expected_candidate_refs[target] += int(delta)
            assert expected_candidate_refs[target] >= 0
            if expected_candidate_refs[target] == 0:
                del expected_candidate_refs[target]
        warnings_match = (
