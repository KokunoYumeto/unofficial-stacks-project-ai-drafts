...
if ($config.candidate_id -ne 'stacks-errata-a04446e-r49') { throw 'Unexpected candidate identity.' }
...
$guardSource = Join-Path $root 'CapturedR49Tree20260913.cs'
if ('CapturedR49Tree20260913' -as [type]) { throw 'Use a fresh PowerShell process so the guard source cannot be shadowed by a loaded type.' }
...
$receipt = [ordered]@{
    schema='stacks-r49-tex-mutex-execution/v1'; candidate_id=$config.candidate_id
    ...
    captured_tree_guard=[ordered]@{path='CapturedR49Tree20260913.cs'; sha256=(Get-FileHash -LiteralPath $guardSource -Algorithm SHA256).Hash; mechanism='CREATE_SUSPENDED -> unnamed KILL_ON_JOB_CLOSE Job Object -> resume'; breakaway_allowed=$false}
...
function Invoke-CapturedPython {
    ...
    try {
        $script:activeTree=[CapturedR49Tree20260913]::new($pythonExe, [string[]](@('-B') + $Arguments), $root, $outPath, $errPath)
