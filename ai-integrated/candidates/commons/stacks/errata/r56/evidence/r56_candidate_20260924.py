"""R56 registrar reservation, then candidate-only single-source materialization/replay."""
import argparse
...
BATCH_HASH = 'F9C3D7E6DBA9CEFD63C5A36267F0906200FB13C34CF657EEB759CDE9BD97604F'
...
def main():
    ...
    raw = (CONTROL / 'STACKS_CORRECTION_BATCH_INPUT_20260923.json').read_bytes()
    ...
    review = json.loads((CONTROL/'STACKS_INTAKE_REVIEW_20260923.json').read_bytes())
    ...
    require(first == 1783 and len(batch['units']) == 76 and batch['operation_count'] == 86, 'Unexpected scope')
    sources = {s['source']: s for s in batch['sources']}
    require(set(sources) == {'stacks.tex'}, 'Unexpected sources')
    if a.action == 'lease':
        ...
        namespace = 'commons/stacks/errata/r56'
        ...
        event = {'event_id': f"lease-event-{len(leases['events'])+1:06d}", 'event': 'issued',
            'lease_id': f'stacks-lease-{number:06d}-errata-r56', 'namespace': namespace,
        ...
        require(number == 62 and event['event_id'] == 'lease-event-000120', 'Lease state advanced')
    ...
    if a.action in ('materialize', 'revise'):
        ...
        for name in ('r56_candidate_20260924.py', 'r51_candidate_20260922.py'):
            write(candidate/'evidence'/name, (CONTROL/name).read_bytes())
        if a.action == 'revise':
            for failure in sorted(CONTROL.glob('R56_*BUILD_REGRESSION_20260923.json')):
        ...
        for i, u in enumerate(batch['units']):
            ...
            formula.append({'unit_id': sid, 'class': kind, 'operation_ids': operation_ids,
                'formula_policy': 'Only the exact listed source spans change. Original mathematical data, labels, reference targets, citation keys and environments are retained. The corrected localization criterion includes its full proof; mathematical dependency groups and complete proof notes are mandatory.'})
        require(len(units) == 76 and len(operations) == 86, 'Materialized count drift')
        ...
        require(len(rejected) == 1, 'Rejected/optional coverage changed')
        ...
        write(candidate/'materialization.json', {'schema': 'stacks-r56-materialization/v1', 'candidate_id': 'stacks-errata-a04446e-r56',
    ...
    verify_proof_evidence(json.loads((candidate/'evidence/STACKS_INTAKE_REVIEW_20260923.json').read_bytes()), candidate/'evidence')
    ...
    for name, source in sources.items():
        ...
        from verify_stacks_intake_review_20260924 import verify_structure
    ...
    write(candidate/'replay/source-replay.json', {'schema': 'stacks-r56-independent-source-replay/v1', 'candidate_id': 'stacks-errata-a04446e-r56',
