"""Exact Stacks chapter proposal replay; no source or registry writes."""
import argparse
...
REQUIRED_PROOFS = {
    'SUBSTACK_VERTICAL_DERIVATION_20260923.md',
    'SUBSTACK_VERTICAL_PRIMARY_REVIEW_20260924.json',
    'LOCALIZED_CARTESIAN_DERIVATION_20260924.md',
    'LOCALIZED_CARTESIAN_PRIMARY_REVIEW_20260924.json',
    'STACKS_LOCALIZED_CARTESIAN_PROOF_20260924.tex',
...
def verify_proof_evidence(review, directory=HERE):
    require(review.get('mathematical_review_complete') is True,
            'Independent mathematical repairs are not yet finalized')
    rows = review.get('required_proof_evidence', [])
    names = [row['path'] for row in rows]
    require(len(names) == len(set(names)) and REQUIRED_PROOFS <= set(names),
    ...
    groups = {group['id']: group for group in review['groups']}
    for gid, filename in [('STACKS-RECON-066', 'SUBSTACK_VERTICAL_DERIVATION_20260923.md'),
                          ('STACKS-RECON-077', 'LOCALIZED_CARTESIAN_DERIVATION_20260924.md')]:
        link = groups[gid].get('proof_evidence', {})
        require(link.get('file') == filename and link.get('required_in_candidate') is True,
                'Mathematical correction lost its complete proof link')
    fragment = (directory / 'STACKS_LOCALIZED_CARTESIAN_PROOF_20260924.tex').read_text(encoding='utf-8').rstrip('\n')
    targets = [op for op in groups['STACKS-RECON-077']['operations'] if op['line'] == 3124]
    ...
    for receipt_name in ['SUBSTACK_VERTICAL_PRIMARY_REVIEW_20260924.json',
                         'LOCALIZED_CARTESIAN_PRIMARY_REVIEW_20260924.json']:
        receipt = json.loads((directory / receipt_name).read_bytes())
        proof = receipt['proof']
        require(proof in [{k: row[k] for k in ('path', 'bytes', 'sha256')} for row in rows],
                'Primary review does not bind the retained derivation')
    dependencies = review.get('dependency_groups', [])
    expected = {
        'substack-fibre-condition': {'STACKS-RECON-006', 'STACKS-RECON-066'},
        'localized-cartesian-criterion': {
            f'STACKS-RECON-{n:03d}' for n in list(range(44, 66)) + list(range(67, 72)) + [77]
        },
    }
    require(len(dependencies) == len(expected), 'Mathematical dependency groups are missing')
    require({d['id']: set(d['review_ids']) for d in dependencies} == expected,
...
def verify_structure(before, after, operations):
    ...
    signature = sorted((op['review_id'], op['line'], op['old_text'], op['replacement_text'])
                       for op in changed)
    require(signature == EXPECTED_REFERENCE_CHANGES, 'Unexpected reference-key exception')
...
def verify_one(name, count, existing_count, model):
    ...
    review, review_id = intake.loaded(HERE / f'{stem}_INTAKE_REVIEW_20260923.json')
    ...
    retained_units = json.loads((HERE / 'STACKS_PRIOR_ADMITTED_UNITS_20260923.json').read_bytes())
    ...
    require([g['id'] for g in review['groups']] ==
            [f'STACKS-RECON-{n:03d}' for n in range(1,len(review['groups'])+1)], 'Group IDs differ')
    ...
    paths={f'{stem}_PENDING_SOURCE_REPAIRS_20260923.patch':patch,
           f'{stem}_PENDING_CUMULATIVE_REPAIRS_20260923.patch':cumulative_patch}
    receipt={'schema':'stacks-source-intake-review-replay/v1','source':name,'review':review_id,
        ...
        'labels_citation_keys_environments':'UNCHANGED',
        'reference_key_changes':reference_changes,
        'reference_key_change_policy':'All original reference targets, labels and citation keys retained',
    ...
    paths[f'{stem}_INTAKE_REVIEW_REPLAY_20260923.json']=(json.dumps(receipt,indent=2,ensure_ascii=False)+'\n').encode()
...
def main():
    ...
    if a.candidate_head:
        ...
        for path in git('diff-tree','-r','--no-commit-id','--name-only',CURRENT,head).decode().splitlines():
            require(path=='ai-integrated/registry/leases.json' or path.startswith('ai-integrated/candidates/commons/stacks/errata/r56/'),'Not a candidate-only descendant: '+path)
    else:
        require(head==CURRENT,'HEAD advanced; rebind explicitly')
    model=intake.import_model()
    summaries=[]
    for name,count,existing in [('stacks.tex',114,0)]:
