"""Validate R52 and all unchanged earlier release gates at their exact checkpoints."""
import argparse
import json
import os
import re
import textwrap
try:
    from . import r52_successor as r
    from . import validate_r51_successor as prior
except ImportError:
    import r52_successor as r
    import validate_r51_successor as prior

CURRENT = 'validation/r52-successor-current.json'
...
def validate(ref='HEAD', verify_frozen_core=False):
    ...
    r.require(issued['event_id'] == 'lease-event-000110' and issued['state'] == 'active' and released['event_id'] == 'lease-event-000111'
    ...
    r.require(exp['historical_registry_ids'] == 1423 and exp['included_effective_textual_units'] == 1421
    ...
    allowed = names | export | {r.OVERLAYS,r.LEASES,r.ADMISSION,r.COMPOSITION,CURRENT,
        'categories.tex','README.md','ai-integrated/README.md','tools/r52_successor.py',
        'tools/validate_r52_successor.py','tools/generate_changes_from_upstream.py',
        'tests/test_legacy_context_replay.py','tests/test_r52_successor.py','tests/test_changes_from_upstream.py','.github/workflows/validate.yml',
    ...
    try:
        os.chdir(r.ROOT/'ai-integrated'); exec(compile(code,'r52-full-registry-contract','exec'),{'__name__':'__main__'})
    finally: os.chdir(cwd)
    return {'status':'R52_CUMULATIVE_SUCCESSOR_PASS','units':83,'operations':107,'effective_export_units':1421,
