...
def historical_inputs(checkpoint, scope, raw_loader):
    """Project sealed734prefixes without suppressing successor ledger rows.
    ...
    the successor contract and are not mistaken for734's original frontier.
    """
    if digest(canonical(checkpoint)) != CONTRACT_SHA256:
        raise ValueError("I7.3.4 historical projection requires the sealed receipt")
    historical_raw, tables = {}, {}
    for ledger in checkpoint["ledgers"]:
        path = ledger["path"]
        raw = raw_loader(path)
        if not isinstance(raw, bytes) or b"\r" in raw or not raw.endswith(b"\n"):
            raise ValueError("I7.3.4 invalid live ledger serialization: " + path)
        lines = [line + b"\n" for line in raw.split(b"\n")[:-1]]
        frozen = b"".join(lines[:ledger["final_rows"] + 1])
        if len(frozen) != ledger["bytes"] or digest(frozen) != ledger["sha256"]:
            raise ValueError("I7.3.4 historical ledger prefix changed: " + path)
