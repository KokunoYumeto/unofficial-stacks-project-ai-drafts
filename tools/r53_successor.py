"""Separate, hash-bound R53 candidate admission and cumulative composition."""
import argparse
...
CANDIDATE='3770215f2e07fb918bc7acd0771852a964771d17'
MANIFEST='81AFF5CD86F3483165D1C654AB0EC666E75638718CA3ECEB73942E4EB796494B'
PREFIX='ai-integrated/candidates/commons/stacks/errata/r53/'
ID='stacks-errata-a04446e-r53'
OVERLAYS=common.OVERLAYS
LEASES=common.LEASES
ADMISSION='validation/r53-admission-2026-09-23.json'
COMPOSITION='validation/r53-source-composition-2026-09-23.json'
...
def evidence():
    ...
    require(ids==[f'MC-STK-ERR-{i}' for i in range(1688,1742)],'Unit scope drift')
    operations=json.loads(read('operation-spec.json'))['operations'];require(len(operations)==64,'Operation scope drift')
...
def main():
    ...
    if a.action=='check':print(json.dumps({'passed':True,'files':len(names),'units':len(ids),'operations':64}));return
    ...
    if a.action=='admit':
        ...
        require(issued['event_id']=='lease-event-000114' and issued['state']=='active' and issued['lease_id']==m['lease_id'],'Lease changed')
        release={**issued,'event_id':'lease-event-000115','event':'released','state':'released','issued_at_utc':now(),'supersedes_event_id':issued['event_id']}
        ...
        write(ROOT/ADMISSION,{'schema':'stacks-r53-separate-admission/v1','passed':True,'previous_public':PRIOR,'candidate_commit':CANDIDATE,
        ...
        reg=json.loads((ROOT/OVERLAYS).read_bytes());require(reg['registered_entries'][-1]['id']==ID,'R53 not admitted')
        ...
        write(ROOT/COMPOSITION,{'schema':'stacks-r53-exact-source-composition/v1','passed':True,'previous_public':PRIOR,
            'candidate_commit':CANDIDATE,'manifest_sha256':MANIFEST,'admission_commit':head,'sources':sources,
            'operations':64,'units':54,'whole_isolated_payload_copied':False,'only_admitted_operations_applied':True,
    ...
    print(json.dumps({'action':a.action,'passed':True,'units':54,'operations':64}))
