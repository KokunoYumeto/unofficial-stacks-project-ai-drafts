"""Separate, hash-bound R57 candidate admission and cumulative composition."""
import argparse
...
CANDIDATE='c1ff53e21956a95cd0f6b4c3e7e29a8f4c66acd4'
MANIFEST='DB3D26E746FD92D2974F88025FE703F2E7783F7A89D20F8B3E9A1143BCC36193'
PREFIX='ai-integrated/candidates/commons/stacks/errata/r57/'
ID='stacks-errata-a04446e-r57'
OVERLAYS=common.OVERLAYS
LEASES=common.LEASES
ADMISSION='validation/r57-admission-2026-09-25.json'
COMPOSITION='validation/r57-source-composition-2026-09-25.json'
...
def evidence():
    ...
    require(ids==[f'MC-STK-ERR-{i}' for i in range(1859,1906)],'Unit scope drift')
    operations=json.loads(read('operation-spec.json'))['operations'];require(len(operations)==56,'Operation scope drift')
    ...
    proof_review=json.loads(read('evidence/FIELDS_INTAKE_REVIEW_20260924.json'))
    ...
    require(len(required)==3, 'Complete proof evidence missing')
    ...
    require(len(deps['groups'])==1 and all(g['required_together'] for g in deps['groups']), 'Incomplete mathematical dependency groups')
    require(deps['required_proof_evidence']==proof_review['required_proof_evidence'], 'Lost proof dependency')
    fragment=read('evidence/FIELDS_NORMAL_DECOMPOSITION_PROOF_20260925.tex').decode().rstrip('\n')
    proof_ops=[o for o in operations if o['producer_id']=='FIELDS-RECON-059' and o['line']==3679]
...
def main():
    ...
    if a.action=='check':print(json.dumps({'passed':True,'files':len(names),'units':len(ids),'operations':56}));return
    ...
    if a.action=='admit':
        ...
        require(issued['event_id']=='lease-event-000122' and issued['state']=='active' and issued['lease_id']==m['lease_id'],'Lease changed')
        release={**issued,'event_id':'lease-event-000123','event':'released','state':'released','issued_at_utc':now(),'supersedes_event_id':issued['event_id']}
        ...
        write(ROOT/ADMISSION,{'schema':'stacks-r57-separate-admission/v1','passed':True,'previous_public':PRIOR,'candidate_commit':CANDIDATE,
        ...
        reg=json.loads((ROOT/OVERLAYS).read_bytes());require(reg['registered_entries'][-1]['id']==ID,'R57 not admitted')
        ...
        write(ROOT/COMPOSITION,{'schema':'stacks-r57-exact-source-composition/v1','passed':True,'previous_public':PRIOR,
            'candidate_commit':CANDIDATE,'manifest_sha256':MANIFEST,'admission_commit':head,'sources':sources,
            'operations':56,'units':47,'whole_isolated_payload_copied':False,'only_admitted_operations_applied':True,
    ...
    print(json.dumps({'action':a.action,'passed':True,'units':47,'operations':56}))
