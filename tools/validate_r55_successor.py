"""Validate R55 and all unchanged earlier release gates at their exact checkpoints."""
import argparse
import json
import os
import re
import textwrap
try:
    from . import r55_successor as r
    from . import validate_r54_successor as prior
except ImportError:
    import r55_successor as r
    import validate_r54_successor as prior

CURRENT = 'validation/r55-successor-current.json'
...
def validate(ref='HEAD', verify_frozen_core=False):
    ...
    r.require(issued['event_id'] == 'lease-event-000118' and issued['state'] == 'active' and released['event_id'] == 'lease-event-000119'
    ...
    r.require(exp['historical_registry_ids'] == 1518 and exp['included_effective_textual_units'] == 1516
    ...
    allowed = names | export | {r.OVERLAYS,r.LEASES,r.ADMISSION,r.COMPOSITION,CURRENT,
        'sites.tex','README.md','ai-integrated/README.md','tools/r55_successor.py',
        'tools/validate_r55_successor.py','tools/generate_changes_from_upstream.py',
        'tests/test_r55_successor.py','tests/test_changes_from_upstream.py','.github/workflows/validate.yml',
    ...
    try:
        os.chdir(r.ROOT/'ai-integrated'); exec(compile(code,'r55-full-registry-contract','exec'),{'__name__':'__main__'})
    finally: os.chdir(cwd)
    return {'status':'R55_CUMULATIVE_SUCCESSOR_PASS','units':29,'operations':35,'effective_export_units':1516,
