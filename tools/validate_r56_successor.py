"""Validate R56 and all unchanged earlier release gates at their exact checkpoints."""
import argparse
import json
import os
import re
import textwrap
try:
    from . import r56_successor as r
    from . import validate_r55_successor as prior
except ImportError:
    import r56_successor as r
    import validate_r55_successor as prior

CURRENT = 'validation/r56-successor-current.json'
...
def validate(ref='HEAD', verify_frozen_core=False):
    ...
    r.require(issued['event_id'] == 'lease-event-000120' and issued['state'] == 'active' and released['event_id'] == 'lease-event-000121'
    ...
    r.require(exp['historical_registry_ids'] == 1594 and exp['included_effective_textual_units'] == 1592
        and len(exp['chapters']) == 35 and not exp['contains_new_theorem_additions'], 'Export scope drift')
    allowed = names | export | {r.OVERLAYS,r.LEASES,r.ADMISSION,r.COMPOSITION,CURRENT,
        'stacks.tex','README.md','ai-integrated/README.md','tools/r56_successor.py',
        'tools/validate_r56_successor.py','tools/generate_changes_from_upstream.py',
        'tests/test_r56_successor.py','tests/test_changes_from_upstream.py','.github/workflows/validate.yml',
        'CHANGES_FROM_UPSTREAM.md','ai-integrated/changes/index.html','validation/changes-from-upstream-2026-08-30.json',
        'validation/upstream-ref-check-2026-09-24.json'}
    ...
    try:
        os.chdir(r.ROOT/'ai-integrated'); exec(compile(code,'r56-full-registry-contract','exec'),{'__name__':'__main__'})
    finally: os.chdir(cwd)
    return {'status':'R56_CUMULATIVE_SUCCESSOR_PASS','units':76,'operations':86,'effective_export_units':1592,
        'chapter_patches':35,'changed_paths':len(changes),'frozen_core_revalidated':verify_frozen_core,'prior':earlier}
