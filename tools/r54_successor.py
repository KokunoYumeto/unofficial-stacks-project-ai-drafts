"""Separate, hash-bound R54 candidate admission and cumulative composition."""
import argparse
...
CANDIDATE='365fbb635605c6066ebacc5db77fc360981ad1fd'
MANIFEST='5C0BCA8A118B09F69E24C01C3A6A7BAD2C23F8EAA2357EE6E2496EF9B7346729'
PREFIX='ai-integrated/candidates/commons/stacks/errata/r54/'
ID='stacks-errata-a04446e-r54'
OVERLAYS=common.OVERLAYS
LEASES=common.LEASES
ADMISSION='validation/r54-admission-2026-09-23.json'
COMPOSITION='validation/r54-source-composition-2026-09-23.json'
...
def evidence():
    ...
    require(ids==[f'MC-STK-ERR-{i}' for i in range(1742,1754)],'Unit scope drift')
    operations=json.loads(read('operation-spec.json'))['operations'];require(len(operations)==16,'Operation scope drift')
...
def main():
    ...
    if a.action=='check':print(json.dumps({'passed':True,'files':len(names),'units':len(ids),'operations':16}));return
    ...
    if a.action=='admit':
        ...
        require(issued['event_id']=='lease-event-000116' and issued['state']=='active' and issued['lease_id']==m['lease_id'],'Lease changed')
        release={**issued,'event_id':'lease-event-000117','event':'released','state':'released','issued_at_utc':now(),'supersedes_event_id':issued['event_id']}
        ...
        write(ROOT/ADMISSION,{'schema':'stacks-r54-separate-admission/v1','passed':True,'previous_public':PRIOR,'candidate_commit':CANDIDATE,
        ...
        reg=json.loads((ROOT/OVERLAYS).read_bytes());require(reg['registered_entries'][-1]['id']==ID,'R54 not admitted')
        ...
        write(ROOT/COMPOSITION,{'schema':'stacks-r54-exact-source-composition/v1','passed':True,'previous_public':PRIOR,
            'candidate_commit':CANDIDATE,'manifest_sha256':MANIFEST,'admission_commit':head,'sources':sources,
            'operations':16,'units':12,'whole_isolated_payload_copied':False,'only_admitted_operations_applied':True,
    ...
    print(json.dumps({'action':a.action,'passed':True,'units':12,'operations':16}))
