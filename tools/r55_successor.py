"""Separate, hash-bound R55 candidate admission and cumulative composition."""
import argparse
...
CANDIDATE='42aa81c7cfe82f8a881c73d2fe3eecdfc3e0a7c2'
MANIFEST='6D6433A6D8AA26D344FA5CE1F9384B651E9FEB2350127789A99E04F6606B3E69'
PREFIX='ai-integrated/candidates/commons/stacks/errata/r55/'
ID='stacks-errata-a04446e-r55'
OVERLAYS=common.OVERLAYS
LEASES=common.LEASES
ADMISSION='validation/r55-admission-2026-09-23.json'
COMPOSITION='validation/r55-source-composition-2026-09-23.json'
...
def evidence():
    ...
    require(ids==[f'MC-STK-ERR-{i}' for i in range(1754,1783)],'Unit scope drift')
    operations=json.loads(read('operation-spec.json'))['operations'];require(len(operations)==35,'Operation scope drift')
...
def main():
    ...
    if a.action=='check':print(json.dumps({'passed':True,'files':len(names),'units':len(ids),'operations':35}));return
    ...
    if a.action=='admit':
        ...
        require(issued['event_id']=='lease-event-000118' and issued['state']=='active' and issued['lease_id']==m['lease_id'],'Lease changed')
        release={**issued,'event_id':'lease-event-000119','event':'released','state':'released','issued_at_utc':now(),'supersedes_event_id':issued['event_id']}
        ...
        write(ROOT/ADMISSION,{'schema':'stacks-r55-separate-admission/v1','passed':True,'previous_public':PRIOR,'candidate_commit':CANDIDATE,
        ...
        reg=json.loads((ROOT/OVERLAYS).read_bytes());require(reg['registered_entries'][-1]['id']==ID,'R55 not admitted')
        ...
        write(ROOT/COMPOSITION,{'schema':'stacks-r55-exact-source-composition/v1','passed':True,'previous_public':PRIOR,
            'candidate_commit':CANDIDATE,'manifest_sha256':MANIFEST,'admission_commit':head,'sources':sources,
            'operations':35,'units':29,'whole_isolated_payload_copied':False,'only_admitted_operations_applied':True,
    ...
    print(json.dumps({'action':a.action,'passed':True,'units':29,'operations':35}))
