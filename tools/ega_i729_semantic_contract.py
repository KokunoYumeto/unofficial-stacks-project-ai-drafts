...
def historical_inputs(checkpoint, scope, raw_loader):
    """Project exact sealed prefixes, never a mutable snapshot from the caller.

    Successor rows remain in the live files. The older receipt binds only its
    original prefix; every byte in that prefix is still independently checked.
    """
    historical_raw = {}
    tables = {}
    for ledger in checkpoint["ledgers"]:
        ...
        if not isinstance(raw, bytes) or b"\r" in raw or not raw.endswith(b"\n"):
            raise ValueError("I7.2.9 invalid live ledger serialization: " + path)
        lines = [line + b"\n" for line in raw.split(b"\n")[:-1]]
        frozen = b"".join(lines[:ledger["final_rows"] + 1])
        if len(frozen) != ledger["bytes"] or digest(frozen) != ledger["sha256"]:
            raise ValueError("I7.2.9 historical ledger prefix changed: " + path)
    ...
    historical_scope = dict(scope, statement_review_snapshot=checkpoint["statement_review_snapshot"],
