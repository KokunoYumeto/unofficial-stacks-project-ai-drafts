"""Separate, hash-bound R52 candidate admission and cumulative composition."""
import argparse
...
CANDIDATE='2fff6cd4072bdd1e39db1a90a512395a16cc81e1'
MANIFEST='9436F60D78296691613806E84E3E73D730FA0865C9BDBA8EF6BC1B237631739E'
PREFIX='ai-integrated/candidates/commons/stacks/errata/r52/'
ID='stacks-errata-a04446e-r52'
OVERLAYS=common.OVERLAYS
LEASES=common.LEASES
ADMISSION='validation/r52-admission-2026-09-22.json'
COMPOSITION='validation/r52-source-composition-2026-09-22.json'
...
def evidence():
    ...
    require(ids==[f'MC-STK-ERR-{i}' for i in range(1605,1688)],'Unit scope drift')
    operations=json.loads(read('operation-spec.json'))['operations'];require(len(operations)==107,'Operation scope drift')
    ...
    for row in batch['sources']:
        ...
        require(old['diagnostics']==first['diagnostics']==second['diagnostics'] and not first['diagnostics']['fatal_duplicate_glyph_rerun'],'Diagnostic regression')
        ...
        require([r['page'] for r in visual['pages']]==list(range(1,old['pages']+1)),'Missing page comparison')
...
def main():
    ...
    if a.action=='check':print(json.dumps({'passed':True,'files':len(names),'units':len(ids),'operations':107}));return
    ...
    if a.action=='admit':
        ...
        require(issued['event_id']=='lease-event-000110' and issued['state']=='active' and issued['lease_id']==m['lease_id'],'Lease changed')
        release={**issued,'event_id':'lease-event-000111','event':'released','state':'released','issued_at_utc':now(),'supersedes_event_id':issued['event_id']}
        ...
        write(ROOT/ADMISSION,{'schema':'stacks-r52-separate-admission/v1','passed':True,'previous_public':PRIOR,'candidate_commit':CANDIDATE,
        ...
        reg=json.loads((ROOT/OVERLAYS).read_bytes());require(reg['registered_entries'][-1]['id']==ID,'R52 not admitted')
        ...
        write(ROOT/COMPOSITION,{'schema':'stacks-r52-exact-source-composition/v1','passed':True,'previous_public':PRIOR,
            'candidate_commit':CANDIDATE,'manifest_sha256':MANIFEST,'admission_commit':head,'sources':sources,
            'operations':107,'units':83,'whole_isolated_payload_copied':False,'only_admitted_operations_applied':True,
    ...
    print(json.dumps({'action':a.action,'passed':True,'units':83,'operations':107}))
