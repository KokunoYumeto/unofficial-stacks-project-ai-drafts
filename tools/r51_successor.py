"""Separate, hash-bound R51 candidate admission and cumulative composition."""
import argparse
...
CANDIDATE='15cc3cd91f87e5737cf0becb6dcd77bdfc27f62e'
MANIFEST='DC9BD24ED9E52C7E56FFBF0230472B1D380D0A2FB049CEDA9B4D3F006F06BFCF'
PREFIX='ai-integrated/candidates/commons/stacks/errata/r51/'
ID='stacks-errata-a04446e-r51'
OVERLAYS=common.OVERLAYS
LEASES=common.LEASES
ADMISSION='validation/r51-admission-2026-09-22.json'
COMPOSITION='validation/r51-source-composition-2026-09-22.json'
...
def evidence():
    ...
    require(ids==[f'MC-STK-ERR-{i}' for i in range(1576,1605)],'Unit scope drift')
    operations=json.loads(read('operation-spec.json'))['operations'];require(len(operations)==33,'Operation scope drift')
...
def main():
    ...
    if a.action=='check':print(json.dumps({'passed':True,'files':len(names),'units':len(ids),'operations':33}));return
    ...
    if a.action=='admit':
        ...
        require(issued['event_id']=='lease-event-000108' and issued['state']=='active' and issued['lease_id']==m['lease_id'],'Lease changed')
        release={**issued,'event_id':'lease-event-000109','event':'released','state':'released','issued_at_utc':now(),'supersedes_event_id':issued['event_id']}
        ...
        write(ROOT/ADMISSION,{'schema':'stacks-r51-separate-admission/v1','passed':True,'previous_public':PRIOR,'candidate_commit':CANDIDATE,
        ...
        reg=json.loads((ROOT/OVERLAYS).read_bytes());require(reg['registered_entries'][-1]['id']==ID,'R51 not admitted')
        ...
        write(ROOT/COMPOSITION,{'schema':'stacks-r51-exact-source-composition/v1','passed':True,'previous_public':PRIOR,
            'candidate_commit':CANDIDATE,'manifest_sha256':MANIFEST,'admission_commit':head,'sources':sources,
            'operations':33,'units':29,'whole_isolated_payload_copied':False,'only_admitted_operations_applied':True,
    ...
    print(json.dumps({'action':a.action,'passed':True,'units':29,'operations':33}))
