"""Validate R57 and all unchanged earlier release gates at their exact checkpoints."""
import argparse
import json
import os
import re
import textwrap
try:
    from . import r57_successor as r
    from . import validate_r56_successor as prior
except ImportError:
    import r57_successor as r
    import validate_r56_successor as prior

CURRENT = 'validation/r57-successor-current.json'
...
def validate(ref='HEAD', verify_frozen_core=False):
    ...
    r.require(issued['event_id'] == 'lease-event-000122' and issued['state'] == 'active' and released['event_id'] == 'lease-event-000123'
    ...
    r.require(exp['historical_registry_ids'] == 1641 and exp['included_effective_textual_units'] == 1639
    ...
    allowed = names | export | {r.OVERLAYS,r.LEASES,r.ADMISSION,r.COMPOSITION,CURRENT,
        'fields.tex','README.md','ai-integrated/README.md','tools/r57_successor.py',
        'tools/validate_r57_successor.py','tools/generate_changes_from_upstream.py',
        'tests/test_r57_successor.py','tests/test_changes_from_upstream.py','.github/workflows/validate.yml',
        'CHANGES_FROM_UPSTREAM.md','ai-integrated/changes/index.html','validation/changes-from-upstream-2026-08-30.json',
        'validation/upstream-ref-check-2026-09-25.json'}
    ...
    try:
        os.chdir(r.ROOT/'ai-integrated'); exec(compile(code,'r57-full-registry-contract','exec'),{'__name__':'__main__'})
    finally: os.chdir(cwd)
    return {'status':'R57_CUMULATIVE_SUCCESSOR_PASS','units':47,'operations':56,'effective_export_units':1639,
