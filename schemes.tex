...
\begin{reference}
\cite[Chapter II, \S 1, nos. 32 and 35, pp. 225 and 228]{FAC}
\end{reference}
...
$f : X \to Y$ and $g : Y \to Z$ be morphisms of locally
...
\begin{reference}
\cite[Chapter II, \S 1, no. 32, p. 225]{FAC}
\end{reference}
\begin{slogan}
...
\begin{reference}
\cite[Chapter II, \S 1, no. 31, p. 224]{FAC}
The affine global-section assertion is proved in
\cite[Chapter II, \S 3, no. 43, Proposition 4, p. 235]{FAC}.
Its extension to every closed classical subvariety is
\cite[Chapter II, \S 3, no. 44, Proposition 7, Corollary 3,
p. 237]{FAC}.
The exactness and object-detection assertions for the classical
associated-sheaf functor are
\cite[Chapter II, \S 4, no. 48, Propositions 1 and 2,
pp. 240--241]{FAC}.
\end{reference}
\begin{history}
In the irreducible classical case, the no. 43 proof identifies the
regular functions with the intersection of the local rings inside the
function field and then uses the Nullstellensatz to identify that intersection
with the coordinate ring. The printed line places
$\Gamma(V, \mathcal{O}_V)=A=\bigcap_x\mathcal{O}_{x,V}$ before the
Nullstellensatz step that establishes the second equality; the noncircular
order is $\Gamma(V, \mathcal{O}_V)=\bigcap_x\mathcal{O}_{x,V}$ first and
$A=\bigcap_x\mathcal{O}_{x,V}$ second. The lemma below proves the result
directly for every ring and every associated module. The no. 44 corollary
recovers the source's reducible closed-subvariety case by exactness of global
sections on affine space. In no. 48 the source identifies each classical
closed-point stalk with a localization of the coordinate ring, proves
exactness stalkwise, and detects a module from those stalks using maximal
ideals. The statement below includes every prime stalk and, more directly,
recovers the module as the global sections of its associated sheaf.
\end{history}
...
\label{lemma-f-open}
\begin{reference}
\cite[Chapter II, \S 3, no. 42, p. 234]{FAC}
\end{reference}
\begin{history}
For a regular function $f$ on a classical algebraic variety $V$, the cited
source writes $V_f$ for the locus where the value $f(x)$ is nonzero. On the
associated reduced scheme this is the locus where the germ of $f$ is a unit,
namely the open $D(f)$ below.
\end{history}
Let $X$ be a locally ringed space.
...
The coordinate-function criterion in the classical reduced setting
appears in \cite[Chapter II, \S 1, no. 32, Proposition 5, p. 225]{FAC}.
\end{reference}
Let $X$ be a locally ringed space.
Let $Y$ be an affine scheme.
The map
...
restriction mapping on the affine schemes is defined in terms
...
\begin{reference}
\cite[Chapter II, \S 4, no. 48, coordinate-ring discussion,
p. 240]{FAC}
\end{reference}
\begin{history}
The cited discussion identifies a classical affine variety over the fixed
algebraically closed field with the character space of its reduced finitely
generated coordinate algebra. Thus it sees the classical closed points and
their Zariski topology. The equivalence below uses the full prime spectrum,
retains nilpotents, imposes no finite-type hypothesis, and records the
contravariance on morphisms.
\end{history}
...
\begin{reference}
\cite[Chapter II, \S 3, no. 42, Proposition 2, p. 234]{FAC}
\end{reference}
\begin{history}
The cited proof realizes $V_f$ as the closed hypersurface
$\lambda f(x)=1$ in $V \times \mathbf{A}^1$. The localization construction
below is the scheme-theoretic version and applies over an arbitrary ring.
\end{history}
Let $Y$ be an affine scheme.
...
\begin{reference}
\cite[Chapter II, \S 1, no. 33, pp. 225--226]{FAC}
The affine-product assertion is repeated in
\cite[Chapter II, \S 3, no. 42, pp. 233--234]{FAC}.
\end{reference}
...
\begin{reference}
\cite[Chapter II, \S 3, no. 45, Theorem 2, pp. 237--238]{FAC}
\end{reference}
\begin{history}
The cited theorem proves that the global sections of a coherent algebraic
sheaf on an affine classical variety generate every stalk. The lemma below
identifies an arbitrary quasi-coherent module on an affine scheme with the
sheaf associated to its module of global sections. Thus those global sections
generate every localization, without coherence, irreducibility, reducedness,
or finite-type hypotheses.
\end{history}
Let $(X, \mathcal{O}_X) = (\Spec(R), \mathcal{O}_{\Spec(R)})$
...
shows that there exists an $R$-module $M$ such that
...
to a section $s_i \in \mathcal{F}(D(f_i))$. (Namely, $\varphi^{-1}_i(m_i)$.)
...
\begin{reference}
\cite[Chapter II, \S 3, no. 44, Proposition 7, Corollary 2,
p. 237]{FAC}
The same exactness conclusion is proved in
\cite[Chapter II, \S 3, no. 45, Theorem 2, Corollary 2,
p. 238]{FAC}.
The classical finite-module/coherent-sheaf correspondence is
\cite[Chapter II, \S 4, no. 49, Theorem 1, pp. 241--242]{FAC}.
\end{reference}
\begin{history}
The no. 44 corollary proves surjectivity on global sections for the quotient
$\mathcal{O}_X^p/\mathcal{F}$ on an affine classical variety by vanishing of
$H^1(X,\mathcal{F})$. The no. 45 corollary uses finite global generation to
deduce exactness for a sequence of coherent algebraic sheaves. The equivalence
below identifies global sections with the corresponding module functor and
gives both conclusions for arbitrary quasi-coherent modules on an affine
scheme. In no. 49 the two canonical maps are proved isomorphisms by finite
free presentations and exactness of global sections; its first diagram is the
finite-presentation proof of the unit isomorphism. The source's first remark
points to a later alternate derivation of the counit. The statement below
makes the natural quasi-inverse equivalence explicit and removes both the
finite-generation and coherence restrictions.
\end{history}
Let $(X, \mathcal{O}_X) = (\Spec(R), \mathcal{O}_{\Spec(R)})$
be an affine scheme.
...
\begin{reference}
\cite[Chapter II, \S 1, no. 31, Proposition 4 and Corollary,
p. 225]{FAC}
\end{reference}
Let $R$ be a ring.
...
\begin{reference}
\cite[Chapter II, \S 1, no. 34, pp. 226--228]{FAC}
\end{reference}
\begin{history}
...
Over an algebraically closed field, the closed points and actual-function
sheaf of a prealgebraic variety in \cite{FAC} give the classical counterpart
of a reduced scheme of finite type. Its additional axiom that the diagonal
be closed is precisely the separatedness condition below. The associated
scheme retains nonclosed points, and allowing nilpotents goes beyond this
classical actual-function description.
\end{history}
...
\begin{reference}
\cite[Chapter II, \S 3, no. 42, Theorem 1 and Corollary, p. 234]{FAC}
\end{reference}
\begin{history}
The cited theorem says that affine opens form a basis for a classical
algebraic variety, hence every open covering admits an affine refinement.
The result below holds for every scheme by local affineness. The source's
additional assertion that finite intersections of members of such a cover
remain affine uses its closed-diagonal axiom; it is not valid for arbitrary
schemes, as Remark \ref{remark-intersection-affine-opens} records.
\end{history}
...
\begin{reference}
\cite[Chapter II, \S 1, nos. 31--32 and 35, and \S 2, no. 37,
pp. 224--225 and 228--231]{FAC}
\end{reference}
...
Suppose that $k$ is algebraically closed and that
$T \subset k^n$ is a locally closed algebraic set. Via the identification
of $k^n$ with the closed points of $\mathbf{A}^n_k$, its classical sheaf of
functions locally represented by rational functions regular at the point
agrees with the structure sheaf of the reduced induced scheme on $T$.
More precisely, if $T = U \cap F$ with $U$ open and $F$ closed, then for
$x \in T$ the local ring is
$$
\left(k[T_1, \ldots, T_n]/I(F)\right)_{\mathfrak m_x}.
$$
Indeed, the Hilbert Nullstellensatz identifies $I(F)$ as the radical ideal
defining $F$, and the assertion follows from
Example \ref{example-closed-immersion-affines} after restriction to $U$.

Under the same hypotheses, let $T \subset k^n$ and $T' \subset k^m$ be
locally closed algebraic sets, each endowed with its reduced induced scheme
structure. A map $\varphi : T \to T'$ of classical point sets is regular in
the classical sense if and only if it is induced by a morphism of schemes.
Indeed, its coordinate functions determine a unique morphism
$T \to \mathbf{A}^m_k$ by Lemma \ref{lemma-morphism-into-affine}.
The scheme $T$ is Jacobson by
Morphisms, Lemma \ref{morphisms-lemma-ubiquity-Jacobson-schemes}; hence its
closed points are dense in every closed subset. The prescribed image on
closed points therefore forces the morphism to factor through $T'$; after
restriction to an open in which $T'$ is closed, the reduced factorization is
Lemma \ref{lemma-map-into-reduction}. Conversely, a morphism pulls back local
sections. Thus the classical coordinate criterion is the reduced locally
closed case of the affine-scheme mapping property. A bijection is biregular
exactly when the associated scheme morphism is an isomorphism, by
Lemma \ref{lemma-isomorphism-locally-ringed}.

Here ``regular'' describes a map preserving regular functions. It must not
be confused with a regular morphism in the sense of
More on Morphisms, Definition \ref{more-morphisms-definition-regular}.

In \cite{FAC}, every locally closed algebraic subset with this induced
structure is called a subvariety; the source notes that Weil reserves that
word for a closed irreducible subvariety. The inclusion is an immersion.
For a classical variety $Y$, a map $Y \to T$ is regular if and only if its
composite $Y \to T \to X$ is regular. Scheme-theoretically, the corresponding
factorization through the reduced induced structure uses that $Y$ is reduced,
as in Lemma \ref{lemma-map-into-reduction}; set-theoretic image containment
alone does not give this factorization for an arbitrary nonreduced source.
\end{remark}
...
\begin{lemma}
\label{lemma-reduction-functorial}
\begin{reference}
\cite[(5.1.5)]{EGA1}
\end{reference}
For every morphism of schemes $f : X \to Y$ there is a unique morphism
$$
f_{red} : X_{red} \longrightarrow Y_{red}
$$
such that the diagram
$$
\xymatrix{
X_{red} \ar[r]^{f_{red}} \ar[d] & Y_{red} \ar[d] \\
X \ar[r]^f & Y
}
$$
commutes. Moreover, $(\text{id}_X)_{red} = \text{id}_{X_{red}}$ and
$(g \circ f)_{red} = g_{red} \circ f_{red}$ whenever the composition is
defined.
\end{lemma}

\begin{proof}
The scheme $X_{red}$ is reduced. Hence the composition
$X_{red} \to X \to Y$ factors through $Y_{red}$ by
Lemma \ref{lemma-map-into-reduction}. The factorization is unique by
Lemma \ref{lemma-characterize-closed-subspace}. The two functoriality
assertions follow from the same uniqueness.
\end{proof}
...
\label{lemma-glue}
\begin{reference}
\cite[Chapter II, \S 1, no. 34, Proposition 8, p. 227]{FAC}
\end{reference}
\begin{slogan}
...
\begin{reference}
\cite[Chapter II, \S 1, no. 33, Propositions 6 and 7,
pp. 225--226, and no. 34, Corollary to Proposition 8, p. 227]{FAC}
\end{reference}
...
\begin{remark}
\label{remark-classical-products-varieties}
\begin{reference}
\cite[Chapter II, \S 1, nos. 33 and 35, pp. 225--226 and 228]{FAC}
\end{reference}
Let $k$ be an algebraically closed field. For locally closed algebraic sets
$T \subset k^n$ and $T' \subset k^m$, endowed with their reduced induced
scheme structures, the classical product $T \times T'$ agrees with the
scheme product over $k$ on closed points and with its classical
regular-function sheaf. The product is reduced by
Algebra, Lemma \ref{algebra-lemma-perfect-reduced}, since $k$ is perfect.
The full scheme product generally has additional
nonclosed points, as described in Lemma \ref{lemma-points-fibre-product}.
Even on closed points, the induced Zariski topology is generally finer than
the product topology; for example, the diagonal in $k \times k$ is Zariski
closed but is not closed for the product of the two Zariski topologies when
$k$ is infinite.

The universal property of the scheme product says that a morphism
$W \to T \times T'$ is equivalent to a pair of morphisms $W \to T$ and
$W \to T'$. It also supplies the projections, constant-point sections, and
products of morphisms used in the classical argument. The converse direction
for a product of morphisms, and consequently the converse direction for a
product isomorphism, require both source factors to be nonempty: choosing a
$k$-point in either factor gives the section which recovers the other map.
Without this hypothesis an empty factor makes the product map carry no
information about the other map.
\end{remark}
...
\begin{proof}
As in the proof of Lemma \ref{lemma-fibre-topological} this reduces
...
\begin{proof}
Let $f : X \to Y$ be a quasi-compact morphism of schemes over $S$,
and let $S' \to S$ be a morphism of schemes. Set
$Y' = Y \times_S S'$ and $X' = X \times_S S'$.
The canonical identification $X' = X \times_Y Y'$ identifies
$f' : X' \to Y'$ with the base change of $f$ by the projection
$q : Y' \to Y$.

Choose an affine open covering $Y = \bigcup_{i \in I} U_i$.
For each $i$, write $f^{-1}(U_i) = \bigcup_{j = 1}^{n_i} V_{ij}$
as a finite union of affine opens, using
Lemma \ref{lemma-quasi-compact-affine}.
Choose an affine open covering
$q^{-1}(U_i) = \bigcup_{k \in K_i} W_{ik}$.
For each $i$ and $k$ we have
$$
(f')^{-1}(W_{ik}) =
\bigcup\nolimits_{j = 1}^{n_i} V_{ij} \times_{U_i} W_{ik}.
$$
By Lemma \ref{lemma-affine-covering-fibre-product}
the right hand side is a finite affine open covering.
Thus $(f')^{-1}(W_{ik})$ is quasi-compact.
The $W_{ik}$ form an affine open covering of $Y'$, so
Lemma \ref{lemma-quasi-compact-affine} shows that $f'$ is quasi-compact.
\end{proof}
...
universally closed morphisms and morphisms satisfying the
...
\begin{lemma}
\label{lemma-diagonal-identities}
\begin{reference}
\cite[(5.3.1), Propositions 5.3.2 and 5.3.15, and Corollaries 5.3.4 and
5.3.12]{EGA1}
\end{reference}
Let $X$, $Y$, and $T$ be schemes over $S$.
\begin{enumerate}
\item If $f : T \to X$ and $g : T \to Y$ are morphisms over $S$, then
$$
(f, g) = (f \times_S g) \circ \Delta_{T/S}.
$$
\item Via the canonical isomorphism
$$
(X \times_S Y) \times_S (X \times_S Y) \cong
(X \times_S X) \times_S (Y \times_S Y),
$$
we have
$$
\Delta_{X \times_S Y/S} = \Delta_{X/S} \times_S \Delta_{Y/S}.
$$
\item If $S' \to S$ is a morphism, then the base change of
$\Delta_{X/S}$ to $S'$ is $\Delta_{X_{S'}/S'}$ via the canonical
isomorphism
$$
(X \times_S X)_{S'} \cong X_{S'} \times_{S'} X_{S'}.
$$
\item If $f : X \to Y$ is a morphism over $S$ and $S' \to S$ is a
morphism, then via the canonical isomorphism
$$
(X \times_S Y)_{S'} \cong X_{S'} \times_{S'} Y_{S'},
$$
the base change of the graph morphism
$$
\Gamma_f = (\text{id}_X, f) : X \longrightarrow X \times_S Y
$$
is the graph morphism
$$
\Gamma_{f_{S'}} = (\text{id}_{X_{S'}}, f_{S'}) :
X_{S'} \longrightarrow X_{S'} \times_{S'} Y_{S'}.
$$
\item If $f : X \to Y$ is a morphism over $S$, then
$$
(f \times_S f) \circ \Delta_{X/S} = \Delta_{Y/S} \circ f.
$$
\end{enumerate}
\end{lemma}

\begin{proof}
All five assertions follow from the universal property of fibre products.
For (1), the two composites with the projections to $X$ and $Y$ are $f$ and
$g$. For (2), the four composites with the projections to $X$, $X$, $Y$,
and $Y$ agree. For (3), both composites with the projections to $X_{S'}$
are the identity. For (4), the two composites with the projections to
$X_{S'}$ and $Y_{S'}$ are $\text{id}_{X_{S'}}$ and $f_{S'}$. For (5),
composing either side with either projection to $Y$ gives $f$.
\end{proof}
...
\begin{reference}
\cite[Chapter II, \S 1, no. 34, axiom $(\mathrm{VA}_{II})$,
p. 227]{FAC}
\end{reference}
Let $f : X \to S$ be a morphism of schemes.
\begin{enumerate}
...
we see that $\Delta_{X/S}$ is a closed immersion if and only if
...
\begin{reference}
\cite[Chapter II, \S 3, no. 42, Proposition 1, p. 234]{FAC}
\end{reference}
\begin{history}
The cited source proves that two affine opens of an algebraic variety have
affine intersection by pulling back the closed diagonal inside their affine
product, and explicitly notes failure for prealgebraic varieties. Part (1)(a)
is the relative scheme-theoretic form: separatedness supplies the closed
diagonal, and the statement is false without it.
\end{history}
Let $f : X \to S$ be a morphism of schemes.
\begin{enumerate}
...
\begin{reference}
\cite[Chapter II, \S 1, no. 35, p. 228]{FAC}
\end{reference}
...
\item Given a morphism $f : X \to Y$ of schemes where $Y$ has $P$, then
...
by Lemmas \ref{lemma-separated-permanence} and
\ref{lemma-composition-quasi-compact} applied to
$X \to Y \to \Spec(\mathbf{Z})$. If $X$ has $P$, then so does $f$ by
Lemmas \ref{lemma-compose-after-separated} and
\ref{lemma-quasi-compact-permanence} applied to
...
\ref{lemma-separated-permanence} and
\ref{lemma-quasi-compact-preserved-base-change}.
...
\begin{lemma}[Valuative criterion of separatedness]
...
$$
where $\mathcal{F}_i, \mathcal{F}_{ijk}$ denote the
...
third terms of the exact sequence are quasi-coherent
