...
for every $U \in \Ob(\mathcal{C})$ (and inherit the action on morphisms
...
it to the reader to verify that they are each other's inverses.
...
presheaves and sheaves on a topological space, as defined
...
We first replace $G\textit{-Sets}$ by a
...
U_{\alpha(j)} \times_U U_{\alpha(j')}$ compatible with the
...
$\mathcal{V} = \{V_j \to U\}_{j \in J}$, with the refinement given
...
{\v C}ech cohomology set of a covering and its functoriality
properties.
...
$s_{j, i}$ and $s_{j, i'}$ for all $j$ and hence by the sheaf property
...
is a covering we see that $s, s'$ are the images of the same element of
...
notation is from Definition \ref{definition-global-sections}.
\end{example}
...
we see that $\mathcal{F} \mapsto \mathcal{F}^+$ commutes
...
In the situation of Lemma \ref{lemma-pushforward-sheaf}, the functor $u_s : \mathcal{G} \mapsto (u_p \mathcal{G})^\#$
...
In the situation of Lemma \ref{lemma-pushforward-sheaf}, for any presheaf $\mathcal{G}$ on $\mathcal{C}$
...
gives rise to a zeroth {\v C}ech cohomology set
...
sets. We conclude that $u^p\mathcal{F}$ is a sheaf if $\mathcal{F}$
...
\begin{slogan}
Finite coproducts of sheaves conserve quasi-compactness.
\end{slogan}
...
are equal. This is shown in the next paragraph
...
for some $\alpha_{i, j} < \beta$. Since the cardinality of
...
\medskip\noindent
For an arbitrary object $V$ of $\mathcal{C}$ we can pick $a : j \to i$
and an object $V_j \in \Ob(\mathcal{C}_j)$ with $V = u_j(V_j)$. Then we
can set
$$
...
of $a : j \to i$ and of the object $V_j$ with $u_j(V_j) = V$; we omit
the verification. Moreover, if $\alpha : V \to V'$ is a morphism of
$\mathcal{C}$, then we can choose $a : j \to i$ and a morphism
...
\colim_{b : k \to j} H^0(u_b(\mathcal{U}_j), f_{a \circ b}^{-1}\mathcal{F}) =
\colim_{b : k \to j} f_{a \circ b}^{-1}\mathcal{F}(u_b(U_j)) =
\mathcal{G}(U)
$$
...
and the $\varphi_{\text{id}, \text{id}, V_i}$ imply that $\varphi_a$
is the adjoint of the map $\varphi_{\text{id}}$ via
$$
...
is determined by the map $\varphi_{\text{id}} : \mathcal{F} \to f_{i, *}\mathcal{H}$.
...
(Lemma \ref{lemma-limit-sheaf}), the functors $f_a^{-1}$ commute with finite
...
$$
\colim_{a : j \to i} \mathcal{F}_j(u_a(X_i)) =
...
$a : j \to i$ and $b : k \to j$ with composition $c = a \circ b$
we have
...
$$
\colim_{i \in \Ob(\mathcal{I})} f_i^{-1}f_{i, *}\mathcal{F}
\longrightarrow
\mathcal{F}
$$
...
$$
(\colim_{i \in \Ob(\mathcal{I})} f_i^{-1}f_{i, *}\mathcal{F})(U) \to \mathcal{F}(U)
$$
is bijective. Choose an $i$ and an object $U_i$ of $\mathcal{C}_i$
with $u_i(U_i) = U$. Then the left hand side is equal to
$$
(\colim_{i \in \Ob(\mathcal{I})} f_i^{-1}f_{i, *}\mathcal{F})(U) =
...
In the situation of Lemma \ref{lemma-exact-cocontinuous}, for any presheaf $\mathcal{G}$ on $\mathcal{D}$
...
\begin{proof}
Let $U \in \Ob(\mathcal{C})$.
Let $\{E_i \to v(u(U))\}$ be a covering of $v(u(U))$ in $\mathcal{E}$.
...
shouldn't any covering of $U\cap Z$ come from a covering
...
(but not always, see Example \ref{example-not-equivalent})
...
\mathcal{F} \circ v
$$
The first equality is Lemma \ref{lemma-cocontinuous-morphism-topoi}.
The second equality is Lemma \ref{lemma-pu-sheaf}.
The third equality is Lemma \ref{lemma-adjoint-functors}.
The final equality is the definition of $v^p$ in
Section \ref{section-functoriality-PSh}. This proves (1).
For $\mathcal{G}$ as in (2) we have
$$
g^{-1}\mathcal{G} = (u^p\mathcal{G})^\# = (v_p\mathcal{G})^\#
$$
The first equality is Lemma \ref{lemma-cocontinuous-morphism-topoi}.
The second equality is Lemma \ref{lemma-adjoint-functors}.
\end{proof}
...
be a covering in $\mathcal{D}$. Then $\{v(V_j) \to v(u(U))\}_{j \in J}$ is
...
immediate from Lemmas \ref{lemma-have-functor-other-way} and
\ref{lemma-have-functor-other-way-morphism}.
\end{proof}
...
be functors of the underlying categories such that $v$ is right adjoint to $u$.
Lemma \ref{lemma-continuous-with-continuous-left-adjoint}
shows that if $v$ is continuous, then $u$ is cocontinuous.
...
Lemma \ref{lemma-when-shriek} we obtain a morphism of topoi
$$
...
where $j_{U!}^{PSh}\mathcal{G}$ is the presheaf defined by the formula
in Lemma \ref{lemma-describe-j-shriek} and $\gamma$ is the composition of
$j_{U!}^{PSh}\mathcal{G} \to j_{U!}^{PSh}*$ with the identification
...
The value of the presheaf $j_{U!}^{PSh}\mathcal{F}_\varphi$
on $X$ is
$$
...
$$
by the description of $j^{-1} = j_{V/U}^{-1}$ as the restriction
...
\textit{PSh}(\mathcal{C})
$$
which are respectively right and left adjoint to $j_U^{-1}$. By
Lemma \ref{lemma-describe-j-shriek}
...
a {\it glueing datum for sheaves of sets with respect to
...
{\it absolute glueing datum for sheaves of sets on $\mathcal{C}$}.
...
$j_V = j_U \circ j_f$ as in Lemma \ref{lemma-relocalize}.
...
There is a variant of Lemma \ref{lemma-glue-sheaves-absolute}
...
impose some additional hypotheses on the site or on the object
...
$(s_i|_{V_i \times_V V_j/U}, \varphi_i \circ \text{pr}_1)$, and
likewise the restriction of the pair $(s_j, \varphi_j)$ to
$V_i \times_V V_j$ is the pair
$(s_j|_{V_i \times_V V_j/U}, \varphi_j \circ \text{pr}_2)$.
Hence, if the family $(s_i, \varphi_i)$ lies in
$\check{H}^0(\mathcal{V}, \mathcal{H})$, then we see that
$\varphi_i \circ \text{pr}_1 = \varphi_j \circ \text{pr}_2$.
...
of the extension by the empty set produce back the original sheaf.
...
because the assumption on $U$ is equivalent to the full faithfulness
...
\noindent
The following lemma is important in order to understand the relation
...
$u' : \mathcal{C}/U \to \mathcal{D}/V$, $U'/U \mapsto u(U')/V$.
...
$\phi_i : u(U_i') \to V_{\alpha(i)}$ over $u(U')$.
...
$u(\alpha)$. The category of such pairs $(U', \psi)$ has an
...
of the limit with $s_{(U, \text{id})} = s$. This proves
...
in $\mathcal{C}/U$ such that $u(c_{ik}) = c \circ u(f_i \circ f_{ik})$.
Hence (4) holds
for $u : \mathcal{C}/U \to \mathcal{D}/u(U)$.
...
\begin{proof}
Warning: Some of the statements above may look a bit confusing at first;
...
Lemma \ref{lemma-sheaf-coequalizer-representable}.
Denote by $g : \Sh(\mathcal{C}) \to \Sh(\mathcal{C}')$ the
...
The fourth equality is by Lemma \ref{lemma-special-equivalence} (4).
...
$j_{\mathcal{F}, *}$ is the one that associates to
...
Lemma \ref{lemma-localize-morphism-topoi}
may be different from the morphism of sites given to us in the lemma.
...
Set $\mathcal{G} = h_V^\#$ and $\mathcal{F} = h_U^\#$.
...
Two such $(V, y, f)$, $(V', y', f')$ determine the same element if
there exists a morphism $\phi : V \to V'$ such that
$u(\phi)(y) = y'$ and $f' \circ \phi = f$, and in general you have
to take chains of identifications like this to get the correct equivalence
...
For any functor $u : \mathcal{C} \to \textit{Sets}$, the functor $u^p$ is a right adjoint to the stalk functor
on presheaves.
\end{lemma}
...
For a set $E$ we define $p_*E = u^sE$ to be the sheaf
...
above defines a morphism of topoi
...
it reflects injections, it reflects
...
\end{enumerate}
Then the category of neighbourhoods of $p$ is cofiltered
...
\right.
$$
This functor commutes with products and fibred products,
...
are given by identity maps. Condition (3) holds because
$\mathcal{F}_p = \mathcal{F}(U_0)$ and since the topology is chaotic
...
$$
The first equality holds since $f^{-1} = u_s$, the second equality
...
$q^{-1} \circ j_U^{-1} = p^{-1}$. Since $U/U$ is a final object of
...
family of points of $\mathcal{C}/U_i$. For $j \in J_i$ denote by
...
\begin{proof}
Denote by $p_i$ the stalk functor (\ref{equation-stalk}) on
...
does, see Categories, Lemma \ref{categories-lemma-directed-commutes}.

\medskip\noindent
We say that a system $(I, \geq, U_i, f_{ii'})$
is a {\it refinement} of $(J, \geq, V_j, g_{jj'})$ if
$J \subset I$, the ordering on $J$ is induced from that of $I$
...
For $j \geq j_0$ we set $V_{j, k} = V_j \times_{f'\circ g_{j j_0}, W} W_k$.
...
have distinct images in $\colim_j \mathcal{F}(V_{j, k})$.
...
it follows that $s, s'$ have distinct images in $\mathcal{F}_p$.
...
such that
\begin{enumerate}
\item $(I, \geq, U_i, f_{ii'})$ is a refinement of $(J, \geq, V_j, g_{jj'})$,
...
by refinement in the second. Denote by $\mathcal{S}$ the class
...
a point $p$ such that $s, s'$ have distinct images in
...
pro-\'etale site of any scheme has enough weakly contractible objects.
...
Let $a : \mathcal{F} \to \mathcal{F}'$ be a surjective map of sheaves
...
\begin{proof}
Let $\alpha : \mathcal{F} \to \mathcal{G}$ be a map of sheaves
...
$G$-sets and $H$-sets , with coverings the countable families of jointly
...
Definition \ref{definition-site}
applied to the empty covering of $U$.
\end{proof}
...
$$
Then $u(\mathcal{U})' = \{u(U_i) \to u(U)\}_{i \in I_1}$ is still a covering of $u(U)$
...
$i \mapsto \mathcal{G}_i$ be a diagram. We know that the colimit of
this diagram is the sheafification of the colimit in the category of
presheaves, see
Lemma \ref{lemma-colimit-sheaves}.
Denote by $\colim^{Psh}$ the colimit in the category
...
\medskip\noindent
Let $a, b : \mathcal{G}_1 \to \mathcal{G}_2$ be two morphisms of $E$.
...
on sheaves of algebraic structures which agree with pushforward
...
\noindent
The problem comes with trying to define the pullback.
...
called an ``{\it esp\`ece de structure alg\'ebrique $\ll$d\'efinie
par limites projectives finies$\gg$}''. For such an esp\`ece you
...
\medskip\noindent
A sheaf of monoids is given by a triple $(\mathcal{F}, \cdot, 1)$
with suitable axioms.

\medskip\noindent
Let $R$ be a ring. A sheaf of $R$-modules is given by
...
Let $\mathcal{C}$ be a site. Let $a, b : V \to U$ be morphisms of $\mathcal{C}$
such that
$$
\xymatrix{
...
$$
of topoi. Then the base change maps
...
(3) of Definition \ref{definition-topology}.
...
such that the sieve generated by the $f_i$ is contained
in $S$. Since each $S'\times_U U_i$ is in $J(U_i)$ we
see that there are coverings $\{U_{ij} \to U_i\}$ of the
site such that the image of $h_{U_{ij}} \to h_{U_i}$ is contained
in $S' \times_U U_i$. By definition of the base change
this means that the image of $h_{U_{ij}} \to h_U$ is contained
...
Note that $S \in J(U)$ by definition. Instead of constructing
...
$$
Take $\alpha \in S(T)$ for some object $T \in \mathcal{C}$.
...
= f_{i'} \circ \alpha_{i'}'$ is a second choice, then
...
be two sets of coverings defining the structure of a site on $\mathcal{C}$.
...
add to $\text{Cov}(\mathcal{C})$ any set of families of morphisms with fixed target
...
sieve $S \cap S'$ is a covering sieve too, see Lemma \ref{lemma-topology-basic}.
...
$S' \cap S$ which is also a covering sieve, see Lemma \ref{lemma-topology-basic}.
...
Suppose that $\varphi \in \Mor_{\textit{PSh}(\mathcal{C})}(S, L\mathcal{F})$.
...
Assumptions and notation are as in Theorem \ref{theorem-topology-and-topos}.
